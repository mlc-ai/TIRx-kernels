...
Kernels implemented directly in TIRx and admitted through portfolio-wide
correctness and performance validation. Their canonical modules retain the
supported workload and stable provenance; run logs and intermediate candidates
remain outside this package. See the
[measured speedups](tirx_kernels/curated/README.md) for the benchmark
...
- **KDA backward:**
  [`curated_kda_backward_packed`](tirx_kernels/curated/kda_backward_packed.py) ⟨sm_100a⟩
- **KDA decode:**
  [`curated_kda_decode_multishape`](tirx_kernels/curated/kda_decode_multishape.py) ⟨sm_100a⟩
- **MSA prefill:**
  [`curated_msa_prefill_multishape`](tirx_kernels/curated/msa_prefill_multishape.py) ⟨sm_100a⟩
- **MSA decode:**
  [`curated_msa_decode_multishape`](tirx_kernels/curated/msa_decode_multishape.py) ⟨sm_100a⟩
- **VSA forward:**
  [`curated_vsa_multishape`](tirx_kernels/curated/vsa_multishape.py) ⟨sm_100a⟩
- **DeepSeek-V4 sparse MLA:**
  [`curated_mla_dsv4_multishape`](tirx_kernels/curated/mla_dsv4_multishape.py) ⟨sm_100a⟩
