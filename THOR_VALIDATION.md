...
and reproduction command.  The adapters are enabled with `--with-references`;
the roster is `scripts/thor_flashinfer_representative.yaml`, and the report is
generated by `scripts/report_thor_native.py`.  FlashInfer is the timed baseline
where it exposes the same fused operation.  SGLang supplies production shape
provenance for GDN and grouped KDA rather than a second timing column.

The broader historical performance campaign includes every admitted Thor
kernel that has a default benchmark workload. It uses the same Proton timing
budgets as the repository's SM100/B200 data.
...
The B200 result is retained only as secondary porting evidence, not the primary
performance baseline. See
[THOR_B200_PERFORMANCE.md](THOR_B200_PERFORMANCE.md) for the complete historical
table, per-kernel summaries, GEMM effective throughput, round variability,
provenance, and comparison limitations. Generate it from the raw JSON with
...
The recorded native-baseline run used `MAXN` with dynamic clocks because
`jetson_clocks` requires root on this machine. Lock production power and clocks
before interpreting differences near the 5% band as tuning wins.
