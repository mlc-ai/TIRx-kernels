...
def _desc_add(desc, off16):
    ...
    low = K.Cast("uint64", K.Cast("uint32", desc) + K.Cast("uint32", off16))
...
def _replace_smem_desc_addr(desc, smem_ptr):
    ...
    return K.bitwise_or(K.bitwise_and(desc, K.bitwise_not(K.uint64(0x3FFF))), start)
...
def make_kernel(
    *, fp8, h_total, h_valid, groups, q_tokens, ktot, splits, bpc, nstage, swa_rows, comp_rows,
    ...
    stats_bytes = BM * 2 * 4
    esize = 1 if fp8 else 2
    dt = "float8_e4m3fn" if fp8 else K.bf16
    gt = K.u8 if fp8 else K.bf16
    ...
    mma_ws = _MMA_WS[fp8]
    p_swizzle = K.SW64B if fp8 else K.SW128B
    ...
    def host_prelude(params):
        ...
        if q_in_tmem:
            K.call_packed(
                ...
                descriptor, tmap_dtype, 4, K.handle_add_byte_offset(params["q"].data, 0),
            ...
            K.call_packed(
                "runtime.cuTensorMapEncodeTiled",
                descriptor, tmap_dtype, 2, K.handle_add_byte_offset(params["q"].data, 0),
                D, q_rows, row_bytes, atom, BM, 1, 1, 0, 3, 3, 0,
            )
        out_desc = K.stack_alloca("tensormap", 1)
        K.call_packed(
            "runtime.cuTensorMapEncodeTiled",
            out_desc, "bfloat16", 2, K.handle_add_byte_offset(params["out"].data, 0),
    ...
    def mla_dsv4_splitk(
        q: K.gptr[gt, (q_rows * D,)],
        swa: K.gptr[gt, (swa_rows * D,)],
        comp: K.gptr[gt, (comp_rows * D,)],
        indices: K.gptr[K.i32, (q_tokens * ktot,)],
        lens: K.gptr[K.i32, (q_tokens,)],
        sinks: K.gptr[K.f32, (h_total,)],
        out: K.gptr[K.bf16, (q_rows * D,)],
        scale_log2: K.f32,
        bmm2_scale: K.f32,
        item_base: K.i32,
        *,
        ...
        bid = K.cta_id()
        if C > 1:
            K.cta_id_in_cluster([C], preferred=[C])
        rank = bid % C
        ...
        k_tile = smem.alloc((nstage, 64, D), dt, swizzle=K.SW128B)
        if q_in_tmem:
            kv_end = pool.offset
            pool.move_base_to(kv_end - stage_bytes)
            q_tile = smem.alloc((BM, D), dt, swizzle=K.SW128B)
        ...
        q_ready = K.TMABar(pool, 1)
        q_tmem_ready = K.TCGen05Bar(pool, 1)
        k_ready = K.MBarrier(pool, nstage)
        mask_ready = K.MBarrier(pool, nstage)
        k_empty = K.TCGen05Bar(pool, nstage)
        s_ready = K.TCGen05Bar(pool, n_sbuf)
        s_free = K.MBarrier(pool, n_sbuf)
        p_ready = K.MBarrier(pool, 2)
        pv_half = K.TCGen05Bar(pool, 2)                                                            
        p_free = K.TCGen05Bar(pool, 2)                                                  
        recv_full = K.MBarrier(pool, ch_max)                                   
        peer_ready = K.MBarrier(pool, 1)                                                             
        stats_full = K.MBarrier(pool, 1)                                               
        smem.commit()

        def bar_addr(bar, i):
            return cvta(K.address_of(bar.buf[i]))

        def bar_init(bar, i, count):
            K.ptx["mbarrier.init.shared.b64"](bar_addr(bar, i), K.uint32(count))
        ...
        len_t = K.local_scalar("int32")
        K.ptx.ld.global_.s32(len_t, lens.ptr_to([tok]))
        kblk_valid = K.min(K.int32(kblk), (len_t + (BN - 1)) // BN)
        first_blk = rank * bpc
        n_blk = K.local_scalar("int32", init=K.max(K.min(kblk_valid - first_blk, K.int32(bpc)), 0))
        ...
        def role_sync():
            ...
            K.ptx["bar.sync"](K.uint32(1), K.uint32(nthreads))
            iket_end(t_ps)
        ...
        e_warp = K.bitwise_and(warp, K.int32(3))
        e_head = lane if small else 32 * K.bitwise_and(e_warp, 1) + lane
        sink_own = K.local_scalar("float32")

        def preload_sinks():
            K.ptx.ld.global_.f32(sink_own, sinks.ptr_to([K.min(grp * 64 + e_head, K.int32(h_total - 1))]))
        ...
        def loader():
            ...
            with K.If(warp == 0), K.Then():
                with K.If(K.cuda.elect_sync() != K.uint32(0)), K.Then():
                    ...
                    K.ptx["mbarrier.arrive.expect_tx.shared.b64"](
                        bar_addr(q_ready, 0), K.uint32(q_bytes)
                    )
                    if q_in_tmem:
                        K.ptx[_TMA_4D](
                            cvta(q_tile.ptr_to(0, 0)),
                            K.reinterpret(K.handle().ty, K.address_of(q_tmap)),
                            K.int32(0), grp * 64, K.int32(0), tok,
                            bar_addr(q_ready, 0), K.uint64(_Q_HINT),
                        )
                    else:
            ...
            ring = K.PipelineState(nstage, phase=0)
            sub = K.bitwise_and(lane, K.int32(7))                                       
            idx_next = K.local_scalar("int32", init=K.int32(-1))
            ...
            def load_idx(k_blk, gated=True):
                ...
                K.assign(idx_next, K.int32(-1))
                col_n = (first_blk + k_blk) * BN + 16 * lw + lane
                with K.If(K.And(lane < 16, k_blk < n_blk) if gated else lane < 16), K.Then():
                    if guard_cols:
                        with K.If(col_n < ktot), K.Then():
                            K.ptx.ld.global_.s32(idx_next, indices.ptr_to([tok * ktot + col_n]))
                    else:
                        K.ptx.ld.global_.s32(idx_next, indices.ptr_to([tok * ktot + col_n]))

            load_idx(K.int32(0), gated=False)
            preload_sinks()
            iket_end(t_pi)
            role_sync()
            cluster_arrive()
            with K.serial(n_blk, unroll=False) as k:
                t_idx = iket_range("ld-idx")
                gblk = first_blk + k
                col = gblk * BN + 16 * lw + lane                                
                idx = K.local_scalar("int32", init=idx_next)
                load_idx(k + 1)                                                                
                valid = K.And(K.And(idx >= 0, col < len_t), lane < 16)
                m = K.local_scalar("uint32")
                K.ptx.vote_sync.ballot.b32(m, K.ptx.pred(valid), K.uint32(0xFFFFFFFF))
                                                                            
                idx_q = [K.local_scalar("int32") for _ in range(4)]
                for rq in range(4):
                    K.ptx.shfl_sync.idx.b32(
                        idx_q[rq], idx, K.Cast("uint32", rq * 4 + K.shift_right(lane, K.int32(3))),
                        K.uint32(0x1F), K.uint32(0xFFFFFFFF),
                    )
                stage = ring.stage
                iket_end(t_idx)
                ...
                if q_in_tmem:
                    ...
                    with K.If(k == 2), K.Then():
                        ...
                        K.ptx["fence.proxy.async.shared::cta"]()
                with K.If(k >= nstage), K.Then():
                    hot_wait(k_empty, stage, K.bitwise_xor((k // nstage) & 1, 1))
                ...
                with K.If(lane == 0), K.Then():
                    K.ptx.st.shared.u16(maskbuf.ptr_to([stage, lw]), K.Cast("uint16", m))
                    K.ptx["mbarrier.arrive.shared.b64"](bar_addr(mask_ready, stage), K.uint32(1))
                kview = k_tile[stage]

                def gather_from(pool_buf, pool_rows):
                    for rq in range(4):
                        row = 16 * lw + rq * 4 + K.shift_right(lane, K.int32(3))
                        src_row = K.Cast(
                            "int64", K.min(K.max(idx_q[rq], K.int32(0)), K.int32(pool_rows - 1))
                        )
                        for a in range(natom):
                ...
                K.ptx[_CP_ASYNC_ARRIVE](k_ready.buf.ptr_to([stage]))
                iket_end(t_gi)
                ring.advance()
            cluster_wait()
            if C > 1:
                ...
                with K.If(K.cuda.elect_sync() != K.uint32(0)), K.Then():
                    ...
                    K.ptx["fence.proxy.async.shared::cta"]()
                    for r in range(C):
                        with K.If(K.And(r != rank, warp == r % N_LOADER_WARPS)), K.Then():
                            ra = K.local_scalar("uint32")
                            K.ptx["mapa.shared::cluster.u32"](ra, bar_addr(peer_ready, 0), K.uint32(r))
                            K.ptx["mbarrier.arrive.release.cluster.shared::cluster.b64"](ra)
                    iket_end(t_an)
        ...
        def qk_issuer():
            ...
            with K.If(K.cuda.elect_sync() != K.uint32(0)), K.Then():
                ...
                with K.serial(n_blk, unroll=False) as k:
                    ...
                    if not two_pass:
                        with K.If(k >= n_sbuf), K.Then():
                            hot_wait(s_free, sbuf, K.bitwise_xor((k // n_sbuf) & 1, 1))
                    iket_end(t_wk)
                    t_qk = iket_range("mm-qk")
                                                                                        
                    K.ptx["fence.proxy.async.shared::cta"]()
                    K.ptx["tcgen05.fence::after_thread_sync"]()
                    s_col = K.uint32(S_BASE) + K.Cast("uint32", sbuf) * K.uint32(S_STRIDE)
                    if q_in_tmem:
                        for ks in range(16):
                            ...
                            K.ptx[mma_ws](
                                ...
                                K.uint32(idesc_qk),
                                K.ptx.pred(K.Cast("bool", K.uint32(1 if ks > 0 else 0))),
                                K.uint64(0),
                            )
                    else:
                    ...
                    K.ptx[_COMMIT](bar_addr(s_ready, sbuf))
                    iket_end(t_qk)
                    ring.advance()
                    sb.advance()
            cluster_wait()
            if C > 1:
                ...
                K.ptx["bar.sync"](K.uint32(11), K.uint32(32 + (32 if small else 64)))
                K.ptx["fence.proxy.async.shared::cta"]()
                with K.If(K.cuda.elect_sync() != K.uint32(0)), K.Then():
                    for dest in range(C):
                        with K.If(dest != rank), K.Then():
                            rb = K.local_scalar("uint32")
                            K.ptx["mapa.shared::cluster.u32"](rb, bar_addr(stats_full, 0), K.uint32(dest))
                            rs = K.local_scalar("uint32")
                            K.ptx["mapa.shared::cluster.u32"](
                                rs, cvta(stats_all.ptr_to([rank, 0, 0])), K.uint32(dest)
                            )
                            K.ptx[_BULK_S2C](
                                rs, cvta(stats_all.ptr_to([rank, 0, 0])), K.uint32(stats_bytes), rb,
                            )
                iket_end(t_sp)
        ...
        def pv_issuer():
            ...
            with K.If(K.cuda.elect_sync() != K.uint32(0)), K.Then():
                ...
                with K.serial(n_blk, unroll=False) as k:
                    ...
                    K.ptx["fence.proxy.async.shared::cta"]()
                    K.ptx["tcgen05.fence::after_thread_sync"]()
                    accum0 = K.local_scalar(
                        "uint32", init=K.if_then_else(k > 0, K.uint32(1), K.uint32(0))
                    )
                    for half in range(2):
                        for ks in range(pv_steps):
                            K.ptx[mma_ws](
                                K.uint32(O_BASE + half * (64 if small else 128)),
                                ...
                                K.ptx.pred(
                                    K.Cast(
                                        "bool",
                                        K.bitwise_or(accum0, K.uint32(1 if ks > 0 else 0)),
                                    )
                                ),
                                K.uint64(0),
                            )
                        if two_pass:
                            ...
                            with K.If(k == n_blk - 1), K.Then():
                                K.ptx[_COMMIT](bar_addr(pv_half, half))
                    K.ptx[_COMMIT](bar_addr(k_empty, stage))
                                                                                            
                                                                                           
                                                                                   
                    K.ptx[_COMMIT](bar_addr(p_free, sbuf))
                    iket_end(t_pv)
                    ring.advance()
                    sb.advance()
            cluster_wait()
        ...
        def load_s(sbuf, dst):
            s_col = K.uint32(S_BASE) + K.Cast("uint32", sbuf) * K.uint32(S_STRIDE)
            if q_in_tmem:
                peer = K.alloc_local((32,), "float32")
                with K.If(warp < N_LOADER_WARPS + 2):
                    with K.Then():
                        K.ptx[_TMEM_LD32](
                            *[dst[i] for i in range(32)], K.cuda.get_tmem_addr(s_col, 0, 0)
                        )
                        K.ptx[_TMEM_LD32](
                            *[peer[i] for i in range(32)], K.cuda.get_tmem_addr(s_col, 0, 32)
                        )
                    with K.Else():
                        K.ptx[_TMEM_LD32](
                            *[peer[i] for i in range(32)], K.cuda.get_tmem_addr(s_col, 0, 0)
                        )
                        K.ptx[_TMEM_LD32](
                            *[dst[i] for i in range(32)], K.cuda.get_tmem_addr(s_col, 0, 32)
                        )
                ...
                for i in range(8):
                    ...
                    K.ptx["st.shared.v4.u32"](
                        score_exchange.ptr_to([K.bitwise_xor(w, 2), off]),
                        peer_words[4 * i], peer_words[4 * i + 1],
                        peer_words[4 * i + 2], peer_words[4 * i + 3],
                    )
                K.ptx["bar.sync"](
                    K.uint32(8) + K.Cast("uint32", K.bitwise_and(w, 1)), K.uint32(64)
                )
                incoming = K.alloc_local((4,), "float32")
                ...
                for i in range(8):
                    ...
                    for j in range(4):
                        K.assign(dst[4 * i + j], dst[4 * i + j] + incoming[j])
            elif small:
                K.ptx[_TMEM_LD16](*[dst[i] for i in range(16)], K.cuda.get_tmem_addr(s_col, 0, 0))
            else:
                K.ptx[_TMEM_LD32](*[dst[i] for i in range(32)], K.cuda.get_tmem_addr(s_col, 0, 0))
        ...
        def scaled_masked(s, x, stage, shalf):
            ...
            if small:
                K.ptx.ld.shared.u16(mask, maskbuf.ptr_to([stage, shalf]))
            else:
                K.ptx.ld.shared.u32(mask, maskbuf.ptr_to([stage, shalf * 2]))
            if h_total == 64 and q_tokens <= 64:
                ...
                mloc = [
                    K.local_scalar("float32", init=K.float32(NEG_INF))
                    for _ in range(4)
                ]
                for base in range(0, score_elems, 8):
                    for j in range(base, base + 8):
                        vj = (
                            K.bitwise_and(
                                K.shift_right(mask, K.uint32(j)), K.uint32(1)
                            )
                            != K.uint32(0)
                        )
                        K.assign(
                            x[j],
                            K.if_then_else(
                                vj, s[j] * scale_log2, K.float32(NEG_INF)
                            ),
                        )
                ...
                m012 = K.local_scalar("float32")
                K.ptx.max.f32(m012, mloc[0], mloc[1], mloc[2])
                return K.max(m012, mloc[3])
            mloc = K.local_scalar("float32", init=K.float32(NEG_INF))
            for j in range(score_elems):
                vj = K.bitwise_and(K.shift_right(mask, K.uint32(j)), K.uint32(1)) != K.uint32(0)
                K.assign(x[j], K.if_then_else(vj, s[j] * scale_log2, K.float32(NEG_INF)))
                K.assign(mloc, K.max(mloc, x[j]))
            return mloc

        def exp_pack(x, m_new, pw):
            """pw <- P row (bf16/fp8) for x - m_new; returns the row sum."""
            psum = K.local_scalar("float32", init=K.float32(0.0))
            e = K.alloc_local((score_elems,), "float32")
            for j in range(score_elems):
                K.ptx["ex2.approx.ftz.f32"](e[j], x[j] - m_new)
                K.assign(psum, psum + e[j])
            if fp8:
                for i in range(score_elems // 4):
                    K.assign(
                        pw[i],
                        K.cuda.fp8x4_e4m3_from_float4(e[4 * i], e[4 * i + 1], e[4 * i + 2], e[4 * i + 3]),
                    )
            else:
                for i in range(score_elems // 2):
                    K.ptx.cvt.rn.bf16x2.f32(pw[i], e[2 * i + 1], e[2 * i])
            return psum
        ...
        def softmax():
            ...
            cluster_arrive()
            w = warp - N_LOADER_WARPS
            head = lane if small else 32 * K.bitwise_and(w, 1) + lane
            shalf = w if small else K.shift_right(w, 1)
            m_run = K.local_scalar("float32", init=K.float32(NEG_INF))
            l_run = K.local_scalar("float32", init=K.float32(0.0))
            ptid = w * 32 + lane                                       
            pw = K.alloc_local((score_elems * esize // 4,), "uint32")

            def reduce_max(buf_idx, own):
                K.ptx.st.shared.f32(rowbuf.ptr_to([buf_idx, ptid]), own)
                if small:
                    K.ptx["bar.sync"](K.uint32(8), K.uint32(128))
                    total = K.local_scalar("float32", init=own)
                    for wr in range(4):
                        peer = K.local_scalar("float32")
                        K.ptx.ld.shared.f32(peer, rowbuf.ptr_to([buf_idx, wr * 32 + lane]))
                        K.assign(total, K.max(total, peer))
                    return total
                K.ptx["bar.sync"](
                    K.uint32(8) + K.Cast("uint32", K.bitwise_and(w, 1)), K.uint32(64)
                )
                peer = K.local_scalar("float32")
                K.ptx.ld.shared.f32(peer, rowbuf.ptr_to([buf_idx, K.bitwise_xor(w, 2) * 32 + lane]))
                return K.max(own, peer)
            if two_pass:
                                                                                       
                xs = [K.alloc_local((score_elems,), "float32") for _ in range(TWO_PASS_MAX_BLOCKS)]
                mloc = K.local_scalar("float32", init=K.float32(NEG_INF))
                for k in range(TWO_PASS_MAX_BLOCKS):
                    with K.If(k < n_blk), K.Then():
                        ...
                        K.ptx["tcgen05.fence::after_thread_sync"]()
                        s = K.alloc_local((score_elems,), "float32")
                        load_s(k, s)
                        K.ptx["tcgen05.wait::ld.sync.aligned"]()
                        mk = scaled_masked(s, xs[k], k % nstage, shalf)
                        K.assign(mloc, K.max(mloc, mk))
                        iket_end(t_math)
                K.assign(m_run, reduce_max(K.int32(0), mloc))
                                                                                            
                for k in range(TWO_PASS_MAX_BLOCKS):
                    with K.If(k < n_blk), K.Then():
                        t_ps = iket_range("sm-pstore")
                        with K.If(m_run == K.float32(NEG_INF)):
                            with K.Then():
                                for i in range(score_elems * esize // 4):
                                    K.assign(pw[i], K.uint32(0))
                            with K.Else():
                                psum = exp_pack(xs[k], m_run, pw)
                                K.assign(l_run, l_run + psum)
                        if k >= 2:
                            hot_wait(p_free, k % 2, ((k // 2) & 1) ^ 1)
                            K.ptx["fence.proxy.async.shared::cta"]()
                        store_p(k % 2, head, shalf, pw)
                        K.ptx["fence.proxy.async.shared::cta"]()
                        K.ptx["mbarrier.arrive.shared.b64"](bar_addr(p_ready, k % 2), K.uint32(1))
                        iket_end(t_ps)
            else:
                ring = K.PipelineState(nstage, phase=0)
                sb = K.PipelineState(n_sbuf, phase=0)
                with K.serial(n_blk, unroll=False) as k:
                    ...
                    K.ptx["mbarrier.arrive.shared.b64"](bar_addr(s_free, sbuf), K.uint32(1))
                    x = K.alloc_local((score_elems,), "float32")
                    mloc = scaled_masked(s, x, stage, shalf)
                    kb = K.bitwise_and(k, 1)
                    m_blk = reduce_max(kb, mloc)
                                                                                          
                    need = K.local_scalar("uint32")
                    K.ptx.vote_sync.any.pred(
                        need, m_blk - m_run > K.float32(RESCALE_THRESHOLD), K.uint32(0xFFFFFFFF)
                    )
                    m_new = K.local_scalar(
                        "float32",
                        init=K.if_then_else(
                            K.Or(need != K.uint32(0), m_run == K.float32(NEG_INF)),
                            K.max(m_run, m_blk), m_run,
                        ),
                    )
                    alpha = K.local_scalar("float32", init=K.float32(1.0))
                    with K.If(m_new == K.float32(NEG_INF)):
                        with K.Then():
                            for i in range(score_elems * esize // 4):
                                K.assign(pw[i], K.uint32(0))
                        with K.Else():
                            K.ptx["ex2.approx.ftz.f32"](alpha, m_run - m_new)
                            psum = exp_pack(x, m_new, pw)
                            lt = K.local_scalar("float32")
                            K.ptx.fma.rn.f32(lt, l_run, alpha, psum)
                            K.assign(l_run, lt)
                    K.assign(m_run, m_new)
                    iket_end(t_math)
                    t_ps = iket_range("sm-pstore")
                    with K.If(k >= 2), K.Then():
                        hot_wait(p_free, K.bitwise_and(k, 1), K.bitwise_xor((k // 2) & 1, 1))
                        K.ptx["fence.proxy.async.shared::cta"]()                              
                    store_p(K.bitwise_and(k, 1), head, shalf, pw)
                    iket_end(t_ps)
                    t_wpv = iket_range("sm-wait-pv-rescale")
                    with K.If(K.And(k > 0, need != K.uint32(0))), K.Then():
                                                                             
                        hot_wait(p_free, (k - 1) % 2, ((k - 1) // 2) & 1)
                        with K.If(K.uint32(1) != K.uint32(0)), K.Then():
                            K.ptx["tcgen05.fence::after_thread_sync"]()
                            o = K.alloc_local((32,), "float32")
                            for c in range(4 if small else 8):
                                addr = K.cuda.get_tmem_addr(K.uint32(O_BASE), 0, c * 32)
                                K.ptx[_TMEM_LD32](*[o[i] for i in range(32)], addr)
                                K.ptx["tcgen05.wait::ld.sync.aligned"]()
                                for i in range(32):
                                    K.assign(o[i], o[i] * alpha)
                                K.ptx[_TMEM_ST32](addr, *[o[i] for i in range(32)])
                            K.ptx["tcgen05.wait::st.sync.aligned"]()
                            K.ptx["tcgen05.fence::before_thread_sync"]()
                    K.ptx["fence.proxy.async.shared::cta"]()
                    K.ptx["mbarrier.arrive.shared.b64"](bar_addr(p_ready, K.bitwise_and(k, 1)), K.uint32(1))
                    iket_end(t_wpv)
                    ring.advance()
                    sb.advance()
            ...
            K.ptx.st.shared.f32(lbuf.ptr_to([ptid]), l_run)
            if small:
                K.ptx["bar.sync"](K.uint32(8), K.uint32(128))
                l_tot = K.local_scalar("float32", init=K.float32(0.0))
                for wr in range(4):
                    peer_l = K.local_scalar("float32")
                    K.ptx.ld.shared.f32(peer_l, lbuf.ptr_to([wr * 32 + lane]))
                    K.assign(l_tot, l_tot + peer_l)
            else:
                K.ptx["bar.sync"](
                    K.uint32(8) + K.Cast("uint32", K.bitwise_and(w, 1)), K.uint32(64)
                )
                peer_l = K.local_scalar("float32")
                K.ptx.ld.shared.f32(peer_l, lbuf.ptr_to([K.bitwise_xor(w, 2) * 32 + lane]))
                l_tot = K.local_scalar("float32", init=l_run + peer_l)
                                                                                            
            cluster_wait()
            with K.If(w == 0 if small else shalf == 0), K.Then():
                K.ptx.st.shared.f32(stats_all.ptr_to([rank, head, 0]), m_run)
                K.ptx.st.shared.f32(stats_all.ptr_to([rank, head, 1]), l_tot)
                if C > 1:
                                                                                                  
                    K.ptx["bar.arrive"](K.uint32(11), K.uint32(32 + (32 if small else 64)))
            if two_pass and not small:
                                                                                                 
                K.ptx["bar.arrive"](K.uint32(3), K.uint32(256))
            iket_end(t_stats)
        ...
        def epilogue():
            ...
            conflict-free.
            """
            ew = K.bitwise_and(warp, K.int32(3))                             
            head = lane if small else 32 * K.bitwise_and(ew, 1) + lane
            shalf = K.int32(0) if small else K.shift_right(ew, 1)
            is_smx = K.bool(True) if small else warp >= N_LOADER_WARPS
            etid = (warp - N_LOADER_WARPS) * 32 + lane if small else warp * 32 + lane
            zero_o = n_blk == 0
            hsw = K.bitwise_and(head, K.int32(7))                                        
            pid = ew if small else K.shift_right(warp, K.int32(1))                          
            is_leader = lane == 0 if small else K.And(K.bitwise_and(warp, K.int32(1)) == 0, lane == 0)

            def pair_sync():
                K.ptx["bar.sync"](K.uint32(4) + K.Cast("uint32", pid), K.uint32(32 if small else 64))

            def o_col(c):
                return O_BASE + (c % 2) * 64 + (c // 4) * 128

            oa = K.alloc_local((32,), "float32")
            ob = K.alloc_local((32,), "float32")
            pk = K.alloc_local((32,), "uint32")
            regs = (oa, ob)                                                         
            ...
            def fetch(c):
                if two_pass:
                    ...
                    K.ptx["tcgen05.fence::after_thread_sync"]()
                if small:
                    base = K.uint32(O_BASE) + K.Cast("uint32", c // 4) * K.uint32(64)
                    K.ptx[_TMEM_LD32](*[oa[i] for i in range(32)], K.cuda.get_tmem_addr(base, 0, 0))
                    K.ptx[_TMEM_LD32](*[ob[i] for i in range(32)], K.cuda.get_tmem_addr(base, 0, 32))
                else:
                    K.ptx[_TMEM_LD32](
                        *[oa[i] for i in range(32)], K.cuda.get_tmem_addr(K.uint32(0), 0, o_col(c))
                    )
                    K.ptx[_TMEM_LD32](
                        *[ob[i] for i in range(32)], K.cuda.get_tmem_addr(K.uint32(0), 0, o_col(c) + 32)
                    )
                K.ptx["tcgen05.wait::ld.sync.aligned"]()
            ...
            def clear_if_empty():
                ...
                with K.If(zero_o), K.Then():
                    for d in range(64):
                        K.assign(elem(d), K.float32(0.0))

            def scale_regs(scale_v):
                scale2 = K.local_scalar("uint64")
                K.ptx.mov.b64(scale2, scale_v, scale_v)
                for d in range(0, 64, 2):
                    vals2 = K.local_scalar("uint64")
                    K.ptx.mov.b64(vals2, elem(d), elem(d + 1))
                    K.ptx["mul.rn.ftz.f32x2"](vals2, vals2, scale2)
                    K.ptx.mov.b64(elem(d), elem(d + 1), vals2)

            def pack_piece(i):
                """pk[4i..4i+3] <- bf16x2 of dims 8i..8i+7."""
                for w in range(4 * i, 4 * i + 4):
                    K.ptx.cvt.rn.bf16x2.f32(pk[w], elem(2 * w + 1), elem(2 * w))

            def stage_piece(c, i):
                piece = K.bitwise_xor(K.int32(i), hsw)
                K.ptx["st.shared.v4.u32"](
                    cvta(K.ptr_byte_offset(staging.ptr_to([0]), c * CHUNK + head * 128 + piece * 16, "uint16")),
            ...
            def tma_store_chunk(c):
                ...
                K.ptx[_TMA_S2G_2D](
                    K.reinterpret(K.handle().ty, K.address_of(out_tmap)),
                    K.int32(c * 64),
                    tok * h_total + grp * 64,
                    cvta(K.ptr_byte_offset(staging.ptr_to([0]), c * CHUNK, "uint16")),
                )
            ...
            def head_scales(h, sink_v):
                ...
                mr = [K.local_scalar("float32") for _ in range(C)]
                lr = [K.local_scalar("float32") for _ in range(C)]
                for r in range(C):
                    K.ptx.ld.shared.f32(mr[r], stats_all.ptr_to([r, h, 0]))
                    K.ptx.ld.shared.f32(lr[r], stats_all.ptr_to([r, h, 1]))
                m_all = K.local_scalar("float32", init=mr[0])
                for r in range(1, C):
                    K.assign(m_all, K.max(m_all, mr[r]))
                fr = [K.local_scalar("float32") for _ in range(C)]
                l_all = K.local_scalar("float32", init=K.float32(0.0))
                for r in range(C):
                    K.ptx["ex2.approx.ftz.f32"](fr[r], mr[r] - m_all)
                    K.assign(fr[r], K.if_then_else(mr[r] == K.float32(NEG_INF), K.float32(0.0), fr[r]))
                    K.assign(l_all, l_all + lr[r] * fr[r])
                sink_e = K.local_scalar("float32")
                K.ptx["ex2.approx.ftz.f32"](sink_e, sink_v * K.float32(LOG2E) - m_all)
                inv = K.local_scalar(
                    "float32",
                    init=K.if_then_else(
                        m_all == K.float32(NEG_INF), K.float32(0.0),
                        K.cuda.fdividef(bmm2_scale, l_all + sink_e),
                    ),
                )
                return [K.local_scalar("float32", init=fr[r] * inv) for r in range(C)]

            def select_rank(vals, r_dyn):
                v = K.local_scalar("float32", init=vals[0])
                for r in range(1, C):
                    K.assign(v, K.if_then_else(r_dyn == r, vals[r], v))
                return v

                                                                                           
            cbase = ew if small else K.if_then_else(is_smx, K.int32(0), K.int32(4)) + 2 * shalf
            ...
            n_mine = K.local_scalar("int32", init=(K.int32(8) - rank + C - 1) // C)
            with K.If(etid == 0), K.Then():
                K.ptx["mbarrier.arrive.expect_tx.shared.b64"](
                    bar_addr(stats_full, 0), K.uint32((C - 1) * stats_bytes)
                )
                for s_ in range(ch_max):
                    with K.If(s_ < n_mine), K.Then():
                        K.ptx["mbarrier.arrive.expect_tx.shared.b64"](
                            bar_addr(recv_full, s_), K.uint32((C - 1) * XCHUNK)
                        )
            ...
            def send_chunk(c):
                ...
                pair_sync()
                with K.If(is_leader), K.Then():
                    K.cuda.mbarrier_wait_acquire_cluster(K.address_of(peer_ready.buf[0]), 0)
                    src_idx = rank - K.if_then_else(rank > dest, K.int32(1), K.int32(0))
                    rb = K.local_scalar("uint32")
                    K.ptx["mapa.shared::cluster.u32"](rb, bar_addr(recv_full, slot), K.Cast("uint32", dest))
                    raddr = K.local_scalar("uint32")
                    K.ptx["mapa.shared::cluster.u32"](
                        raddr,
                        cvta(K.ptr_byte_offset(recv.ptr_to([0]), (slot * (C - 1) + src_idx) * XCHUNK, "uint16")),
                        K.Cast("uint32", dest),
                    )
                    K.ptx[_BULK_S2C](
                        raddr,
            ...
            def reduce_chunk(c):
                ...
                K.cuda.mbarrier_wait_acquire_cluster(K.address_of(stats_full.buf[0]), 0)
                iket_end(t_ws)
                sc2 = [K.local_scalar("uint64", init=K.uint64(0)) for _ in range(C - 1)]
                with K.If(head < HGc), K.Then():
                    scales = head_scales(head, sink_own)
                    clear_if_empty()
                    scale_regs(select_rank(scales, rank))
                    for si in range(C - 1):
                        r_dyn = K.int32(si) + K.if_then_else(K.int32(si) >= rank, K.int32(1), K.int32(0))
                        sc_r = select_rank(scales, r_dyn)
                        K.ptx.mov.b64(sc2[si], sc_r, sc_r)
                t_wr = iket_range("ep-wait-recv")
                K.cuda.mbarrier_wait_acquire_cluster(K.address_of(recv_full.buf[slot]), 0)
                iket_end(t_wr)
                t_red = iket_range("ep-reduce-store")
                with K.If(head < HGc), K.Then():
                    if compact_exchange:
                        for i in range(4):
                            words = [K.alloc_local((4,), "uint32") for _ in range(C - 1)]
                            for si in range(C - 1):
                                K.ptx["ld.shared.v4.u32"](
                            ...
                            for si in range(C - 1):
                                for q in range(4):
                                    pairs = K.alloc_local((2,), "uint16")
                                    K.ptx.mov.b32(pairs[0], pairs[1], words[si][q])
                                    for hp in range(2):
                                        bf16_pair = K.local_scalar("uint32")
                                        K.ptx.cvt.rn.bf16x2.e4m3x2(bf16_pair, pairs[hp])
                                        lo = K.cuda.uint_as_float(
                                            K.shift_left(bf16_pair, K.uint32(16))
                                        )
                                        hi = K.cuda.uint_as_float(
                                            K.bitwise_and(bf16_pair, K.uint32(0xFFFF0000))
                                        )
                                        d = 16 * i + 4 * q + 2 * hp
                                        vals2 = K.local_scalar("uint64")
                                        acc2 = K.local_scalar("uint64")
                                        K.ptx.mov.b64(vals2, lo, hi)
                                        K.ptx.mov.b64(acc2, elem(d), elem(d + 1))
                                        K.ptx["fma.rn.ftz.f32x2"](acc2, vals2, sc2[si], acc2)
                                        K.ptx.mov.b64(elem(d), elem(d + 1), acc2)
                            for oi in range(2):
                                pack_piece(2 * i + oi)
                        ...
                        for i in range(8):
                            ...
                            for si in range(C - 1):
                                for q in range(4):
                                    w = 4 * i + q
                                    lo = K.cuda.uint_as_float(
                                        K.shift_left(words[si][q], K.uint32(16))
                                    )
                                    hi = K.cuda.uint_as_float(
                                        K.bitwise_and(words[si][q], K.uint32(0xFFFF0000))
                                    )
                                    vals2 = K.local_scalar("uint64")
                                    acc2 = K.local_scalar("uint64")
                                    K.ptx.mov.b64(vals2, lo, hi)
                                    K.ptx.mov.b64(acc2, elem(2 * w), elem(2 * w + 1))
                                    K.ptx["fma.rn.ftz.f32x2"](acc2, vals2, sc2[si], acc2)
                                    K.ptx.mov.b64(elem(2 * w), elem(2 * w + 1), acc2)
                            pack_piece(i)
                            stage_piece(c, i)
                K.ptx["fence.proxy.async.shared::cta"]()
                pair_sync()
        ...
        if two_pass:
            ...
            with K.If(K.And(warp >= N_LOADER_WARPS, warp < 8)), K.Then():
                epilogue()
            if not small:
                with K.If(warp < N_LOADER_WARPS), K.Then():
                    t_cb = iket_range("cta-bar")
                    K.ptx["bar.sync"](K.uint32(3), K.uint32(256))                        
                    iket_end(t_cb)
                    epilogue()
        else:
                                                                                               
                                                   
            with K.If(K.And(warp >= N_LOADER_WARPS, warp < 8) if small else warp < 8), K.Then():
                t_cb = iket_range("cta-bar")
                K.ptx["bar.sync"](K.uint32(3), K.uint32(128 if small else 256))
        ...
        if C > 1:
            ...
            K.cuda.cta_sync()
        iket_end(t_fin)
        with K.If(warp == 8), K.Then():
            K.ptx["tcgen05.dealloc.cta_group::1.sync.aligned.b32"](
                K.uint32(0), K.uint32(TMEM_USED)
            )
...
def get_kernel(**config: Any):
    """Return the traced Kern PrimFunc this config's planner selects."""
