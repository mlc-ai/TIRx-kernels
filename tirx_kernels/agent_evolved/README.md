...
| [`agent_evolved_alphamoe_fp8_blockscale_qwen3next`](alphamoe_fp8_blockscale_qwen3next.py) | `m128_official` | GB200 | CUDA event | 63.750 | FlashInfer `trtllm_fp8_block_scale_routed_moe` | 117.939 | 1.8500x | `run_bench(timer="event")` same-run pair, reproduced at 1.8500x/1.8524x over two runs; the baseline is CUDA-graph captured exactly as the evaluation harness captures it, and the default Proton timer is *not* used because its per-kernel instrumentation overstates this many-kernel baseline (2.12x). The locked evaluation harness, timing with CUPTI and flushing L2 per iteration, scores 59.984 us vs 112.537 us (1.8761x), verdict STABLE over 22 checks, max_rms_ratio 9.038e-05, five runs 59.984/60.040/60.096/60.368/60.471 us. Evolution run `alphamoe_m128_e512_topk10_k2048_i128_fp8-20260915-183022` member `split-grid-self-first` on official row `alphamoe-qwen3-next-tp4-m128-e512-top10-k2048-i128-fp8`. |
