...
def get_kernel(**kwargs: Any):
    ...
    def sm100_fp8_paged_mqa_logits(
        ...
        # measured SASS byte-identical to the retired txl.Bind on 5 specializations,
        ...
        lane_idx_u32 = txl.Cast("uint32", txl.cuda.mov_sreg(32, "laneid"))

        with txl.If(warp_idx == spec_warp_start), txl.Then():
            txl.ptx.prefetch.tensormap(txl.address_of(tensor_map_q))
            txl.ptx.prefetch.tensormap(txl.address_of(tensor_map_kv))
            txl.ptx.prefetch.tensormap(txl.address_of(tensor_map_kv_scales))
        ...
        smem_kv_scales = smem.alloc((num_math_warpgroups, num_kv_stages, umma_m), txl.f32, align=16)
        ...
        empty_kv_barriers = txl.MBarrier(smem, num_kv_stages * num_math_warpgroups)
        full_umma_barriers = txl.TCGen05Bar(smem, num_umma_barriers)
        empty_umma_barriers = txl.MBarrier(smem, num_umma_barriers)
        ...
        def exist_q_atom_idx_expr(q_atom_idx, end_q_atom_idx, end_kv_idx):
            return txl.Or(
                ...
                txl.And(q_atom_idx == end_q_atom_idx, txl.uint32(0) < end_kv_idx),
            )

        def load_num_kv(q_atom_idx_arg, runtime_batch_size_arg):
            context_len = txl.local_scalar("uint32")
            if config.varlen:
                context_idx = txl.local_scalar("uint32", init=q_atom_idx_arg)
                with txl.If(q_atom_idx_arg + txl.uint32(1) < runtime_batch_size_arg), txl.Then():
                    index_pair = txl.alloc_local([2], "int32")
                    txl.ptx.ld.global_.s32(
                        index_pair[0], indices.ptr_to([txl.Cast("int32", q_atom_idx_arg)])
                    )
                    txl.ptx.ld.global_.s32(
                        index_pair[1],
                        indices.ptr_to([txl.Cast("int32", q_atom_idx_arg + txl.uint32(1))]),
                    )
                    with txl.If(index_pair[0] == index_pair[1]), txl.Then():
                        txl.assign(context_idx, q_atom_idx_arg + txl.uint32(1))
                txl.ptx.ld.global_.u32(
                    context_len, context_lens_flat.ptr_to([txl.Cast("int32", context_idx)])
                )
            else:
                q_idx = q_atom_idx_arg // txl.uint32(num_next_n_atoms)
                lens_idx = q_idx * txl.uint32(config.next_n) + txl.uint32(config.next_n - 1)
                txl.ptx.ld.global_.u32(
                    context_len, context_lens_flat.ptr_to([txl.Cast("int32", lens_idx)])
                )
        ...
        def make_smem_desc(desc, smem_ptr):
            txl.cuda.tcgen05.encode_matrix_descriptor(
                txl.address_of(desc), smem_ptr, ldo=0, sdo=desc_sdo, swizzle=desc_swizzle
            )
        ...
        def issue_tma_q(stage_idx, tma_q_atom_idx):
            with txl.If(txl.cuda.elect_sync() != txl.uint32(0)), txl.Then():
                ...
                txl.ptx[TMA_G2S_2D](
                    ...
                    txl.int32(0),
                    txl.Cast("int32", q_token_idx * txl.uint32(num_heads)),
        ...
        start_kv_idx = txl.local_scalar("uint32")
        end_kv_idx = txl.local_scalar("uint32")
        txl.assign(start_kv_idx, start_kv_tile_idx * txl.uint32(num_tiles_per_split))
        txl.assign(end_kv_idx, end_kv_tile_idx * txl.uint32(num_tiles_per_split))
        ...
        start_num_kv = txl.local_scalar("uint32", init=num_kv_result)
        ...
        with txl.If(txl.Or(warp_idx == tma_warp_0, warp_idx == tma_warp_1)), txl.Then():
            for pf_i in range((num_prefetch_lines + 63) // 64):
                ...
                # `txl.uint32` and the generated CUDA shows `line_idx_ptr[0]`.
                # Left as an expression it is re-substituted at every use, which
                # re-reads laneid each time (17 sreg reads vs the original's 5).
                line_idx = txl.local_scalar(
                    "uint32",
                    init=(warp_idx_u32 - txl.uint32(tma_warp_0)) * txl.uint32(32)
                    + lane_idx_u32
                    + txl.uint32(pf_i * 64),
                )
                with txl.If(line_idx < txl.uint32(num_prefetch_lines)), txl.Then():
                    txl.ptx.prefetch.global_.L2(
                        block_table_flat.ptr_to([txl.Cast("int64", line_idx * txl.uint32(32))])
                    )
        ...
        with txl.If(warp_idx == umma_warp_0), txl.Then():
            with txl.If(txl.cuda.elect_sync() != txl.uint32(0)), txl.Then():
                for init_i in range(num_kv_stages):
                    ...
                    txl.ptx.mbarrier.init.shared.b64(
                        empty_kv_barriers.ptr_to([num_kv_stages + init_i]), txl.uint32(4)
                    )
                txl.ptx.fence.mbarrier_init.release.cluster()
        ...
        def scheduler_state():
            ...
            state["fetched"] = txl.alloc_local([1], "uint32")
            ...
            for name in ("kv_idx", "num_kv"):
                txl.assign(state[name][0], txl.uint32(0))
        ...
        def pump(state):
            ...
            txl.assign(state["fetched"][0], scheduler_result[3])
        ...
        def load_block_table(state, kv_ptr, cached, lane_offset_tiles):
            ...
            with txl.If(kv_ptr[0] == txl.uint32(32)), txl.Then():
                ...
                block_table_offset = txl.local_scalar(
                    "uint64",
                    init=txl.Cast("uint64", atom_to_block_table_row_expr(state["q_atom"][0]))
                    * txl.Cast("uint64", block_table_stride),
                )
                prefetch_tile_idx = txl.local_scalar(
                    "uint32",
                    init=state["kv_idx"][0]
                    + txl.uint32(lane_offset_tiles)
                    + lane_idx_u32 * txl.uint32(num_tiles_per_split),
                )
                block_table_index = txl.local_scalar(
                    "uint64",
                    init=block_table_offset
                    + txl.Cast("uint64", prefetch_tile_idx * txl.uint32(num_pages_per_tile)),
                )
                for block_i in range(num_pages_per_tile):
                    ...
                    with txl.If(
                        txl.And(
                            ...
                            prefetch_tile_idx * txl.uint32(num_pages_per_tile) + txl.uint32(block_i)
                        ...
                        with txl.Then():
                            txl.ptx.ld.global_.u32(
                                ...
                                block_table_flat.ptr_to(
                                    [txl.Cast("int64", block_table_index + txl.uint64(block_i))]
                                ),
                            )
                        with txl.Else():
            ...
            with txl.If(txl.cuda.elect_sync() != txl.uint32(0)), txl.Then():
                for block_i in range(num_pages_per_tile):
                    txl.ptx[TMA_G2S_3D](
                        ...
                        full_kv_barriers.ptr_to([txl.uint32(base) + kv_state.stage]),
                        cache_policy_evict_normal,
                    )
                    txl.ptx[TMA_G2S_2D](
                        smem_kv_scales.ptr_to([group, kv_state.stage, block_i * page_size]),
                        txl.address_of(tensor_map_kv_scales),
                        txl.int32(0),
                        txl.Cast("int32", kv_block_idx[block_i]),
                        full_kv_barriers.ptr_to([txl.uint32(base) + kv_state.stage]),
                        cache_policy_evict_normal,
                ...
                full_kv_barriers.arrive(
                    txl.uint32(base) + kv_state.stage,
        ...
        with tma0:
            ...
            q_state = txl.RingState(num_q_stages, stage=1)
            kv_state = txl.RingState(num_kv_stages)
            pump(state)
            with txl.If(state["fetched"][0] != txl.uint32(0)), txl.Then():
                issue_tma_q(txl.uint32(0), state["next_q"][0])
            kv_ptr = txl.alloc_local([1], "uint32")
            cached = txl.alloc_local([num_pages_per_tile], "uint32")
            txl.assign(kv_ptr[0], txl.uint32(32))
            with txl.While(state["fetched"][0] != txl.uint32(0)):
                load_atom_advance(state["next_q"][0], batch_size)
                next_advance = txl.local_scalar("uint32", init=atom_advance_result)
                prefetch_q = txl.local_scalar("uint32", init=txl.uint32(0))
                with (
                    txl.If(
                        txl.And(
                    ...
                    txl.assign(prefetch_q, txl.uint32(1))
                ...
                with txl.If(prefetch_q != txl.uint32(0)), txl.Then():
        ...
        with umma:
            umma_group_idx = txl.Cast("uint32", txl.warp_id_in_role())
            ...
            with txl.If(umma_group_idx == txl.uint32(1)), txl.Then():
            ...
            kv_state = txl.RingState(num_kv_stages)
            umma_state = txl.RingState(num_umma_stages)
            q_stage = txl.local_scalar("uint32", init=txl.uint32(0))
            pump(state)
            with txl.While(state["fetched"][0] != txl.uint32(0)):
                with txl.If(state["q_atom"][0] != state["next_q"][0]), txl.Then():
                    txl.assign(q_stage, q_state.stage)
                ...
                empty_umma_barriers.wait(
                    umma_group_idx * txl.uint32(num_umma_stages) + umma_stage,
                    umma_phase ^ txl.uint32(1),
                )
                ...
                for k_phase in range(head_dim // umma_k):
                    ...
                    with txl.If(txl.cuda.elect_sync() != txl.uint32(0)), txl.Then():
                        txl.ptx[MMA](
                            txl.uint32(tmem_col)
                            + umma_group_idx * txl.uint32(umma_n * num_umma_stages)
                            + umma_stage * txl.uint32(umma_n),
                            desc_a,
                            desc_b,
                            runtime_instr_desc_hi,
                            txl.uint32(0),
                            ...
                            txl.uint32(0),
                            txl.uint32(0),
                            txl.ptx.pred(txl.uint32(k_phase)),
                        )
                with txl.If(txl.cuda.elect_sync() != txl.uint32(0)), txl.Then():
                    full_umma_barriers.arrive(
                        umma_group_idx * txl.uint32(num_umma_stages) + umma_stage, cta_group=1
                    )
                kv_state.advance()
                umma_state.advance()
                pump(state)
        ...
        with math:
            ...
            is_paired_atom = txl.local_scalar("uint32", init=txl.uint32(0))
            state = scheduler_state()
            q_state = txl.RingState(num_q_stages)
            kv_state = txl.RingState(num_kv_stages)
            umma_state = txl.RingState(num_umma_stages)
            q_stage = txl.local_scalar("uint32", init=txl.uint32(0))
            has_q_stage = txl.local_scalar("uint32", init=txl.uint32(0))
            pump(state)
            ...
            def reduce_and_store(num_iters_c, kv_offset, scale_kv, umma_stage_idx_arg):
                ...
                scale_kv_half = txl.local_scalar("float32")
                txl.ptx.mul.rn.f32(scale_kv_half, scale_kv, txl.float32(0.5))
                for q_inner_i in range(num_iters_c):
                    tmem_addr = (
                        tmem_start_base
                        + umma_stage_idx_arg * txl.uint32(umma_n)
                        + txl.uint32(q_inner_i * num_heads)
                    )
                    txl.ptx[TC_LD](*[accum[h] for h in range(num_heads)], tmem_addr)
                    ...
                    if q_inner_i == num_iters_c - 1:
                        ...
                        with txl.If(lane_idx == 0), txl.Then():
                            empty_umma_barriers.arrive(
                                math_wg_u32 * txl.uint32(num_umma_stages) + umma_stage_idx_arg
                            )
                    sum_0 = txl.cuda.make_float2(txl.float32(0), txl.float32(0))
                    sum_1 = txl.cuda.make_float2(txl.float32(0), txl.float32(0))
                    for head_j_group in range(num_heads // 4):
                        head_j = head_j_group * 4
                        sum_0 = relu2_fma_f32x2(
                            txl.cuda.make_float2(accum[head_j], accum[head_j + 1]),
                        ...
                        sum_1 = relu2_fma_f32x2(
                            txl.cuda.make_float2(accum[head_j + 2], accum[head_j + 3]),
                            txl.cuda.make_float2(
                    ...
                    txl.ptx.add.rn.f32x2(sum_v, sum_0, sum_1)
                    ...
                    txl.ptx.mul.rn.f32(result_f32, scale_kv_half, _add)
                    ...
                    if config.logits_dtype == "float32":
                        txl.ptx.st.global_.f32(logits_flat.ptr_to([logits_offset]), result)
                    else:
                        txl.ptx.st.global_.b16(logits_flat.ptr_to([logits_offset]), result)

            with txl.While(state["fetched"][0] != txl.uint32(0)):
                with txl.If(state["q_atom"][0] != state["next_q"][0]), txl.Then():
                    with txl.If(has_q_stage != txl.uint32(0)), txl.Then():
                        with txl.If(lane_idx == 0), txl.Then():
                            empty_q_barriers.arrive(q_stage)
                    txl.assign(q_stage, q_state.stage)
                    full_q_barriers.wait(q_stage, q_state.phase)
                    q_state.advance()
                    txl.assign(has_q_stage, txl.uint32(1))
                    ...
                    if config.varlen:
                        load_atom_advance(state["next_q"][0], batch_size)
                        txl.assign(
                            is_paired_atom, txl.Cast("uint32", atom_advance_result == txl.uint32(2))
                        )
                txl.assign(state["q_atom"][0], state["next_q"][0])
                txl.assign(state["kv_idx"][0], state["next_kv"][0])
                kv_offset = txl.local_scalar(
                    "uint64",
                    init=txl.Cast("uint64", atom_to_token_idx_expr(state["q_atom"][0]))
                    * txl.Cast("uint64", logits_stride)
                    + txl.Cast("uint64", (state["kv_idx"][0] + math_wg_u32) * txl.uint32(umma_m)),
                )
                kv_stage = kv_state.stage
                kv_phase = kv_state.phase
                full_kv_barriers.wait(math_wg_u32 * txl.uint32(num_kv_stages) + kv_stage, kv_phase)
                scale_kv = txl.local_scalar("float32")
                ...
                full_umma_barriers.wait(
                    math_wg_u32 * txl.uint32(num_umma_stages) + umma_stage, umma_phase
                )
                txl.ptx.tcgen05.fence__after_thread_sync()
                with txl.If(lane_idx == 0), txl.Then():
                    empty_kv_barriers.arrive(math_wg_u32 * txl.uint32(num_kv_stages) + kv_stage)
                if config.varlen:
                    with txl.If(is_paired_atom != txl.uint32(0)):
                        with txl.Then():
...
def _load_sglang_cutedsl_reference() -> tuple[Any, Any]:
    ...
    """
    try:
        from tirx_kernels.deepgemm._shared._sglang_cutedsl.cutedsl_paged_mqa_logits import (
