...
from .cutedsl_fp8_paged_mqa_logits import FP8MQALogitsKernel

logger = logging.getLogger(__name__)


def is_sm100_supported(device: torch.device | None = None) -> bool:
    """Local stand-in for ``sglang.srt.utils.is_sm100_supported`` (SM 10.x GPU)."""
...
_EPI_KV_UMMA_SUB = dict(max_kv_pipeline=True, max_umma_pipeline=True, smem_subpartition_opt=True)
_EPI_NOWAIT = dict(_EPI_KV_UMMA_SUB, remove_kv_wait_in_epilogue=True)


def _propose_epi_config(
    num_heads: int, next_n_k: int, batch_split: int, max_ctx: int, num_sms: int
) -> tuple[int, dict]:
...
class CuteDSLPagedMQALogitsRunner:
    ...
    def _compile(
        ...
        kv_fake = cute.runtime.make_fake_compact_tensor(
            cutlass.Uint8, (sym_num_phys_blocks, block_bytes), stride_order=(1, 0)
        )
        ...
        w_dtype = cutlass.Float16 if epi_dtype == torch.float16 else to_cutlass[epi_dtype]
        w_fake = cute.runtime.make_fake_compact_tensor(w_dtype, (N, sym_B), stride_order=(0, 1))

        logits_fake = cute.runtime.make_fake_tensor(
            to_cutlass[output_dtype], (cute.sym_int(), max_ctx), stride=(cute.sym_int64(), 1)
        )
    ...
    def forward(
        ...
        auto_nst, pipeline_flags = _propose_epi_config(H, next_n, B, max_context_len, num_sms)
...
# The upstream module also registers ``torch.ops.sglang.cute_dsl_fp8_paged_mqa_logits``
# (a ``torch.library.custom_op`` wrapper around ``CuteDSLPagedMQALogitsRunner.forward``
# plus its fake); it is omitted here so this copy never claims the ``sglang`` op
# namespace. The benchmark reference calls the runner class directly.
