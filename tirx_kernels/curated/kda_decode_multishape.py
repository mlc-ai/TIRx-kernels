...
optimization run. It covers all thirty official rows: tokens per sequence T = 1..6,
...
def _make_base():
    ...
    def build_kernel(*, T, H, HV, gate_lb, cpt, num_units, num_main, per_sm, copy_est=0, allreduce=None, bar_arrive=False, warp_auto=True, vec_split=True, nw=4, l2pf=False):
        ...
        def kda_decode_persist(
            ...
            state_in: txl.gptr[txl.bf16],
            state_out: txl.gptr[txl.bf16],
            output: txl.gptr[txl.bf16],
            ssm_idx: txl.gptr[txl.i32],
            num_accepted: txl.gptr[txl.i32],
            scale: txl.f32,
            lower_bound: txl.f32,
            num_slots: txl.i32,
            num_seqs: txl.i32,
            ...
            lane = txl.lane_id()
            smem = txl.smem_pool()
            tiles = smem.alloc((3 * tile_elems,), txl.bf16, align=1024)
            raw = smem.alloc((raw_elems,), txl.bf16, align=128)
            ftab = smem.alloc((nphase * FST,), txl.f32, align=16)
            vtab = smem.alloc((T * D,), txl.f32, align=16)
            svec = smem.alloc((2 * T + 2,), txl.f32, align=16)
            cpslots = smem.alloc((MAXC + 4,), txl.i32, align=16)
            bar_state = txl.MBarrier(smem, 1)
            bar_state.init(1)
            bar_vec = txl.MBarrier(smem, 1)
            ...
            def main_role():
                ks = lane & 7
                cw = lane >> 3
                nmain_me = txl.max((num_units - cta + num_main - 1) // num_main, txl.int32(0))
                ...
                slot0_next = txl.local_scalar("int32", init=txl.int32(0))
                acc_next = txl.local_scalar("int32", init=txl.int32(1))
                acc_next2 = txl.local_scalar("int32", init=txl.int32(1))
                slot0_next2 = txl.local_scalar("int32", init=txl.int32(0))
                ...
                def start_slot(uu, acc_val):
                    ...
                    if not spec:
                        return n
                    acc_c = txl.max(txl.min(acc_val, txl.int32(T)), txl.int32(1))
                    s = txl.local_scalar("int32")
                    txl.ptx.ld.global_.s32(s, ssm_idx.ptr_to([n * T + acc_c - 1]))
                    return s

                def load_acc(uu):
                    _, _, _, n, _ = unit_coords(uu)
                    txl.ptx.ld.global_.s32(acc_next, num_accepted.ptr_to([n]))
                ...
                if spec:
                    ...
                    idx_pre = txl.alloc_local((IDX_PRE,), "int32")
                    for i in range(IDX_PRE):
                        txl.assign(idx_pre[i], txl.int32(-1))
                ...
                with txl.If((tid == 0) & (cta < num_units)), txl.Then():
                    if spec:
                        ...
                        cands = txl.alloc_local((T,), "int32")
                        for t in range(T):
                            txl.ptx.ld.global_.s32(cands[t], ssm_idx.ptr_to([n0 * T + t]))
                        acc_c = txl.max(txl.min(a0, txl.int32(T)), txl.int32(1))
                        sel = cands[T - 1]
                        for t in range(T - 2, -1, -1):
                            sel = txl.if_then_else(acc_c == t + 1, cands[t], sel)
                        issue_state(cta, sel)
                        with txl.If(cta + num_main < num_units), txl.Then():
                            load_acc(cta + num_main)
                        if l2pf:
                            with txl.If(cta + 2 * num_main < num_units), txl.Then():
                ...
                def build_flags(cb):
                    ...
                    nz = txl.min(txl.int32(pmax), num_slots - cb)
                ...
                def scan_untouched(cb, cnt, want_rank, dst_idx):
                    ...
                    pend = txl.min(pmax, num_slots - cb)
                    with txl.serial(0, (pend + 31) // 32, unroll=False) as b:
                        pl = b * 32 + lane
                        valid = pl < pend
                        flag = txl.local_scalar("uint32", init=txl.uint32(1))
                ...
                def preprocess(t, hv, h):
                    ...
                    rb = t * (RAW_TOK_B // 2)
                    qw = txl.alloc_local((2,), "uint32", align=8)
                    kw = txl.alloc_local((2,), "uint32", align=8)
                    gw = txl.alloc_local((2,), "uint32", align=8)
                    vw = txl.alloc_local((2,), "uint32", align=8)
                    txl.ptx.ld.shared.v2.b32(qw[0], qw[1], raw.ptr_to([rb + lane * 4]))
                    txl.ptx.ld.shared.v2.b32(kw[0], kw[1], raw.ptr_to([rb + 128 + lane * 4]))
                    txl.ptx.ld.shared.v2.b32(gw[0], gw[1], raw.ptr_to([rb + 256 + lane * 4]))
                    txl.ptx.ld.shared.v2.b32(vw[0], vw[1], raw.ptr_to([rb + 384 + lane * 4]))
                    bbits = txl.local_scalar("uint16")
                    txl.ptx.ld.shared.b16(bbits, raw.ptr_to([rb + 512 + (hv % 8)]))
                    qf = txl.alloc_local((4,), "float32")
                    kf = txl.alloc_local((4,), "float32")
                    gf = txl.alloc_local((4,), "float32")
                    ...
                    cpart = txl.local_scalar("float32", init=txl.float32(0.0))
                    for e in range(4):
                        txl.ptx["fma.rn.f32"](sq, qf[e], qf[e], sq)
                        txl.ptx["fma.rn.f32"](sk, kf[e], kf[e], sk)
                        txl.ptx["fma.rn.f32"](cpart, qf[e], kf[e], cpart)
                    for x in (16, 8, 4, 2, 1):
                        txl.ptx["add.f32"](sq, sq, _shfl_bfly(sq, x))
                        txl.ptx["add.f32"](sk, sk, _shfl_bfly(sk, x))
                        txl.ptx["add.f32"](cpart, cpart, _shfl_bfly(cpart, x))
                    rq = txl.local_scalar("float32")
                    rk = txl.local_scalar("float32")
                    tmp = txl.local_scalar("float32")
                    txl.ptx["add.f32"](tmp, sq, txl.float32(1e-6))
                    txl.ptx["rsqrt.approx.ftz.f32"](rq, tmp)
                    txl.ptx["mul.f32"](rq, rq, scale)
                    txl.ptx["add.f32"](tmp, sk, txl.float32(1e-6))
                    txl.ptx["rsqrt.approx.ftz.f32"](rk, tmp)

                    txl.ptx["mul.f32"](cpart, cpart, rq)
                    txl.ptx["mul.f32"](cpart, cpart, rk)
                    for e in range(4):
                        txl.ptx["mul.f32"](qf[e], qf[e], rq)
                        txl.ptx["mul.f32"](kf[e], kf[e], rk)
                    if gate_lb:
                        gb = T * (RAW_TOK_B // 2)
                        ...
                        for e in range(4):
                            ...
                            txl.ptx["mul.f32"](x, x, ea)
                            txl.ptx["mul.f32"](x, x, txl.float32(-LOG2E))
                            txl.ptx["ex2.approx.ftz.f32"](x, x)
                            txl.ptx["add.f32"](x, x, txl.float32(1.0))
                            txl.ptx["rcp.approx.ftz.f32"](x, x)
                            txl.ptx["mul.f32"](x, x, lower_bound)
                            txl.ptx["mul.f32"](x, x, txl.float32(LOG2E))
                            txl.ptx["ex2.approx.ftz.f32"](gf[e], x)
                    else:
                        for e in range(4):
                            txl.ptx["mul.f32"](gf[e], gf[e], txl.float32(LOG2E))
                            txl.ptx["ex2.approx.ftz.f32"](gf[e], gf[e])
                    for pp in range(2):
                        pair = lane * 2 + pp
                ...
                d2 = txl.alloc_local((J,), "uint64", align=8)
                ub = txl.alloc_local((J,), "float32")
                zero2 = _f2(txl.float32(0.0), txl.float32(0.0))
                ...
                def reduce_and_finish(t, hv, n, row0):
                    ...
                    bc = txl.alloc_local((2,), "uint32", align=8)
                    txl.ptx.ld.shared.v2.b32(bc[0], bc[1], svec.ptr_to([2 * t]))
                    beta_t = txl.reinterpret("float32", bc[0])
                    c_t = txl.reinterpret("float32", bc[1])
                    vals = []
                    for j in range(J):
                    ...
                    if allreduce:
                        for s in (1, 2, 4):
                            for i in range(M):
                                txl.ptx["add.f32"](vals[i], vals[i], _shfl_bfly(vals[i], s))
                        tok = n * T + t
                        out_base = (txl.cast(tok, "int64") * HV + hv) * D + row0
                        for j in range(J):
                            col = warp * rows_per_warp + j * 4 + cw
                            vv = txl.local_scalar("uint32")
                            txl.ptx.ld.shared.b32(vv, vtab.ptr_to([t * D + row0 + col]))
                            diff = txl.local_scalar("float32")
                            txl.ptx["sub.f32"](diff, txl.reinterpret("float32", vv), vals[j])
                            txl.ptx["mul.f32"](ub[j], diff, beta_t)
                            o = txl.local_scalar("float32")
                    ...
                    for s in (4, 2, 1):
                        if len(vals) > 1:
                            ...
                            txl.ptx["add.f32"](vals[0], vals[0], _shfl_bfly(vals[0], s))
                    assert len(vals) == R
                    ...
                    tok = n * T + t
                    out_base = (txl.cast(tok, "int64") * HV + hv) * D + row0
                    uloc = txl.alloc_local((R,), "float32")
                    for i in range(R):
                        other = txl.local_scalar("float32", init=_shfl_bfly(vals[i], 4))
                        dd1 = txl.if_then_else(is_d2, other, vals[i])
                        dd2 = txl.if_then_else(is_d2, vals[i], other)
                        col = warp * rows_per_warp + (jbase + i) * 4 + cw
                        vv = txl.local_scalar("uint32")
                        txl.ptx.ld.shared.b32(vv, vtab.ptr_to([t * D + row0 + col]))
                        diff = txl.local_scalar("float32")
                        txl.ptx["sub.f32"](diff, txl.reinterpret("float32", vv), dd1)
                        txl.ptx["mul.f32"](uloc[i], diff, beta_t)
                        o = txl.local_scalar("float32")
                ...
                def update_pass(phase, tile, with_dots):
                    if with_dots:
                        for j in range(J):
                            ...
                            txl.assign(d2[j], zero2)
                    u2 = txl.alloc_local((J,), "uint64", align=8)
                    for j in range(J):
                        txl.assign(u2[j], _f2(ub[j], ub[j]))
                    for c in range(2):
                        kp2 = txl.alloc_local((4,), "uint64", align=8)
                        a2 = txl.alloc_local((4,), "uint64", align=8)
                        k2 = txl.alloc_local((4,), "uint64", align=8)
                        q2 = txl.alloc_local((4,), "uint64", align=8)
                        for ii in range(4):
                            i = c * 4 + ii
                            if with_dots:
                        ...
                        for j in range(J):
                            packed = txl.alloc_local((4,), "uint32", align=16)
                            for ii in range(4):
                                i = c * 4 + ii
                                ...
                                if with_dots:
                                    ...
                                    txl.ptx.fma.rn.f32x2(d2[j], q2[ii], S[j * 8 + i], d2[j])
                ...
                def store_barrier():
                    ...
                    if bar_arrive:
                        with txl.If(warp == 0):
                            with txl.Then():
                                txl.ptx.bar.sync(txl.uint32(1), txl.uint32(NT))
                            with txl.Else():
                                txl.ptx.bar.arrive(txl.uint32(1), txl.uint32(NT))
                    else:
                        txl.cuda.cta_sync()

                def store_checkpoint(tile, dst_slot, hv, row0):
                    dst0 = (txl.cast(dst_slot, "int64") * HV + hv) * (D * D) + row0 * D
                    for pc in range(n_pieces):
                        txl.ptx[_BULK_S2G](
                ...
                def store_warp_rows(tile, dst_slot, hv, row0):
                    ...
                    dst0 = (txl.cast(dst_slot, "int64") * HV + hv) * (D * D) + wrow * D
                    txl.ptx[_BULK_S2G](
                        state_out.ptr_to([dst0]),
                        tiles.ptr_to([tile * tile_elems + warp * rows_per_warp * D]),
                        txl.uint32(rows_per_warp * D * 2),
                    )
                    txl.ptx.cp.async_.bulk.commit_group()
                ...
                with txl.serial(0, nmain_me, unroll=False) as sm:
                    ...
                    txl.assign(vcount, vcount + txl.int32(1))
                    _rng_end(tk)
                    tk = _rng("preprocess")
                    with txl.If(lane == 0 if warp_auto else tid == 0), txl.Then():
                        if spec:
                            txl.ptx.ld.global_.s32(nxt_slot, ssm_idx.ptr_to([n * T]))
                        else:
                            txl.assign(nxt_slot, n)
                    with txl.If(tid == 0), txl.Then():
                        if spec:
                            with txl.If(has_next), txl.Then():
                                txl.assign(slot0_next, start_slot(u + num_main, acc_next))
                            if l2pf:
                                with txl.If(u + 2 * num_main < num_units), txl.Then():
                                    txl.assign(slot0_next2, start_slot(u + 2 * num_main, acc_next2))
                        ...
                        with txl.If(has_next):
                            with txl.Then():
                                if spec:
                                    ...
                                    if l2pf:
                                        with txl.If(u + 3 * num_main < num_units), txl.Then():
                    ...
                    for j in range(J):
                        ...
                        txl.assign(d2[j], zero2)
                    for i in range(8):
                        a2 = txl.local_scalar("uint64")
                        k2 = txl.local_scalar("uint64")
                        q2 = txl.local_scalar("uint64")
                        txl.ptx.ld.shared.b64(a2, txl.ptx.addr(vec_ptr(0, i), 8))
                        txl.ptx.ld.shared.v2.b64(k2, q2, txl.ptx.addr(vec_ptr(0, i), 16))
                        for j in range(J):
                            txl.ptx.mul.rn.f32x2(S[j * 8 + i], a2, S[j * 8 + i])
                ...
                if spec:
                    with txl.If(ncopy_me > 0), txl.Then():
                        ...
                        txl.cuda.cta_sync()
                        nfast = txl.min(ncopy_me, txl.int32(MAXC))
                        with txl.serial(0, (nfast + 2) // 3, unroll=False) as kb:
                            tk = _rng("copy-batch")
                            k0 = kb * 3
                        ...
                        with txl.If(ncopy_me > MAXC), txl.Then():
                            txl.cuda.cta_sync()
...
def _make_defer():
    ...
    after the final hand-off: tirx-lite scalar expressions are lazy, so advancing it first made the
    ...
    def build_kernel(*, T, H, HV, gate_lb, cpt, num_units, num_main, per_sm, copy_est=0, allreduce=None, vec_split=True, nw=4, ntiles=None, warp_auto=True, bar_arrive=False, l2pf=False):
        ...
        def kda_decode_persist(
            ...
            lane = txl.lane_id()
            smem = txl.smem_pool()
            tiles = smem.alloc((ntiles * tile_elems,), txl.bf16, align=1024)
            raw = smem.alloc((raw_elems,), txl.bf16, align=128)
            ftab = smem.alloc((nphase * FST,), txl.f32, align=16)
            vtab = smem.alloc((T * D,), txl.f32, align=16)
            svec = smem.alloc((2 * T + 2,), txl.f32, align=16)
            cpslots = smem.alloc((MAXC + 4,), txl.i32, align=16)
            flags = smem.alloc((FLAG_WORDS,), txl.u32, align=16)
            bar_state = txl.MBarrier(smem, 1)
            bar_state.init(1)
            bar_vec = txl.MBarrier(smem, 1)
            bar_vec.init(nw if vec_split else 1)
            ...
            def main_role():
                ...
                def build_flags(cb):
                    ...
                    with txl.If(tid < FLAG_WORDS), txl.Then():
                        txl.ptx.st.shared.b32(flags.ptr_to([tid]), txl.uint32(0))
                    txl.cuda.cta_sync()
                    total = num_seqs * T
                    with txl.serial(0, (total + NT - 1) // NT, unroll=False) as i:
                        e = i * NT + tid
                        with txl.If(e < total), txl.Then():
                            sidx = txl.local_scalar("int32")
                            txl.ptx.ld.global_.s32(sidx, ssm_idx.ptr_to([e]))
                ...
                if spec:
                    ...
                    for i in range(IDX_PRE):
                        ...
                        with txl.If(i * NT + tid < total_idx), txl.Then():
                            txl.ptx.ld.global_.s32(idx_pre[i], ssm_idx.ptr_to([i * NT + tid]))
                    a0 = txl.local_scalar("int32", init=txl.int32(1))
                    cands = txl.alloc_local((T,), "int32")
                    for t in range(T):
                        txl.assign(cands[t], txl.int32(0))
                    with txl.If((tid == 0) & (cta < num_units)), txl.Then():
                        n0 = unit_coords(cta)[3]
                        txl.ptx.ld.global_.s32(a0, num_accepted.ptr_to([n0]))
                        for t in range(T):
                ...
                if spec:
                    ...
                    with txl.serial(0, (num_slots + pmax - 1) // pmax, unroll=False) as chunk:
                        ...
                        with txl.If(tid < FLAG_WORDS), txl.Then():
                            txl.ptx.st.shared.b32(flags.ptr_to([tid]), txl.uint32(0))
                        txl.cuda.cta_sync()
                        with txl.If((chunk == 0) & (tid == 0) & (cta < num_units)), txl.Then():
                            issue_first_state()
                        total = num_seqs * T
                        for i in range(IDX_PRE):
                            rel = idx_pre[i] - cb
                ...
                def reduce_and_finish(t, hv, n, row0):
                    ...
                    if allreduce:
                        ...
                        for j in range(J):
                            ...
                            diff = txl.local_scalar("float32")
                            txl.ptx["sub.f32"](diff, txl.reinterpret("float32", vv), vals[j])
                            uj = txl.local_scalar("float32")
                            txl.ptx["mul.f32"](uj, diff, beta_t)
                ...
                with txl.serial(0, nmain_me, unroll=False) as sm:
                    ...
                    if spec:
                        txl.ptx.ld.global_.s32(nxt_slot, ssm_idx.ptr_to([n * T]))
                        with txl.If((tid == 0) & has_next), txl.Then():
                            txl.assign(slot0_next, start_slot(u + num_main, acc_next))
                        if l2pf:
                            with txl.If((tid == 0) & (u + 2 * num_main < num_units)), txl.Then():
                                txl.assign(slot0_next2, start_slot(u + 2 * num_main, acc_next2))
                                prefetch_l2(u + 2 * num_main, slot0_next2)
                    else:
                        txl.assign(nxt_slot, n)
                    ...
                    issuer = lane == 0 if warp_auto else tid == 0
                    st_slot = txl.local_scalar("int32")
                ...
                if spec:
                    with txl.If(ncopy_me > 0), txl.Then():
                        ...
                        txl.cuda.cta_sync()
                        nfast = txl.min(ncopy_me, txl.int32(MAXC))
                        with txl.serial(0, (nfast + ntiles - 1) // ntiles, unroll=False) as kb:
                            tk = _rng("copy-batch")
                            k0 = kb * ntiles
...
def _make_dyn():
    ...
    def build_kernel(*, T, H, HV, gate_lb, cpt, num_units, num_main, per_sm, copy_est=0, allreduce=None, bar_arrive=False, warp_auto=True, vec_split=True, nw=4, l2pf=False):
        ...
        def kda_decode_persist(
            ...
            vtab = smem.alloc((T * D,), txl.f32, align=16)
            svec = smem.alloc((2 * T + 2,), txl.f32, align=16)
            cpslots = smem.alloc((MAXR + 4,), txl.i32, align=16)
            mailbox = smem.alloc((4,), txl.i32, align=16)
            bar_state = txl.MBarrier(smem, 1)
            bar_state.init(1)
            bar_vec = txl.MBarrier(smem, 1)
            bar_vec.init(nw if vec_split else 1)
            ...
            def main_role():
                ...
                tk1 = txl.local_scalar("int32", init=txl.int32(0))
                tk2 = txl.local_scalar("int32", init=txl.int32(0))
                if static_tickets:
                    txl.assign(tk1, cta)
                    txl.assign(tk2, cta + num_main)
                else:
                    with txl.If(tid == 0), txl.Then():
                        txl.ptx.atom.global_.add.s32(tk1, sched.ptr_to([0]), txl.int32(1))
                        txl.ptx.atom.global_.add.s32(tk2, sched.ptr_to([0]), txl.int32(1))
                ...
                if spec:
                    ...
                    txl.cuda.cta_sync()
                    ucnt = txl.local_scalar("int32")
                    txl.ptx.ld.shared.b32(ucnt, cpslots.ptr_to([MAXR + 1]))
                    txl.assign(total_copy, txl.uniform(ucnt) * DCU)
                else:
                    with txl.If(tid == 0), txl.Then():
                        txl.ptx.st.shared.b32(mailbox.ptr_to([0]), txl.reinterpret("uint32", tk1))
                        txl.ptx.st.shared.b32(mailbox.ptr_to([1]), txl.reinterpret("uint32", tk2))
                ...
                nxt2 = txl.local_scalar("int32")
                txl.ptx.ld.shared.b32(nxt, mailbox.ptr_to([0]))
                txl.ptx.ld.shared.b32(nxt2, mailbox.ptr_to([1]))
                txl.assign(nxt, txl.uniform(nxt) + num_main)
                txl.assign(nxt2, txl.uniform(nxt2) + num_main)
                cur = txl.local_scalar("int32", init=txl.uniform(cta))
                with txl.If(tid == 0), txl.Then():
                    if spec:
                        with txl.If(nxt < num_units), txl.Then():
                            load_acc(nxt)
                        with txl.If((cta >= num_units) & (cta < total_items)), txl.Then():
                ...
                with txl.While(cur < total_items):
                    with txl.If(cur < num_units):
                        with txl.Then():
                            u = cur
                            head, part, hv, n, h = unit_coords(u)
                            row0 = part * cpt
                            ...
                            tk3 = txl.local_scalar("int32", init=txl.int32(0))
                            if static_tickets:
                                txl.assign(tk3, nxt2)
                            else:
                                with txl.If(tid == 0), txl.Then():
                            ...
                            with txl.If(tid == 0), txl.Then():
                                if spec:
                                    with txl.If(has_next), txl.Then():
                                        txl.assign(slot0_next, start_slot(nxt, acc_next))
                            for rnd in range((T + nw - 1) // nw):
                                t = rnd * nw + warp
                                if rnd * nw + nw <= T:
                                    preprocess(t, hv, h)
                    ...
                    txl.assign(cur, nxt)
                    txl.assign(nxt, nxt2)
                    txl.ptx.ld.shared.b32(nxt2, mailbox.ptr_to([2]))
                    txl.assign(nxt2, txl.uniform(nxt2) + num_main)
                    if spec:
                        txl.assign(acc_next, acc_next2)
                tk = _rng("drain")
                with txl.If(lane == 0), txl.Then():
                    txl.ptx.cp.async_.bulk.wait_group.read(0)
                _rng_end(tk)
                ...
                with txl.If(tid == 0), txl.Then():
                    ...
                    with txl.If(done == txl.int32(num_main - 1)), txl.Then():
                        txl.ptx.st.global_.s32(sched.ptr_to([0]), txl.int32(0))
                        txl.ptx.st.global_.s32(sched.ptr_to([1]), txl.int32(0))
            ...
            def main_role():
                ...
                with txl.If((tid == 0) & first_is_item), txl.Then():
                    if spec:
                        ...
                        if k_full > 1:
                            ...
                            with txl.If(nhalf_me > 0), txl.Then():
                                load_acc(F_units + cta // 2)
                        else:
                            issue_state_half(cta, sel)
                            with txl.If(nhalf_me > 1), txl.Then():
                ...
                def run_unit(G, uu, row0, hv, n, h, is_half, nxt_kind, nxt_full_id, nxt_half_q, nxt2_kind, nxt2_full_id, nxt2_half_q):
                    ...
                    with txl.If(tid == 0), txl.Then():
                        if spec:
                            with txl.If(has_next), txl.Then():
                                txl.assign(slot0_next, start_slot(nxt_head, acc_next))
                    for rnd in range((T + nw - 1) // nw):
                        t = rnd * nw + warp
                        if rnd * nw + nw <= T:
                            preprocess(t, hv, h)
                    ...
                    with txl.If(tid == 0), txl.Then():
                        with txl.If(has_next):
                            with txl.Then():
                                ...
                                if spec:
                                    with txl.If(nxt2_kind != 0), txl.Then():
                                        load_acc(nxt2_head)
                            if spec:
                                with txl.Else():
                                    with txl.If(ncopy_me > 0), txl.Then():
                    ...
                    if not is_half:
                        with txl.If(nxt_kind == 2), txl.Then():
                            with txl.If(issuer), txl.Then():
                                txl.ptx.cp.async_.bulk.wait_group.read(0)
                                txl.ptx.fence.proxy.async_.shared__cta()
                            txl.cuda.cta_sync()
...
def _make_clc():
    ...
    def build_kernel(*, T, H, HV, gate_lb, cpt, num_units, num_main, per_sm, copy_est=0, allreduce=None, bar_arrive=False, warp_auto=True, vec_split=True, nw=4, l2pf=False):
        ...
        def kda_decode_persist(
            ...
            smem = txl.smem_pool()
            tiles = smem.alloc((3 * tile_elems,), txl.bf16, align=1024)
            raw = smem.alloc((2 * raw_elems,), txl.bf16, align=128)
            ftab = smem.alloc((nphase * FST,), txl.f32, align=16)
            vtab = smem.alloc((T * D,), txl.f32, align=16)
            svec = smem.alloc((2 * T + 2,), txl.f32, align=16)
            cpslots = smem.alloc((MAXC + 4,), txl.i32, align=16)
            bar_state = txl.MBarrier(smem, 3)
            bar_state.init(1)
            bar_vec = txl.MBarrier(smem, 2)
            bar_vec.init(nw if vec_split else 1)
            clc_handle = smem.pool.alloc((4,), txl.u32, align=16)
            clc_mailbox = smem.pool.alloc((1,), txl.u32, align=4)
            bar_clc = txl.MBarrier(smem, 1)
            bar_clc.init(1)
            txl.ptx.fence.proxy.async_.shared__cta()
            txl.cuda.cta_sync()
            ...
            def main_role():
                ...
                acc_next2 = txl.local_scalar("int32", init=txl.int32(1))
                slot0_next2 = txl.local_scalar("int32", init=txl.int32(0))
                clc_phase = txl.local_scalar("int32", init=txl.int32(0))
                ...
                u = txl.local_scalar("int32", init=cta)
                with txl.While((u >= 0) & (u < num_units)):
                    head, part, hv, n, h = unit_coords(u)
                    row0 = part * cpt
                    cur_slot = txl.local_scalar("int32")
                    nxt_slot = txl.local_scalar("int32")
                    ...
                    txl.assign(scount, scount + txl.int32(1))
                    _rng_end(tk)
                    next_state_stage = txl.local_scalar("int32", init=scount % 3)
                    nxt_u = txl.local_scalar("int32", init=txl.int32(-1))
                    with txl.If(tid == 0), txl.Then():
                        bar_clc.wait(0, clc_phase)
                        txl.assign(clc_phase, clc_phase ^ txl.int32(1))
                        cancelled = txl.local_scalar("uint32")
                        txl.query_cancel_first_ctaid_x(cancelled, txl.address_of(clc_handle[0]))
                        txl.assign(nxt_u, txl.reinterpret("int32", cancelled))
                        txl.ptx.st.shared.b32(clc_mailbox.ptr_to([0]), cancelled)
                        with txl.If(cancelled != txl.uint32(CLC_SENTINEL)), txl.Then():
                            issue_vec(txl.reinterpret("int32", cancelled), vcount & 1)
                            txl.ptx.cp.async_.bulk.wait_group.read(0)
                            issue_state(
                                txl.reinterpret("int32", cancelled),
                    ...
                    txl.ptx.ld.shared.b32(next_bits, clc_mailbox.ptr_to([0]))
                    txl.assign(nxt_u, txl.reinterpret("int32", next_bits))
                    txl.assign(u, nxt_u)
