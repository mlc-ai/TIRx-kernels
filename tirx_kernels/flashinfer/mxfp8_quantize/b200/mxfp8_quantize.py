...
``tirx_kernels/flashinfer/_shared/fp_quant.py``.
"""
...
def _quantize_block(in_global, out_global, row_idx, sf_col, thread_in_unit, *, dtype, use_2t, k):
    ...
    v = ld_global_v4_u32(txl.address_of(in_global[in_off]))
    if use_2t:
        vh = ld_global_v4_u32(txl.address_of(in_global[in_off + 8]))
    ...
    scale_ue8m0_u32 = float_to_ue8m0(mul_f32(global_max, txl.float32(INV_FLOAT8_E4M3_MAX)))
    inv_scale = ue8m0_to_inv_scale(scale_ue8m0_u32)
    out_off = txl.cast(row_idx, "int64") * k + elem_idx
    st_global_u64(txl.address_of(out_global[out_off]), fp8x8_scaled(words[:4], inv_scale, dtype))
    if use_2t:
        st_global_u64(
            txl.address_of(out_global[out_off + 8]), fp8x8_scaled(words[4:], inv_scale, dtype)
        )
...
def get_kernel(
    ...
    if sf_layout == "linear":
        ...
        def mxfp8_quantize_linear(
            ...
            sf_idx_in_warp = txl.truncdiv(lane_idx, txl.int32(tps))
            thread_in_sf = txl.truncmod(lane_idx, txl.int32(tps))

            # Grid-stride loop over flat SF blocks (mxfp8_quantize.py:248-326).
            sf_idx = txl.local_scalar("int32", init=bx * sfbpt + warp_idx * sfbpw + sf_idx_in_warp)
            with txl.While(sf_idx < total_sf):
                ...
                txl.assign(sf_idx, sf_idx + grid_x * sfbpt)
    ...
    def mxfp8_quantize_swizzled(
        ...
        if needs_col_loop:
            ...
            col_unit_idx = txl.truncdiv(tx, txl.int32(tps))
            thread_in_unit = txl.truncmod(tx, txl.int32(tps))

            row_idx = txl.local_scalar("int32", init=bx)
            with txl.While(row_idx < padded_rows):
                with txl.If(row_idx >= m_rows):
                    with txl.Then():
                        ...
                        with txl.While(sc < nsb):
                            ...
                            with txl.If(thread_in_unit == 0), txl.Then():
                                st_global_u8(
                                    ...
                                    txl.cast(scale_ue8m0_u32, "uint8"),
                                )
            ...
            sf_col_idx = _materialize(txl.truncdiv(local_tidx, txl.int32(tps)))
            thread_in_unit = _materialize(txl.truncmod(local_tidx, txl.int32(tps)))
            ...
            with txl.While(row_batch_idx * rows_per_block < padded_rows):
                with txl.If(row_idx2 < padded_rows), txl.Then():
                    with txl.If(row_idx2 >= m_rows):
                        with txl.Then():
                            # Padding row: zero ALL padded SF columns; the stride is
                            # num_sf_blocks_per_row (:609-620).
                            with txl.If(thread_in_unit == 0), txl.Then():
                                pad_col = txl.local_scalar("int32", init=sf_col_idx)
                        ...
                        with txl.Else():
                            with txl.If(sf_col_idx < nsb), txl.Then():
                                scale_ue8m0_u32 = _quantize_block(
                                    in_global,
                                    out_global,
                                    row_idx2,
                                ...
                                with txl.If(thread_in_unit == 0), txl.Then():
                                    st_global_u8(
                                        txl.address_of(sf_out[sf_offset(row_idx2, sf_col_idx)]),
                                        txl.cast(scale_ue8m0_u32, "uint8"),
                                    )
                            # Padding SF columns of a data row (:711-723).
                            if pad_cols != nsb:
                                with txl.If(thread_in_unit == 0), txl.Then():
                                    pad_col2 = txl.local_scalar("int32", init=nsb + sf_col_idx)
                                    with txl.While(pad_col2 < pad_cols):
                                        st_global_u8(
                                            txl.address_of(sf_out[sf_offset(row_idx2, pad_col2)]),
                                            txl.uint8(0),
                                        )
                                        txl.assign(pad_col2, pad_col2 + nsb)
