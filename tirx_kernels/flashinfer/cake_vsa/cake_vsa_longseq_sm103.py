    ...
    def cake_vsa_longseq_sm103(
        ...
        *,
        ...
        with r_mma:
            ...
            k_stage = txl.local_scalar("uint32", init=_u32(0))
            k_phase = txl.local_scalar("int32", init=_i32(0))
            v_stage = txl.local_scalar("uint32", init=_u32(0))
            v_phase = txl.local_scalar("int32", init=_i32(0))
            ...
            for instance in range(2):
                with txl.If(selected_blocks > _i32(0)), txl.Then():
                    k_stage_now = txl.local_scalar("uint32", init=k_stage)
                    k_phase_now = txl.local_scalar("int32", init=k_phase)
            ...
            with txl.While(union_index_m < max_union_count_m):
                for instance in range(2):
                    with txl.If(selected_blocks > union_index_m), txl.Then():
                        v_stage_now = txl.local_scalar("uint32", init=v_stage)
                        v_phase_now = txl.local_scalar("int32", init=v_phase)
                        ...
                        with txl.If(union_index_m + _i32(1) == selected_blocks), txl.Then():
        ...
        with r_qk_load:
            ...
            with txl.While(block_base < nb):
                ...
                with txl.If(n_block < nb), txl.Then():
                    mask_byte = txl.local_scalar("uint16")
                    txl.ptx.ld.global_.nc.b8(mask_byte, block_mask.ptr_to([mask_base + n_block]))
                    txl.assign(selected, txl.cast(mask_byte != txl.uint16(0), "int32"))
                vote = txl.local_scalar("uint32")
                txl.ptx.vote_sync.ballot.b32(
                    vote, txl.ptx.pred(txl.cast(selected != _i32(0), "bool")), _u32(_FULL_MASK)
                )
            ...
            k_load_stage = txl.local_scalar("uint32", init=_u32(0))
            phase_k_empty = txl.local_scalar("int32", init=_i32(1))
            union_index_k = txl.local_scalar("int32", init=_i32(0))
            with txl.While(union_index_k < selected_blocks):
                for instance in range(2):
                    n_block_k = _ld_shared_i32(
                        bar(_SMEM_UNION_BLOCKS + MAX_SELECTED_BLOCKS * 4 * instance)
                        + txl.cast(union_index_k, "uint32") * _u32(4)
                    )
                ...
                txl.assign(union_index_k, union_index_k + _i32(1))
        ...
        with r_v_load:
            ...
            kv_head_v = q_head
            v_load_stage = txl.local_scalar("uint32", init=_u32(0))
            phase_v_empty = txl.local_scalar("int32", init=_i32(1))
            union_index_v = txl.local_scalar("int32", init=_i32(0))
            with txl.While(union_index_v < selected_blocks):
                for instance in range(2):
                    n_block_v = _ld_shared_i32(
                        bar(_SMEM_UNION_BLOCKS + MAX_SELECTED_BLOCKS * 4 * instance)
                        + txl.cast(union_index_v, "uint32") * _u32(4)
                    )
                    token_base_v = n_block_v * 128
                    _mbar_wait(bar(_MBAR_V_EMPTY) + v_load_stage * _u32(8), phase_v_empty)
                    push_leader_v = txl.local_scalar("uint32", init=txl.cuda.elect_sync())
                    with txl.If(push_leader_v != _u32(0)), txl.Then():
                ...
                txl.assign(union_index_v, union_index_v + _i32(1))
