    ...
    def cake_vsa_blk128_compact_sm100(
        ...
        *,
        ...
        with r_softmax:
            ...
            with txl.While(union_index < union_count):
                n_block = _ld_shared_i32(
                    ...
                    + txl.cast(instance * 64 + union_index, "uint32") * _u32(4)
                )
        ...
        with r_correction:
            ...
            max_union_count_c = txl.local_scalar("int32", init=count0_c)
            with txl.If(max_union_count_c < count1_c), txl.Then():
                txl.assign(max_union_count_c, count1_c)
            warp_in_role = warp - 8
        ...
        with r_mma:
            ...
            max_union_count_m = txl.local_scalar("int32", init=count0_m)
            with txl.If(max_union_count_m < count1_m), txl.Then():
                txl.assign(max_union_count_m, count1_m)
            ...
            def qk_chain(instance, k_stage):
                ...
                b_lo = txl.local_scalar(
                    "uint32",
                    init=txl.uniform(((bar(_SMEM_KV) >> 4) & _u32(0x3FFF)) + k_stage * _u32(2048)),
                )
                ...
                txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_KV_EMPTY) + k_stage * _u32(8), pred=leader)
            ...
            with txl.While(union_index_2 < max_union_count_m):
                ...
                for instance in range(2):
                    count = _ld_shared_i32(bar(_SMEM_UNION_COUNT + 4 * instance))
                    counts.append(count)
                    with txl.If(count > union_index_2), txl.Then():
                        k_stage = txl.local_scalar("uint32", init=kv_stage)
                        k_phase = txl.local_scalar("int32", init=kv_phase)
                        _advance_ring(kv_stage, kv_phase)
                        _mbar_wait(bar(_MBAR_KV_FULL) + k_stage * _u32(8), k_phase)
                        qk_chain(instance, k_stage)
                for instance in range(2):
                    with txl.If(counts[instance] > union_index_2), txl.Then():
                        v_stage = txl.local_scalar("uint32", init=kv_stage)
                        v_phase = txl.local_scalar("int32", init=kv_phase)
                        _advance_ring(kv_stage, kv_phase)
                        ...
                        with txl.If(union_index_2 + _i32(1) == counts[instance]), txl.Then():
                            txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_O_FULL + 8 * instance), pred=leader)
                        txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_KV_EMPTY) + v_stage * _u32(8), pred=leader)
        ...
        with r_load:
            ...
            with txl.While(block_base < nb):
                ...
                with txl.If(selected_1 != _i32(0)), txl.Then():
                    ...
                    txl.ptx.st.shared.b32(slot_addr + _u32(64 * 4), n_block_1)
            ...
            with txl.If(selected_count == _i32(0)), txl.Then():
                with txl.If(lane == 0), txl.Then():
                    ...
                    txl.ptx.st.shared.b32(bar(_SMEM_UNION_BLOCKS + 64 * 4), _i32(0))
            ...
            max_union_count_l = txl.local_scalar("int32", init=count0_l)
            with txl.If(max_union_count_l < count1_l), txl.Then():
                txl.assign(max_union_count_l, count1_l)
            phase_kv_empty = txl.local_scalar("int32", init=_i32(1))

            def push_tile(tensor_map, instance, union_index_3):
                count = _ld_shared_i32(bar(_SMEM_UNION_COUNT + 4 * instance))
                with txl.If(count > union_index_3), txl.Then():
                    n_block = _ld_shared_i32(
            ...
            with txl.While(union_index_3 < max_union_count_l):
                for instance in range(2):
