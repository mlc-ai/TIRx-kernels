...
def _advance_ring(stage, phase):
    ...
    with txl.If(stage == _u32(3)), txl.Then():
...
def _advance_v_ring(stage, phase):
    ...
    txl.assign(stage, stage + _u32(1))
    with txl.If(stage == _u32(2)), txl.Then():
        txl.assign(stage, _u32(0))
        _flip(phase)
...
def _block_sum(values):
    ...
    txl.ptx.add.ftz.f32(half, acc_x, acc_y)
    total = txl.local_scalar("float32")
    txl.ptx.add.ftz.f32(total, half, _shfl_xor_f32(half, 16))
    return total
...
def _build_kernel():
    ...
    def cake_vsa_longseq_sm100(
        ...
        block_mask: txl.gptr[txl.u8],
        mb: txl.i32,
        nb: txl.i32,
        selected_blocks: txl.i32,
        num_q_heads: txl.i32,
        ...
        *,
        ...
        with r_softmax:
            q_valid = txl.local_scalar("int32", init=_i32(128))
            with txl.If(q_tile >= mb), txl.Then():
            ...
            instance_token_offset = txl.uniform(instance * 64)
            ...
            with txl.If(instance_valid > _i32(64)), txl.Then():
                txl.assign(instance_valid, _i32(64))
            with txl.If(instance_valid < _i32(0)), txl.Then():
                txl.assign(instance_valid, _i32(0))
            row_valid = txl.local_scalar("int32", init=txl.cast(my_row < instance_valid, "int32"))
            ...
            with txl.While(union_index < union_count):
                n_block = _ld_shared_i32(
                    ...
                    + txl.cast(instance * 192 + union_index, "uint32") * _u32(4)
                )
                ...
                with txl.If(acc_scale_log2 >= _f32(-8.0)):
                    ...
                    with txl.Else():
                        ...
                        txl.ptx.mul.ftz.f32(tau_log2, acc_scale_log2, lse_temperature_scale)
                ...
                with txl.If(return_temperature_lse != _i32(0)):
                    with txl.Then():
                        ...
                        txl.ptx.mul.ftz.f32(tau_scale, softmax_scale_log2, lse_temperature_scale)
                        tau_bias = txl.local_scalar("float32")
                        txl.ptx.mul.ftz.f32(tau_bias, score_bias, lse_temperature_scale)
                ...
                txl.ptx.fma.rn.ftz.f32(row_sum, row_sum, acc_scale, block_sum)
        ...
        with r_correction:
            q_valid_c = txl.local_scalar("int32", init=_i32(128))
            with txl.If(q_tile >= mb), txl.Then():
                txl.assign(q_valid_c, _i32(0))
            ...
            for instance in range(2):
                ...
                instance_valid_c = txl.local_scalar("int32", init=q_valid_c - _i32(64 * instance))
                with txl.If(instance_valid_c > _i32(64)), txl.Then():
                    txl.assign(instance_valid_c, _i32(64))
                ...
                with txl.If(my_row_c < instance_valid_c), txl.Then():
                    ...
                    with txl.If(col_half_c == 0), txl.Then():
                        ...
                        with txl.If(return_softmax_lse != _i32(0)), txl.Then():
                            ...
                            txl.ptx.mul.ftz.f32(max_scaled, final_max, softmax_scale_log2)
                            log_sum = txl.local_scalar("float32")
                            txl.ptx.mul.ftz.f32(log_sum, log2_sum, _f32(_LN2_F32))
                        ...
                        with txl.If(return_temperature_lse != _i32(0)), txl.Then():
                            ...
                            with txl.If(final_temperature_sum > _f32(0.0)), txl.Then():
                                ...
                                txl.ptx.mul.ftz.f32(max_scaled_t, final_max, softmax_scale_log2)
                                max_ln = txl.local_scalar("float32")
                                txl.ptx.mul.ftz.f32(max_ln, max_scaled_t, _f32(_LN2_F32))
                                log_tsum = txl.local_scalar("float32")
                                txl.ptx.mul.ftz.f32(log_tsum, log2_tsum, _f32(_LN2_F32))
                                txl.ptx.fma.rn.ftz.f32(
        ...
        with r_mma:
            ...
            k_stage_cursor = txl.local_scalar("uint32", init=_u32(0))
            k_phase_cursor = txl.local_scalar("int32", init=_i32(0))
            v_stage_cursor = txl.local_scalar("uint32", init=_u32(0))
            v_phase_cursor = txl.local_scalar("int32", init=_i32(0))
            ...
            def qk_chain(instance, k_stage):
                ...
                b_lo = txl.local_scalar(
                    "uint32",
                    init=txl.uniform(((bar(_SMEM_K) >> 4) & _u32(0x3FFF)) + k_stage * _u32(2048)),
                )
                ...
                txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_S_FULL + 8 * instance), pred=leader)
                txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_K_EMPTY) + k_stage * _u32(8), pred=leader)
            ...
            for instance in range(2):
                with txl.If(selected_blocks > _i32(0)), txl.Then():
                    k_stage = txl.local_scalar("uint32", init=k_stage_cursor)
                    k_phase = txl.local_scalar("int32", init=k_phase_cursor)
                    _advance_ring(k_stage_cursor, k_phase_cursor)
                    _mbar_wait(bar(_MBAR_K_FULL) + k_stage * _u32(8), k_phase)
                    qk_chain(instance, k_stage)

            union_index_2 = txl.local_scalar("int32", init=_i32(0))
            with txl.While(union_index_2 < max_union_count_m):
                for instance in range(2):
                    with txl.If(selected_blocks > union_index_2), txl.Then():
                        v_stage = txl.local_scalar("uint32", init=v_stage_cursor)
                        v_phase = txl.local_scalar("int32", init=v_phase_cursor)
                        ...
                        with txl.If(union_index_2 + _i32(1) == selected_blocks), txl.Then():
                            txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_O_FULL + 8 * instance), pred=leader)
                        txl.ptx[_TCGEN05_COMMIT](bar(_MBAR_V_EMPTY) + v_stage * _u32(8), pred=leader)
                    next_union_index = union_index_2 + _i32(1)
                    with txl.If(next_union_index < selected_blocks), txl.Then():
                        next_k_stage = txl.local_scalar("uint32", init=k_stage_cursor)
                        next_k_phase = txl.local_scalar("int32", init=k_phase_cursor)
                        _advance_ring(k_stage_cursor, k_phase_cursor)
                        _mbar_wait(bar(_MBAR_K_FULL) + next_k_stage * _u32(8), next_k_phase)
                        qk_chain(instance, next_k_stage)
                txl.assign(union_index_2, union_index_2 + _i32(1))
        ...
        with r_qk_load:
            ...
            selected_count = txl.local_scalar("int32", init=_i32(0))
            block_base = txl.local_scalar("int32", init=_i32(0))
            with txl.While(block_base < nb):
                n_block_1 = block_base + lane
                selected_1 = txl.local_scalar("int32", init=_i32(0))
                with txl.If(n_block_1 < nb), txl.Then():
                    mask_byte = txl.local_scalar("uint16")
                    txl.ptx.ld.global_.nc.b8(mask_byte, block_mask.ptr_to([mask_base + n_block_1]))
                    txl.assign(selected_1, txl.cast(mask_byte != txl.uint16(0), "int32"))
                vote = txl.local_scalar("uint32")
                txl.ptx.vote_sync.ballot.b32(
                    vote, txl.ptx.pred(txl.cast(selected_1 != _i32(0), "bool")), _u32(_FULL_MASK)
                )
                ballot_count = txl.local_scalar("uint32")
                txl.ptx.popc.b32(ballot_count, vote)
                lower_lanes = txl.shift_left(_u32(1), txl.cast(lane, "uint32")) - _u32(1)
                lower_count = txl.local_scalar("uint32")
                txl.ptx.popc.b32(lower_count, vote & lower_lanes)
                selected_slot = selected_count + txl.cast(lower_count, "int32")
                with txl.If(selected_1 != _i32(0)), txl.Then():
                    slot_addr = bar(_SMEM_UNION_BLOCKS) + txl.cast(selected_slot, "uint32") * _u32(4)
                    txl.ptx.st.shared.b32(slot_addr, n_block_1)
                    txl.ptx.st.shared.b32(slot_addr + _u32(192 * 4), n_block_1)
                txl.assign(selected_count, selected_count + txl.cast(ballot_count, "int32"))
                txl.assign(block_base, block_base + _i32(32))
            with txl.If(selected_count == _i32(0)), txl.Then():
                with txl.If(lane == 0), txl.Then():
                    txl.ptx.st.shared.b32(bar(_SMEM_UNION_BLOCKS), _i32(0))
                    txl.ptx.st.shared.b32(bar(_SMEM_UNION_BLOCKS + 192 * 4), _i32(0))
                txl.assign(selected_count, _i32(1))
            with txl.If(lane < 2), txl.Then():
                txl.ptx.st.shared.b32(
                    bar(_SMEM_UNION_COUNT) + txl.cast(lane, "uint32") * _u32(4), selected_count
                )
            txl.ptx.barrier.sync(_u32(8), _u32(32))
            txl.ptx.fence.proxy.async_.shared__cta()
            _mbar_arrive(bar(_MBAR_UNION_READY))
            k_stage_load = txl.local_scalar("uint32", init=_u32(0))
            phase_k_empty = txl.local_scalar("int32", init=_i32(1))
            union_index_3 = txl.local_scalar("int32", init=_i32(0))
            with txl.While(union_index_3 < selected_blocks):
                for instance in range(2):
                    n_block = _ld_shared_i32(
                        bar(_SMEM_UNION_BLOCKS + 192 * 4 * instance)
                        + txl.cast(union_index_3, "uint32") * _u32(4)
                    )
                ...
                txl.assign(union_index_3, union_index_3 + _i32(1))

        # ---- Role: V load warp (warp 14) ----
        with r_v_load:
            union_ready_phase_v = txl.local_scalar("int32", init=_i32(0))
            _mbar_wait(bar(_MBAR_UNION_READY), union_ready_phase_v)
            _flip(union_ready_phase_v)
            kv_head = q_head
            v_stage_load = txl.local_scalar("uint32", init=_u32(0))
            phase_v_empty = txl.local_scalar("int32", init=_i32(1))
            union_index_4 = txl.local_scalar("int32", init=_i32(0))
            with txl.While(union_index_4 < selected_blocks):
                for instance in range(2):
                    n_block = _ld_shared_i32(
                        bar(_SMEM_UNION_BLOCKS + 192 * 4 * instance)
                        + txl.cast(union_index_4, "uint32") * _u32(4)
                    )
                    token_base = n_block * 128
                ...
                txl.assign(union_index_4, union_index_4 + _i32(1))
