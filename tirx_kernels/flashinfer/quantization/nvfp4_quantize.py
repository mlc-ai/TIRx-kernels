...
def get_kernel(
    ...
    **kwargs,
    ...
    if linear:
        grid_x, block_x, _ = _linear_launch(m, k)
        needs_col_loop = False
        rows_per_block = 0
    else:
        grid_x, block_x, _ = _swizzled_launch(m, k, sf_layout)
        needs_col_loop = nsb > block_x
        rows_per_block = 1 if needs_col_loop else block_x // nsb

    with IRBuilder() as ib:
        with T.prim_func():
            T.func_name("nvfp4_quantize_linear" if linear else "nvfp4_quantize_swizzled")
            ...
            gs_ptr = T.arg("gs_ptr", T.handle())
            in_global = name(
                T.match_buffer(in_ptr, [m * k * (2 if fuse_silu else 1)], dtype, scope="global"),
                "in_global",
            )
            out_global = name(
                T.match_buffer(out_ptr, [m * (k // 2)], "uint8", scope="global"), "out_global"
            )
            sf_shape = (
                m * (k // NVFP4_SF_VEC_SIZE)
                if linear
                else _padded_m(m, sf_layout) * _padded_sf_cols(k)
            )
            sf_out = name(T.match_buffer(sf_ptr, [sf_shape], "uint8", scope="global"), "sf_out")
            gs = name(T.match_buffer(gs_ptr, [1], "float32", scope="global"), "gs")
            T.device_entry()
            ...
            gs_val = scalar("gs_val", "float32", ld_global_f32(gs, 0))

            if linear:
                stride = scalar("stride", "int32", opaque_i32(grid_x * _LINEAR_SF_BLOCKS_PER_TB))
                sf_idx = scalar("sf_idx", "int32", bx * _LINEAR_SF_BLOCKS_PER_TB + tx)
                with T.While(sf_idx < total_sf):
                    ...
                    scale, packed = _process_block(
                        in_global, row_idx, col_idx, gs_val, dtype=dtype, k=k, fuse_silu=fuse_silu
                    )
                    scale = scalar("scale_fp8", "uint8", scale)
                    packed = scalar("packed64", "uint64", packed)
                    st_global_u8(T.address_of(sf_out[sf_idx]), scale)
                    out_off = scalar("out_off", "int32", row_idx * (k // 2) + col_idx * 8)
                    st_global_u64(T.address_of(out_global[out_off]), packed)
                    store(sf_idx, sf_idx + stride)
            elif needs_col_loop:
                row_idx = scalar("row_idx", "int32", bx)
                with T.While(row_idx < padded_rows):
                    with T.If(row_idx >= m_rows):
                        with T.Then():
                            sc_pad = scalar("sc_pad", "int32", tx)
                            with T.While(sc_pad < pad_cols):
                                st_global_u8(
                                    T.address_of(sf_out[sf_offset(row_idx, sc_pad)]), T.uint8(0)
                                )
                                store(sc_pad, sc_pad + block_x)
                        with T.Else():
                            sc = scalar("sc", "int32", tx)
                            with T.While(sc < nsb):
                                scale, packed = _process_block(
                                    in_global,
                                    row_idx,
                                ...
                                scale = scalar("scale_fp8", "uint8", scale)
                                packed = scalar("packed64", "uint64", packed)
                                st_global_u8(T.address_of(sf_out[sf_offset(row_idx, sc)]), scale)
                                out_off = scalar("out_off", "int32", row_idx * (k // 2) + sc * 8)
                                st_global_u64(T.address_of(out_global[out_off]), packed)
                                store(sc, sc + block_x)
                            sc_tail = scalar("sc_tail", "int32", nsb + tx)
                            with T.While(sc_tail < pad_cols):
                                st_global_u8(
                                    T.address_of(sf_out[sf_offset(row_idx, sc_tail)]), T.uint8(0)
                                )
                                store(sc_tail, sc_tail + block_x)
                    store(row_idx, row_idx + grid_x)
            else:
                row_in_block = scalar("row_in_block", "int32", T.truncdiv(tx, T.int32(nsb)))
                sf_idx_in_row = scalar("sf_idx_in_row", "int32", T.truncmod(tx, T.int32(nsb)))
                    ...
                    with T.If(row_idx2 < padded_rows):
                        with T.Then():
                            with T.If(row_idx2 >= m_rows):
                                with T.Then():
                                    local_sf = scalar("local_sf", "int32", sf_idx_in_row)
                                    with T.While(local_sf < pad_cols):
                                        st_global_u8(
                                            T.address_of(sf_out[sf_offset(row_idx2, local_sf)]),
                                            T.uint8(0),
                                        )
                                        store(local_sf, local_sf + nsb)
                                with T.Else():
                                    with T.If(sf_idx_in_row < nsb):
                                        with T.Then():
                                            scale, packed = _process_block(
                                                in_global,
                                                row_idx2,
                                            ...
                                            st_global_u8(
                                                T.address_of(
                                                    sf_out[sf_offset(row_idx2, sf_idx_in_row)]
                                                ),
                                                scale,
                                            )
                                            out_off = scalar(
                                                "out_off",
                                                "int32",
                                                row_idx2 * (k // 2) + sf_idx_in_row * 8,
                                            )
                                            st_global_u64(T.address_of(out_global[out_off]), packed)
                                    if pad_cols != nsb:
                                        pad_col = scalar("pad_col", "int32", nsb + sf_idx_in_row)
                                        with T.While(pad_col < pad_cols):
                                            st_global_u8(
                                                T.address_of(sf_out[sf_offset(row_idx2, pad_col)]),
                                                T.uint8(0),
                                            )
