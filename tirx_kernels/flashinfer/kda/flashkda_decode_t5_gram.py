...
def _make_flashkda_decode_t5_gram(spec: dict[str, Any]):
    ...
    def flashkda_decode_t5_gram(
        ...
        s_grama0 = smem.alloc((16, 64), txl.bf16, swizzle=txl.SW128B, align=128)
        s_grama1 = smem.alloc((16, 64), txl.bf16, swizzle=txl.SW128B, align=128)
        smem.commit()
        work, n = txl.cta_id()
        ...
        # of truth. They cannot be a txl.specialize partition: one warp owns
        ...
        with txl.If(token_owner), txl.Then():
            token = warp
        ...
        with txl.If(row_owner), txl.Then():
        ...
        with txl.If(mma_owner), txl.Then():
        ...
        with txl.If(mma_owner), txl.Then():
            ...
            with txl.If(lane_quad == 2), txl.Then():
                ...
                row_hi = row_lo + 8
                v_lo_bits = txl.alloc_local((NUM_TOKENS,), txl.u16)
                v_hi_bits = txl.alloc_local((NUM_TOKENS,), txl.u16)
                for t in range(NUM_TOKENS):
                ...
                for t in range(NUM_TOKENS):
                    ...
                    txl.assign(solved_lo, _sub(_ptx_un("cvt.f32.bf16", v_lo_bits[t]), ha_lo[t]))
                    txl.assign(solved_hi, _sub(_ptx_un("cvt.f32.bf16", v_hi_bits[t]), ha_hi[t]))
                    for prev in range(t):
        ...
        with txl.If(txl.And(mma_owner, lane_quad >= 2)), txl.Then():
            ...
            with txl.If(lane_quad == 2), txl.Then():
                ...
                txl.assign(out4_lo, hc_lo[4])
                txl.assign(out4_hi, hc_hi[4])
