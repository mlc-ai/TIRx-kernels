...
def _make_flashkda_decode_t4_precomputed(spec: dict[str, Any]):
    ...
    def _flashkda_decode_t4_precomputed(
        ...
        K.assign(group[0], warp * 2 + lane // 16)
        K.assign(lane_group[0], lane % 16)
        ...
        for row_local in range(8):
            ...
            pack = _load_u32x4(
                state,
                head_base[0] + K.cast((tile_row_base[0] + row_l) * HEAD_DIM + k_start[0], "int64"),
            )
            for pr in range(4):
        ...
        with K.If(lane_quad[0] == 2), K.Then():
            ...
            for t in range(NUM_TOKENS):
                ...
                K.assign(
                    solved_lo[0],
                    _sub(
                        _load_bf16_f32(v, K.cast(base_t[0] + tile_row_base[0] + row_lo, "int64")),
                        ha_lo[t],
                    ),
                )
                K.assign(
                    solved_hi[0],
                    _sub(
                        _load_bf16_f32(v, K.cast(base_t[0] + tile_row_base[0] + row_hi, "int64")),
                        ha_hi[t],
                    ),
                )
                for prev in range(t):
        ...
        with K.If(lane_quad[0] >= 2), K.Then():
            ...
            for half in range(2):
                ...
                _store_f32_as_bf16(out, K.cast(base_o[0] + row_lo_f, "int64"), o_lo, active_o)
                _store_f32_as_bf16(out, K.cast(base_o[0] + row_hi_f, "int64"), o_hi, active_o)
                _store_f32_as_bf16(
                    out, K.cast(base_o[0] + row_lo_f, "int64"), K.float32(0.0), K.Not(active_o)
                )
                _store_f32_as_bf16(
                    out, K.cast(base_o[0] + row_hi_f, "int64"), K.float32(0.0), K.Not(active_o)
                )
