...
def _make_warp_uniform(value):
    ...
    K.evaluate(
        K.ptx.shfl_sync.idx.b32(
            out[0], K.reinterpret("uint32", value), K.uint32(0), K.uint32(31), K.uint32(0xFFFFFFFF)
        )
    )
...
def _make_flashkda_decode_t6_gram(spec: dict[str, Any]):
    ...
    def _flashkda_decode_t6_gram(
        ...
        with K.If(warp < NUM_TOKENS), K.Then():
            ...
            _st_shared_u32x4(arena, OFF_SK + (token * HEAD_DIM + elem_start[0]) * 4, k_pub)
            _st_shared_u32x4(arena, OFF_SD + (token * HEAD_DIM + elem_start[0]) * 4, d_pub)
            ...
            with K.If(lane == 0), K.Then():
                ...
                _st_shared_i32(arena, OFF_STOKEN + token * 4, token_pos[0])
                _st_shared_f32(
                    arena,
                    OFF_SBETA + token * 4,
                ...
                for j in range(NUM_TOKENS):
                    with K.If(token_c >= j), K.Then():
                        K.assign(
                            ...
                            _mul(
                                prefix[0],
                                _ld_shared_f32(arena, OFF_SD + (j * HEAD_DIM + k_idx) * 4),
                            ),
                        )
                _st_shared_b16(
                    arena,
        ...
        with K.If(group[0] < ROW_GROUPS), K.Then():
            for row_local in range(ROWS_PER_GROUP):
                ...
                with K.If(lane_group[0] < 8):
                    with K.Then():
                        _st_shared_u32x4(
                            arena, OFF_SSTATE0 + _swz(row_l * 128 + k_start[0] * 2), pack
                        )
                    with K.Else():
                        _st_shared_u32x4(
                            arena, OFF_SSTATE1 + _swz(row_l * 128 + (k_start[0] - 64) * 2), pack
                        )
        ...
        with K.If(warp < MMA_WARPS), K.Then():
            ...
            with K.If(lane_quad[0] == 2), K.Then():
                ...
                for t in range(NUM_TOKENS):
                    base_t = (
                        _ld_shared_i32(arena, OFF_STOKEN + t * 4) * NUM_VALUE_HEADS + hv[0]
                    ) * HEAD_DIM
                    solved_lo = K.alloc_local((1,), K.f32)
                    solved_hi = K.alloc_local((1,), K.f32)
                    K.assign(
                        solved_lo[0],
                        _sub(_load_bf16_f32(v, base_t + tile_row_base[0] + row_lo), ha_lo[t]),
                    )
                    K.assign(
                        solved_hi[0],
                        _sub(_load_bf16_f32(v, base_t + tile_row_base[0] + row_hi), ha_hi[t]),
                    )
                    for prev in range(t):
        ...
        with K.If(K.And(warp < MMA_WARPS, lane_quad[0] >= 2)), K.Then():
            ...
            for src in range(NUM_TOKENS):
                ...
                with K.If(token0 >= src), K.Then():
                    K.assign(
                        coef0[0], _ld_shared_f32(arena, OFF_SR + (token0 * NUM_TOKENS + src) * 4)
                    )
                with K.If(token1 >= src), K.Then():
                    K.assign(
                        coef1[0], _ld_shared_f32(arena, OFF_SR + (token1 * NUM_TOKENS + src) * 4)
                    )
            ...
            for half in range(2):
                ...
                base_o = (
                    _ld_shared_i32(arena, OFF_STOKEN + token_o * 4) * NUM_VALUE_HEADS + hv[0]
            ...
            with K.If(lane_quad[0] == 2), K.Then():
                ...
                K.assign(out4_lo[0], hc_lo[NUM_TOKENS - 2])
                K.assign(out4_hi[0], hc_hi[NUM_TOKENS - 2])
                ...
                base4 = (
                    _ld_shared_i32(arena, OFF_STOKEN + (NUM_TOKENS - 2) * 4) * NUM_VALUE_HEADS
                    + hv[0]
                ) * HEAD_DIM + tile_row_base[0]
                ...
                out5_lo = K.alloc_local((1,), K.f32)
                out5_hi = K.alloc_local((1,), K.f32)
                K.assign(out5_lo[0], hc_lo[NUM_TOKENS - 1])
                K.assign(out5_hi[0], hc_hi[NUM_TOKENS - 1])
                for src5 in range(NUM_TOKENS):
                    coef5 = _ld_shared_f32(
                        arena, OFF_SR + ((NUM_TOKENS - 1) * NUM_TOKENS + src5) * 4
                    )
                    K.assign(out5_lo[0], _fma(coef5, u_lo[src5], out5_lo[0]))
                    K.assign(out5_hi[0], _fma(coef5, u_hi[src5], out5_hi[0]))
                active5 = _ld_shared_i32(arena, OFF_SSLOT + (NUM_TOKENS - 1) * 4) >= 0
                base5 = (
                    _ld_shared_i32(arena, OFF_STOKEN + (NUM_TOKENS - 1) * 4) * NUM_VALUE_HEADS
                    + hv[0]
                ) * HEAD_DIM + tile_row_base[0]
                _store_f32_as_bf16(out, base5 + row_lo_f, out5_lo[0], active5)
                _store_f32_as_bf16(out, base5 + row_hi_f, out5_hi[0], active5)
        ...
        with K.If(group[0] < ROW_GROUPS), K.Then():
            for t in range(NUM_TOKENS):
                ...
                _ld_shared_f32x4(arena, OFF_SD + (t * HEAD_DIM + k_start[0]) * 4, sd_t, 0)
                _ld_shared_f32x4(arena, OFF_SD + (t * HEAD_DIM + k_start[0] + 4) * 4, sd_t, 4)
                _ld_shared_f32x4(arena, OFF_SK + (t * HEAD_DIM + k_start[0]) * 4, sk_t, 0)
                _ld_shared_f32x4(arena, OFF_SK + (t * HEAD_DIM + k_start[0] + 4) * 4, sk_t, 4)
