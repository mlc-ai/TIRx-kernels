...
#
# Helpers use the exact txl.ptx/txl.cuda wrappers available in the target TVM.
# ---------------------------------------------------------------------------
...
def _mma_chain(taddr_d, b_lo, taddr_a, enable_d, *, b_offsets, dhi, idesc):
    ...
    for _i, _b_off in enumerate(b_offsets):
        ...
        txl.ptx[_TCGEN05_MMA_F16](
            ...
            txl.uint32(idesc),
            *_MMA_ZERO_MASKS,
            txl.ptx.pred(enable_d if _i == 0 else 1),
            pred=leader,
        )
...
def _tensormap_acquire(tmap):
    return txl.ptx.fence.proxy.tensormap__generic.acquire.gpu(tmap)
...
def _mma_m16n8k16_bf16_zero(acc, a, b):
    ...
    return txl.ptx.mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32(
...
def _mma_m16n8k8_bf16_zero(acc, a, b):
    ...
    return txl.ptx.mma.sync.aligned.m16n8k8.row.col.f32.bf16.bf16.f32(
        *[acc[i] for i in range(4)],
...
def _mbarrier_wait(barrier, stage, phase):
    # .cu:158-171 -> native txl.cuda.mbarrier_wait: same LAB_WAIT spin loop with
    ...
    return txl.cuda.mbarrier_wait(barrier.ptr_to([stage]), phase ^ barrier.phase_offset)


def _elect_commit(mbar_addr):
    # .cu:239-248
    leader = txl.cuda.elect_sync()
    return txl.ptx.tcgen05.commit.cta_group__1.mbarrier__arrive__one.shared__cluster.b64(
...
def _elect_commit2(barrier_a, barrier_b, stage):
    ...
    for _bar in (barrier_a, barrier_b):
        txl.ptx.tcgen05.commit.cta_group__1.mbarrier__arrive__one.shared__cluster.b64(
...
def _mbarrier_arrive_expect_tx(barrier, stage, bytes_):
    # .cu:258-262
    return txl.ptx.mbarrier.arrive.expect_tx.release.cta.shared__cta.b64(
        barrier.ptr_to([stage]), txl.uint32(bytes_)
    )
...
def _tma_3d_gmem2smem(smem_raw, smem, dst, tmap_ptr, x, y, z, mbar_ptr):
    ...
    return txl.ptx[_TMA_G2S_CTA.format(dim=3)](
        smem_raw.ptr_to([dst - smem]), txl.reinterpret("uint64", tmap_ptr), x, y, z, mbar_ptr
    )


def _tma_2d_gmem2smem(smem_raw, smem, dst, tmap_ptr, x, y, mbar_ptr):
    # .cu:441-449 -> native (see _tma_3d_gmem2smem note)
    return txl.ptx[_TMA_G2S_CTA.format(dim=2)](
        smem_raw.ptr_to([dst - smem]), txl.reinterpret("uint64", tmap_ptr), x, y, mbar_ptr
    )


def _tma_4d_gmem2smem(smem_raw, smem, dst, tmap_ptr, x, y, z, w, mbar_ptr):
    # .cu:452-460 -> native (see _tma_3d_gmem2smem note)
    return txl.ptx[_TMA_G2S_CTA.format(dim=4)](
        smem_raw.ptr_to([dst - smem]), txl.reinterpret("uint64", tmap_ptr), x, y, z, w, mbar_ptr
    )
...
def _tma_store_4d(smem_raw, smem, tmap, x, y, z, w, smem_addr):
    ...
    return txl.ptx[_TMA_S2G.format(dim=4)](
        txl.reinterpret("uint64", tmap), x, y, z, w, smem_raw.ptr_to([smem_addr - smem])
    )
...
def bf16_fused_m128(**kwargs: Any):
    ...
    @txl.kernel(warps=THREADS // 32, arch="sm_100a", min_blocks_per_sm=1, grid=num_seqs * h)
    def _kernel(
        q: txl.gptr[txl.bf16],
        k: txl.gptr[txl.bf16],
        v: txl.gptr[txl.bf16],
        ...
        beta_tma: txl.gptr[txl.bf16, 2],
        A_log: txl.gptr[txl.f32],
        dt_bias: txl.gptr[txl.f32],
        cu_seqlens: txl.gptr[txl.i64],
        seq_order: txl.gptr[txl.i32],
        initial_state: txl.gptr[txl.bf16],
        out: txl.gptr[txl.bf16],
        final_state: txl.gptr[txl.bf16],
        descriptor_storage: txl.gptr[txl.u8],
    ):
        ...
        smem_pool = txl.smem_pool()
        # Declaration order preserves the frozen 0..616 barrier byte map.
        qk_full = txl.MBarrier(smem_pool.pool, 5, leader=True)
        gate_raw_full = txl.TMABar(smem_pool.pool, 5, leader=True)
        qk_raw_full = txl.TMABar(smem_pool.pool, 5, leader=True)
        v_full = txl.TMABar(smem_pool.pool, 5, leader=True)
        v_free = txl.MBarrier(smem_pool.pool, 5, phase_offset=1, leader=True)
        smem_free = txl.TCGen05Bar(smem_pool.pool, 5, phase_offset=1, leader=True)
        raw_inputs_free = txl.TCGen05Bar(smem_pool.pool, 5, phase_offset=1, leader=True)
        state_inp_ready = txl.MBarrier(smem_pool.pool, 5, leader=True)
        old_out_ready = txl.TCGen05Bar(smem_pool.pool, 5, leader=True)
        u_inp_ready = txl.MBarrier(smem_pool.pool, 5, leader=True)
        u2_acc_ready = txl.TCGen05Bar(smem_pool.pool, 5, leader=True)
        u2_inp_ready = txl.MBarrier(smem_pool.pool, 5, leader=True)
        final_ready = txl.TCGen05Bar(smem_pool.pool, 5, leader=True)
        out_empty = txl.MBarrier(smem_pool.pool, 1, phase_offset=1, leader=True)
        tmem_dealloc_ready = txl.MBarrier(smem_pool.pool, 1, leader=True)
        prep_diag_ready = txl.MBarrier(smem_pool.pool, 5, leader=True)
        prep_inv16_ready = txl.MBarrier(smem_pool.pool, 5, leader=True)
        tmem_addr_storage = smem_pool.alloc((1,), txl.i32, align=4)
        if smem_pool.bytes != 620:
            raise AssertionError(f"unexpected mbarrier/TMEM header size: {smem_pool.bytes}")
        smem_pool.alloc((1024 - smem_pool.bytes,), txl.u8)
        smem_stage_storage = smem_pool.alloc(SMEM_STAGE_ALLOCATION, txl.bf16, align=1024)
        smem_out_storage = smem_pool.alloc((2, 32, 128), txl.bf16)
        smem_pool.commit(SMEM_TOTAL)
        smem_raw = txl.decl_buffer(
            (SMEM_TOTAL,), txl.u8, data=qk_full.buf.data, scope="shared.dyn", align=1024
        )
        ...
        smem_g_raw_all = shared_view(
            (SMEM_SMEM_G_RAW_ALL_STAGE_BYTES // 2,), txl.bf16, SMEM_SMEM_G_RAW_ALL_OFF
        )
        smem_v_all = shared_view((SMEM_SMEM_V_ALL_STAGE_BYTES // 2,), txl.bf16, SMEM_SMEM_V_ALL_OFF)
        ...
        smem_prep_beta_all = shared_view(
            (SMEM_SMEM_PREP_BETA_ALL_STAGE_BYTES // 4,), txl.f32, SMEM_SMEM_PREP_BETA_ALL_OFF
        )
        smem_gate_rate_all = shared_view(
            (SMEM_SMEM_GATE_RATE_ALL_STAGE_BYTES // 4,), txl.f32, SMEM_SMEM_GATE_RATE_ALL_OFF
        )
        ...
        smem = txl.local_scalar(
            "uint32", init=txl.cuda.cvta_generic_to_shared(txl.address_of(smem_raw[0]))
        )
        bid = block_idx
        smem_stage_addr = txl.local_scalar(
            "int32",
            init=txl.cast(
                txl.cuda.cvta_generic_to_shared(txl.address_of(smem_stage_storage[0, 0])), "int32"
            ),
        )
        ...
        smem_v_all_addr = txl.local_scalar(
            "int32",
            init=txl.cast(txl.cuda.cvta_generic_to_shared(txl.address_of(smem_v_all[0])), "int32"),
        )
        smem_v_addr = smem_v_all_addr
        smem_inv_work_addr = smem_v_all_addr
        smem_restore_factor_all_addr = txl.local_scalar(
            "int32",
            init=txl.cast(
                txl.cuda.cvta_generic_to_shared(txl.address_of(smem_restore_factor_all[0])), "int32"
            ),
        )
        smem_beta_raw_addr = smem_restore_factor_all_addr
        smem_out_addr = txl.local_scalar(
            "int32",
            init=txl.cast(
                txl.cuda.cvta_generic_to_shared(txl.address_of(smem_out_storage[0, 0, 0])), "int32"
            ),
        )
        ...
        with txl.If(txl.warp_id() == 0), txl.Then():
            txl.ptx.tcgen05.alloc.cta_group__1.sync.aligned.shared__cta.b32(
                txl.address_of(tmem_addr_storage[0]), txl.uint32(256)
            )
        txl.cuda.cta_sync()
        txl.ptx.tcgen05.fence__after_thread_sync()
        taddr_storage = txl.local_scalar(txl.i32)
        txl.ptx.ld.volatile.shared.s32(taddr_storage, txl.address_of(tmem_addr_storage[0]))
        taddr = txl.local_scalar("int32", init=taddr_storage)
        ...
        def _work_item():
            ...
            task_idx: txl.int32 = bid
            seq_idx = txl.local_scalar("int32")
            txl.ptx.ld.global_.s32(seq_idx, seq_order.ptr_to([task_idx // h]))
            head_idx: txl.int32 = task_idx % h
            bos = txl.local_scalar("int64")
            txl.ptx.ld.global_.s64(bos, cu_seqlens.ptr_to([seq_idx]))
            eos = txl.local_scalar("int64")
            txl.ptx.ld.global_.s64(eos, cu_seqlens.ptr_to([seq_idx + 1]))
            seq_len: txl.int32 = txl.cast(eos - bos, "int32")
        ...
        def compute_role(compute_state):
            ...
            warp_in_wg: txl.int32 = warp % 4  # .cu:740
            tmem_row_base: txl.int32 = (warp_in_wg * 32) << 16  # .cu:741
            state_row: txl.int32 = warp_in_wg * 32 + lane  # .cu:742
            ...
            final_state_u32 = txl.decl_buffer(
                (num_seqs * h * D_HEAD * D_HEAD // 2,), "uint32", data=final_state.data
            )
            with txl.unroll(4) as state_col_block:  # .cu:746-822 (#pragma unroll)
                state_frag = txl.alloc_local((32,), "float32")
                with txl.unroll(32) as _zi:  # .cu:749-780
                    txl.ptx.mov.b32(state_frag[_zi], txl.float32(0.0))
                if use_initial_state:  # .cu:781
                    # .cu:783-819: two halves, each 2x uint4 load + bf16->f32 shl/and unpack
                    for _half in (0, 16):
                        with txl.unroll(2) as _blk:
                            _vld = txl.alloc_local((4,), "uint32", align=16)
                            ...
                            with txl.unroll(4) as _pair:
                                txl.ptx.mov.b32(
                                    ...
                                    txl.cuda.uint_as_float(_vld[_pair] << txl.uint32(16)),
                                )
                                txl.ptx.mov.b32(
                                    state_frag[_half + _blk * 8 + _pair * 2 + 1],
                                    txl.cuda.uint_as_float(_vld[_pair] & txl.uint32(0xFFFF0000)),
                                )
                txl.ptx["tcgen05.st.sync.aligned.32x32b.x32.b32"](
                    txl.cast(taddr + 64 + tmem_row_base + state_col_block * 32, "uint32"),
                    *[state_frag[_j] for _j in range(32)],
                )  # .cu:821
            txl.ptx.tcgen05.wait__st.sync.aligned()  # .cu:823 tcgen05.wait::st.sync.aligned
            with txl.serial(
                ...
                compute_stage_byte_base = (
                    txl.cast(compute_state.stage, "int32") * SMEM_STAGE_BYTE_STRIDE
                )
                ...
                with txl.serial(
                    ...
                    state_addr: txl.int32 = (
                        taddr + 64 + tmem_row_base + state_col_block_1 * 32
                    )  # .cu:835
                    _tmem_load_0 = txl.alloc_local((32,), "float32")
                    txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                        *[_tmem_load_0[_j] for _j in range(32)], txl.cast(state_addr, "uint32")
                    )  # .cu:836-837
                    _tmem_load_0_bf16 = txl.alloc_local((16,), "uint32")
                    with txl.unroll(16) as _lp:  # .cu:839-843
                        txl.ptx.mov.b32(
                    ...
                    txl.ptx["tcgen05.st.sync.aligned.32x32b.x16.b32"](
                        txl.cast(taddr + tmem_row_base + state_col_block_1 * 16, "uint32"),
                        *[_tmem_load_0_bf16[_j] for _j in range(16)],
                    )
                    state_scale = txl.alloc_local((16,), "float32")
                    with txl.unroll(2) as state_half:  # .cu:850-859
                        with txl.unroll(4) as state_vec:
                        ...
                        with txl.unroll(8) as _ls:  # .cu:857-858
                            _pk = txl.local_scalar("uint64")
                            ...
                            txl.ptx.mov.b32(
                                _tmem_load_0[state_half * 16 + _ls * 2 + 1], txl.cuda.float2_y(_pk)
                            )
                    txl.ptx["tcgen05.st.sync.aligned.32x32b.x32.b32"](
                        txl.cast(state_addr, "uint32"), *[_tmem_load_0[_j] for _j in range(32)]
                    )  # .cu:860
                txl.ptx.tcgen05.wait__st.sync.aligned()  # .cu:862
                with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:863-865
                ...
                _tmem_load_1 = txl.alloc_local((32,), "float32")
                txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                    *[_tmem_load_1[_j] for _j in range(32)],
                    txl.cast(taddr + 224 + tmem_row_base, "uint32"),
                )  # .cu:868-869
                with txl.unroll(2) as residual_half:  # .cu:870-895
                    residual_v = txl.alloc_local((16,), "float32")
                    residual_beta = txl.alloc_local((16,), "float32")
                    with txl.unroll(16) as residual_col:  # .cu:874-881
                        token_col: txl.int32 = residual_half * 16 + residual_col  # .cu:876
                        _bits = txl.local_scalar("uint16")
                    ...
                    with txl.unroll(8) as _ls:  # .cu:885-887
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(residual_v[_ls * 2], residual_v[_ls * 2 + 1]),
                        ...
                        txl.ptx.mov.b32(residual_v[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.ptx.mov.b32(residual_v[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    residual_v_bf16 = txl.alloc_local((8,), "uint32")
                    with txl.unroll(8) as _lp:  # .cu:888-893
                        txl.ptx.mov.b32(
                            residual_v_bf16[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    txl.ptx["tcgen05.st.sync.aligned.32x32b.x8.b32"](
                        txl.cast(taddr + 224 + tmem_row_base + residual_half * 8, "uint32"),
                        *[residual_v_bf16[_j] for _j in range(8)],
                    )  # .cu:894
                txl.ptx.tcgen05.wait__st.sync.aligned()  # .cu:896
                with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:897-900
                ...
                txl.ptx.tcgen05.wait__st.sync.aligned()  # .cu:915
                with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:916-918
            ...
            with txl.If(store_final_state), txl.Then():  # .cu:923-955
                with txl.unroll(4) as state_col_block_2:
                    _tmem_load_3 = txl.alloc_local((32,), "float32")
                    txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                        *[_tmem_load_3[_j] for _j in range(32)],
                        txl.cast(taddr + 64 + tmem_row_base + state_col_block_2 * 32, "uint32"),
                    )  # .cu:926-927
                    with txl.unroll(2) as _half2:  # .cu:928-953 (two 16-float groups)
                        _pk = txl.alloc_local((8,), "uint32")
                        with txl.unroll(8) as _pj:
                            txl.ptx.mov.b32(
                                _pk[_pj],
                                txl.cuda.float22bfloat162_rn(
            ...
            txl.ptx.bar.sync(txl.uint32(10), txl.uint32(128))  # .cu:956 barrier.sync 10, 128
            with txl.If(compute_local_warp == 0), txl.Then():  # .cu:957-961
        ...
        def epilogue_role(epilogue_state, output_state):
            ...
            warp_id_in_role_1: txl.int32 = txl.warp_id_in_role()  # .cu:974
            epilogue_local_warp: txl.int32 = warp_id_in_role_1  # .cu:975
            warp_in_wg_1: txl.int32 = warp % 4  # .cu:976
            tmem_row_base_1: txl.int32 = (warp_in_wg_1 * 32) << 16  # .cu:977
            state_row_1: txl.int32 = warp_in_wg_1 * 32 + lane  # .cu:978
            with txl.serial(0, num_chunks_1, unroll=False) as chunk_idx_1:  # .cu:982-983
                _mbarrier_wait(final_ready, epilogue_state.stage, epilogue_state.phase)  # .cu:984
                chunk_is_full: txl.int32 = txl.if_then_else(
                    seq_len_1 >= (chunk_idx_1 + 1) * 32, 1, 0
                )  # .cu:985
                with txl.If(chunk_is_full != 0):
                    with txl.Then():  # .cu:986
                        # .cu:987-993 (inline tcgen05.ld.16x256b.x4, 16 uint32 regs)
                        _tmem_load_4 = txl.alloc_local((16,), "uint32")
                        txl.ptx["tcgen05.ld.sync.aligned.16x256b.x4.b32"](
                            *[_tmem_load_4[_j] for _j in range(16)],
                            txl.cast(taddr + 192 + tmem_row_base_1, "uint32"),
                        )
                        # .cu:994-1000 (same at TMEM row +16)
                        _tmem_load_5 = txl.alloc_local((16,), "uint32")
                        txl.ptx["tcgen05.ld.sync.aligned.16x256b.x4.b32"](
                        ...
                        with txl.If(epilogue_local_warp == 0), txl.Then():  # .cu:1008-1012
                            with txl.If(chunk_idx_1 >= 2), txl.Then():
                                txl.ptx.cp.async_.bulk.wait_group.read(1)
                        txl.ptx.bar.sync(txl.uint32(9), txl.uint32(128))  # .cu:1013
                        out_stage_addr: txl.int32 = (
                            smem_out_addr + txl.cast(output_state.phase, "int32") * 8192
                        )  # .cu:1014
                        with txl.unroll(2) as dim_half:  # .cu:1015-1049
                            out_packed = txl.alloc_local((8,), "uint32", align=4)  # .cu:1017
                            with txl.If(dim_half == 0):
                                with txl.Then():  # .cu:1018-1023
                                    with txl.unroll(8) as _lp:
                                        txl.ptx.mov.b32(
                                            out_packed[_lp],
                                            txl.cuda.float22bfloat162_rn(
                            ...
                            with txl.unroll(2) as token_group:  # .cu:1031-1048
                                mtx_idx: txl.int32 = lane // 8  # .cu:1033
                                row_addr: txl.int32 = lane & 7  # .cu:1034
                                dim_base: txl.int32 = (
                                    epilogue_local_warp * 32 + dim_half * 16 + (mtx_idx & 1) * 8
                                )  # .cu:1035
                                token_base: txl.int32 = (
                                    token_group * 16 + mtx_idx // 2 * 8
                                )  # .cu:1036
                                token_addr: txl.int32 = token_base + row_addr  # .cu:1037
                                token_pair: txl.int32 = token_addr // 2  # .cu:1038
                                token_parity: txl.int32 = token_addr & 1  # .cu:1039
                                raw_row: txl.int32 = token_pair + dim_base // 64 * 16  # .cu:1040
                                raw_col: txl.int32 = (
                                    dim_base & 63 ^ (token_pair & 3) << 4 ^ token_parity << 3
                                ) + token_parity * 64  # .cu:1041
                                stsm_offset: txl.int32 = (raw_row * 128 + raw_col) * 2  # .cu:1042
                                pack_base: txl.int32 = token_group * 4  # .cu:1043
                                # .cu:1044-1047 stmatrix.x4.trans
                                txl.ptx.stmatrix.sync.aligned.m8n8.x4.trans.shared.b16(
                                    smem_raw.ptr_to([txl.cast(out_stage_addr, "uint32") - smem + txl.cast(stsm_offset, "uint32")]),
                        ...
                        txl.ptx.bar.sync(txl.uint32(9), txl.uint32(128))  # .cu:1051
                        with txl.If(epilogue_local_warp == 0), txl.Then():  # .cu:1052-1057
                            with txl.If(txl.cuda.elect_sync()), txl.Then():
                                _tma_store_4d(
                                    smem_raw,
                                    txl.cast(smem, "int32"),
                                    out_tma,
                                    0,
                                    txl.cast(bos_1 + txl.cast(chunk_idx_1 * 32, "int64"), "int32"),
                                    head_idx_1,
                                    0,
                                    txl.cast(
                                        smem_out_addr
                            ...
                            txl.ptx.cp.async_.bulk.commit_group()  # .cu:1056
                        output_state.advance()  # .cu:1058
                    with txl.Else():  # .cu:1059-1077 (partial chunk: scalar out stores)
                        _tmem_load_6 = txl.alloc_local((32,), "float32")
                        txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                            *[_tmem_load_6[_j] for _j in range(32)],
                            txl.cast(taddr + 192 + tmem_row_base_1, "uint32"),
                        )  # .cu:1060-1061
                        txl.ptx.tcgen05.wait__ld.sync.aligned()  # .cu:1062
                        txl.ptx.bar.sync(txl.uint32(9), txl.uint32(128))  # .cu:1063
                        with txl.If(epilogue_local_warp == 0), txl.Then():  # .cu:1064-1068
                            with txl.If(txl.cuda.elect_sync()), txl.Then():
                                _mbarrier_arrive(out_empty, 0)
                        with txl.unroll(32) as token_col_1:  # .cu:1069-1076
                            out_token: txl.int64 = bos_1 + txl.cast(
                                chunk_idx_1 * 32 + token_col_1, "int64"
                            )  # .cu:1071
                            with txl.If(out_token < eos_1), txl.Then():  # .cu:1072
                                out_idx: txl.int64 = (
                                    out_token * txl.cast(h, "int64") + txl.cast(head_idx_1, "int64")
                                ) * 128 + txl.cast(state_row_1, "int64")  # .cu:1073
                                txl.ptx.st.global_.b16(
                                    out.ptr_to([out_idx]),
                                    txl.reinterpret(
                                        "uint16", txl.cast(_tmem_load_6[token_col_1], "bfloat16")
                                    ),
                                )  # .cu:1074
                epilogue_state.advance()
            with txl.If(epilogue_local_warp == 0), txl.Then():  # .cu:1081-1083
                txl.ptx.cp.async_.bulk.wait_group(0)
            txl.ptx.bar.sync(txl.uint32(9), txl.uint32(128))  # .cu:1084
            with txl.If(epilogue_local_warp == 0), txl.Then():  # .cu:1085-1089
                with txl.If(txl.cuda.elect_sync()), txl.Then():
                    _mbarrier_arrive(tmem_dealloc_ready, 0)
        ...
        def mma_role(mma_state, out_state):
            ...
            with txl.serial(0, num_chunks_2, unroll=False) as _chunk_idx:  # .cu:1106-1107
                ...
                _mma_b_addr_3: txl.int32 = smem_final_trans_addr + mma_stage_byte_base  # .cu:1215
                _mma_b_lo_3 = txl.uniform(
                    txl.cast(((_mma_b_addr_3 >> 4) & 0x3FFF) | 0x1000000, "uint32")
                )  # .cu:1216
                _mma_chain(
                    tmem_tmem_state_out,
                    txl.cast(_mma_b_lo_3, "int32"),
                    tmem_tmem_u2_inp,
                    1,
                    **_MMA_FINAL_2STEP,
                )  # .cu:1217-1235
                ...
                mma_state.advance()
            _mbarrier_wait(tmem_dealloc_ready, 0, txl.uint32(0))  # .cu:1241
            _tmem_dealloc_addr = txl.local_scalar("int32")  # .cu:1243
            txl.ptx.ld.volatile.shared.s32(_tmem_dealloc_addr, txl.address_of(tmem_addr_storage[0]))
            txl.ptx.tcgen05.dealloc.cta_group__1.sync.aligned.b32(
                txl.cast(_tmem_dealloc_addr, "uint32"), txl.uint32(256)
            )  # .cu:1244
            # Preserve the source's dealloc-before-relinquish order.
            txl.ptx.tcgen05.relinquish_alloc_permit.cta_group__1.sync.aligned()  # .cu:1245
        ...
        def load_role(load_state):
            ...
            with txl.serial(0, num_chunks_3, unroll=False) as chunk_idx_2:  # .cu:1260-1261
                ...
                load_stage_byte_base = txl.cast(load_state.stage, "int32") * SMEM_STAGE_BYTE_STRIDE
                load_stage_bf16_base = load_stage_byte_base // 2
                chunk_is_full_1: txl.int32 = txl.if_then_else(
                    seq_len_3 >= (chunk_idx_2 + 1) * 32, 1, 0
                )  # .cu:1264
                with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:1265-1270
                    with txl.If(chunk_is_full_1 != 0), txl.Then():
                        ...
                        _tma_3d_gmem2smem(
                            ...
                            0,
                            head_idx_2,
                            txl.cast(bos_3 + txl.cast(chunk_idx_2 * 32, "int64"), "int32"),
                            v_full.ptr_to([load_state.stage]),
                        )  # .cu:1268
                with txl.If(chunk_is_full_1 == 0), txl.Then():  # .cu:1271-1285
                    with txl.unroll(16) as v_load_iter:  # .cu:1272-1282
                        ...
                        ) * 128 + txl.cast(segment * 8, "int64")  # .cu:1279
                        txl.ptx["cp.async.cg.shared.global"](
                            ...
                            v.ptr_to([v_src]),
                            16,
                            txl.cast(txl.if_then_else(token_valid != 0, 16, 0), "uint32"),
                        )  # .cu:1280-1281  # fmt: skip
                    txl.ptx.cp.async_.commit_group()  # .cu:1283
                    txl.ptx.cp.async_.wait_group(0)  # .cu:1284
                txl.ptx.bar.sync(txl.uint32(8), txl.uint32(32))  # .cu:1286 barrier.sync 8, 32
                with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:1287-1292
                    with txl.If(chunk_is_full_1 == 0), txl.Then():
        ...
        def prep_role(prep_phase):
            ...
            warp_id_in_role_2: txl.int32 = txl.warp_id_in_role()  # .cu:1310
            prep_instance: txl.int32 = warp_id_in_role_2 // 4  # .cu:1308-1309
            prep_local_warp: txl.int32 = warp_id_in_role_2 % 4  # .cu:1311
            prep_tid: txl.int32 = prep_local_warp * 32 + lane  # .cu:1312
            num_prep_iters: txl.int32 = (num_chunks_4 + 4 - prep_instance) // 5  # .cu:1313
            prep_stage: txl.uint32 = txl.cast(prep_instance, "uint32")  # .cu:1314
            gate_rate_stage_f32: txl.int32 = prep_instance * SMEM_STAGE_F32_STRIDE  # .cu:1315

            def _prep_bar_sync():
                # switch(prep_instance) over the five per-instance named barriers 11..15.
                def _arm(i):
                    if i == 4:
                        txl.ptx.bar.sync(txl.uint32(15), txl.uint32(128))
                        return
                    with txl.If(prep_instance == i):
                        with txl.Then():
                            txl.ptx.bar.sync(txl.uint32(11 + i), txl.uint32(128))
                        with txl.Else():
                            _arm(i + 1)

                _arm(0)

            with txl.If(prep_tid == 0), txl.Then():  # .cu:1316-1319
                a_log_value = txl.local_scalar("float32")
                txl.ptx.ld.global_.f32(a_log_value, A_log.ptr_to([head_idx_3]))
                _i32 = txl.local_scalar("float32")
                txl.ptx.ex2.approx.ftz.f32(_i32, a_log_value * txl.float32(1.4426950408889634))
                txl.ptx.st.shared.f32(smem_gate_rate_all.ptr_to([gate_rate_stage_f32]), _i32)
            _prep_bar_sync()  # .cu:1320-1332
            with txl.serial(0, num_prep_iters, unroll=False) as prep_iter:  # .cu:1339-1340
                chunk_idx_3: txl.int32 = prep_iter * 5 + prep_instance  # .cu:1341
                prep_stage_byte_base = (
                    txl.cast(prep_stage, "int32") * SMEM_STAGE_BYTE_STRIDE
                )  # .cu:1342
                stage_f32 = prep_stage_byte_base // 4
                stage_bf16 = prep_stage_byte_base // 2  # .cu:1343
                chunk_is_full_2: txl.int32 = txl.if_then_else(
                    seq_len_4 >= (chunk_idx_3 + 1) * 32, 1, 0
                )  # .cu:1344
                early_beta_value = txl.local_scalar("float32")  # .cu:1345
                early_gate0 = txl.local_scalar("float32")  # .cu:1346
                txl.assign(early_beta_value, txl.float32(0.0))
                txl.assign(early_gate0, txl.float32(0.0))
                with txl.If(chunk_is_full_2 != 0), txl.Then():  # .cu:1347-1392
                    _mbarrier_wait(raw_inputs_free, prep_stage, prep_phase.phase)  # .cu:1348
                    with txl.If(prep_local_warp == 0), txl.Then():  # .cu:1349-1357
                        with txl.If(txl.cuda.elect_sync()), txl.Then():
                            ...
                            _tma_3d_gmem2smem(
                                ...
                                0,
                                head_idx_3,
                                txl.cast(bos_4 + txl.cast(chunk_idx_3 * 32, "int64"), "int32"),
                    ...
                    with (
                        ...
                        txl.ptx.ld.shared.b32(beta_raw_pair[0], smem_raw.ptr_to([beta_raw_addr - txl.cast(smem, "int32")]))  # .cu:1361  # fmt: skip
                        beta_raw_pair_fp32 = txl.alloc_local((2,), "float32")  # .cu:1362
                        with txl.unroll(1) as _pair:  # .cu:1363-1372
                            txl.ptx.mov.b32(
                                beta_raw_pair_fp32[_pair * 2],
                                txl.cuda.uint_as_float(beta_raw_pair[_pair + 0] << txl.uint32(16)),
                            )
                            txl.ptx.mov.b32(
                                beta_raw_pair_fp32[_pair * 2 + 1],
                                txl.cuda.uint_as_float(
                                    beta_raw_pair[_pair + 0] & txl.uint32(0xFFFF0000)
                                ),
                            )
                        beta_logit = txl.local_scalar(
                            "float32", init=beta_raw_pair_fp32[0]
                        )  # .cu:1373
                        with txl.If(head_idx_3 % 2 != 0), txl.Then():  # .cu:1374-1376
                            txl.assign(beta_logit, beta_raw_pair_fp32[1])
                        _tanh = txl.local_scalar("float32")
                        txl.ptx.tanh.approx.f32(_tanh, beta_logit * txl.float32(0.5))
                        txl.assign(
                            early_beta_value, _tanh * txl.float32(0.5) + txl.float32(0.5)
                        )  # .cu:1377-1379
                    with txl.If(prep_tid < 128), txl.Then():  # .cu:1381-1391
                        ...
                        _bits = txl.local_scalar("uint16")
                        txl.ptx.ld.shared.b16(_bits, smem_g_raw_all.ptr_to([stage_bf16 + prep_tid]))
                        early_gate_raw: txl.f32 = txl.cuda.bfloat162float(
                            txl.reinterpret("bfloat16", _bits)
                        )  # .cu:1384-1385
                        early_gate_arg: txl.f32 = early_gate_rate * (
                            early_gate_raw + early_gate_bias
                        )  # .cu:1386
                        _tanh = txl.local_scalar("float32")
                        txl.ptx.tanh.approx.f32(_tanh, early_gate_arg * txl.float32(0.5))
                        early_gate_sigmoid: txl.f32 = _tanh * txl.float32(0.5) + txl.float32(
                            0.5
                        )  # .cu:1387-1389
                        txl.assign(
                            early_gate0,
                ...
                with txl.If(chunk_is_full_2 != 0), txl.Then():  # .cu:1394-1400
                    with txl.If(prep_local_warp == 0), txl.Then():
                        with txl.If(txl.cuda.elect_sync()), txl.Then():
                            _tma_4d_gmem2smem(
                                smem_raw,
                                txl.cast(smem, "int32"),
                ...
                with txl.If(chunk_is_full_2 == 0), txl.Then():  # .cu:1401-1412
                    with txl.unroll(4) as gate_load_pass:
                        gate_load_item: txl.int32 = gate_load_pass * 128 + prep_tid  # .cu:1404
                        gate_load_row: txl.int32 = gate_load_item // 16  # .cu:1405
                        gate_load_segment: txl.int32 = gate_load_item % 16  # .cu:1406
                        gate_load_token: txl.int64 = bos_4 + txl.cast(
                        ...
                        txl.ptx.mad.lo.s64(
                            ...
                            gate_load_base,
                            txl.int64(128),
                            txl.cast(gate_load_segment * 8, "int64"),
                        )  # .cu:1408
                        txl.ptx["cp.async.cg.shared.global"](
                            ...
                            16,
                            txl.cast(txl.if_then_else(chunk_idx_3 * 32 + gate_load_row < seq_len_4, 16, 0), "uint32"),
                        )  # .cu:1409-1410  # fmt: skip
                with txl.If(chunk_is_full_2 == 0), txl.Then():  # .cu:1413-1429
                    txl.ptx.cp.async_.commit_group()  # .cu:1414
                    txl.ptx.cp.async_.wait_group(0)  # .cu:1415
                    _prep_bar_sync()
                with txl.If(txl.And(prep_local_warp == 2, lane < 32)), txl.Then():  # .cu:1430-1442
                    beta_value = txl.local_scalar("float32", init=early_beta_value)  # .cu:1431
                    with txl.If(chunk_is_full_2 == 0), txl.Then():  # .cu:1432-1440
                        beta_token: txl.int64 = bos_4 + txl.cast(chunk_idx_3 * 32 + lane, "int64")
                        with txl.If(chunk_idx_3 * 32 + lane < seq_len_4), txl.Then():
                            _bits = txl.local_scalar("uint16")
                            ...
                            _tanh = txl.local_scalar("float32")
                            txl.ptx.tanh.approx.f32(_tanh, beta_logit_1 * txl.float32(0.5))
                            txl.assign(
                                beta_value, _tanh * txl.float32(0.5) + txl.float32(0.5)
                            )  # .cu:1436-1438
                    txl.ptx.st.shared.f32(
                        smem_prep_beta_all.ptr_to([stage_f32 + lane]), beta_value
                    )  # .cu:1441
                with txl.If(prep_tid < 128), txl.Then():  # .cu:1443-1472
                    gate_col: txl.int32 = prep_tid  # .cu:1444
                    gate_rate = txl.local_scalar("float32")  # .cu:1445
                    txl.ptx.ld.shared.f32(gate_rate, smem_gate_rate_all.ptr_to([stage_f32]))
                    gate_bias = txl.local_scalar("float32")  # .cu:1446
                    txl.ptx.ld.global_.f32(gate_bias, dt_bias.ptr_to([head_idx_3 * 128 + gate_col]))
                    prefix_log2 = txl.local_scalar("float32", init=txl.float32(0.0))  # .cu:1447
                    with txl.serial(0, 32) as gate_row:  # .cu:1448-1471
                        gate_log2 = txl.local_scalar("float32", init=txl.float32(0.0))  # .cu:1450
                        gate_needs_compute = txl.local_scalar("int32")
                        txl.assign(gate_needs_compute, 1)  # .cu:1451
                        with txl.If(gate_row == 0), txl.Then():  # .cu:1452-1457
                            with txl.If(chunk_is_full_2 != 0), txl.Then():
                                txl.assign(gate_log2, early_gate0)
                                txl.assign(gate_needs_compute, 0)
                        with txl.If(gate_needs_compute != 0), txl.Then():  # .cu:1458-1468
                            with txl.If(chunk_idx_3 * 32 + gate_row < seq_len_4), txl.Then():
                                _bits = txl.local_scalar("uint16")
                                txl.ptx.ld.shared.b16(
                                    _bits,
                                    smem_g_raw_all.ptr_to([stage_bf16 + gate_row * 128 + gate_col]),
                                )
                                gate_raw: txl.f32 = txl.cuda.bfloat162float(
                                    txl.reinterpret("bfloat16", _bits)
                                )  # .cu:1460-1461
                                gate_arg: txl.f32 = gate_rate * (gate_raw + gate_bias)  # .cu:1462
                                _tanh = txl.local_scalar("float32")
                                txl.ptx.tanh.approx.f32(_tanh, gate_arg * txl.float32(0.5))
                                gate_sigmoid: txl.f32 = _tanh * txl.float32(0.5) + txl.float32(
                                    0.5
                                )  # .cu:1463-1465
                                txl.assign(
                                    gate_log2,
                        ...
                        txl.assign(prefix_log2, prefix_log2 + gate_log2)  # .cu:1469
                        txl.ptx.st.shared.f32(
                            smem_gate_all.ptr_to([stage_f32 + gate_row * 128 + gate_col]),
                            prefix_log2,
                        )  # .cu:1470
                _prep_bar_sync()  # .cu:1473-1485
                with txl.If(chunk_is_full_2 != 0), txl.Then():  # .cu:1486-1488
                    _mbarrier_wait(qk_raw_full, prep_stage, prep_phase.phase)
                with txl.If(prep_tid < 128), txl.Then():  # .cu:1489-1493
                    total_log2 = txl.local_scalar("float32")  # .cu:1490
                    txl.ptx.ld.shared.f32(
                        total_log2, smem_gt_prefix_all.ptr_to([stage_f32 + prep_tid])
                    )
                    _i64 = txl.local_scalar("float32")
                ...
                with txl.If(prep_tid == 0), txl.Then():  # .cu:1494-1497
                ...
                with txl.serial(
                    ...
                    gmem_base: txl.int64 = (
                        token_1 * txl.cast(h, "int64") + txl.cast(head_idx_3, "int64")
                    ) * 128 + txl.cast(segment_1 * 8, "int64")  # .cu:1505
                    qk_byte_off = segment_1 * 8 // 64 * 4096 + row_1 * 128 + segment_1 * 8 % 64 * 2
                    qk_swizzled_off = qk_byte_off ^ ((qk_byte_off >> 7 & 7) << 4)
                    q_raw_vec = txl.alloc_local((8,), "float32")  # .cu:1506
                    k_raw_vec = txl.alloc_local((8,), "float32")  # .cu:1507
                    with txl.unroll(8) as _zi:  # .cu:1508-1523
                        txl.assign(q_raw_vec[_zi], txl.float32(0.0))
                        txl.assign(k_raw_vec[_zi], txl.float32(0.0))
                    with txl.If(chunk_is_full_2 != 0):
                        with txl.Then():  # .cu:1524-1562
                            packed = txl.alloc_local((4,), "uint32", align=16)  # .cu:1525
                            txl.ptx.ld.shared.v4.b32(packed[0], packed[1], packed[2], packed[3], smem_raw.ptr_to([smem_q_raw_prefetch_addr + prep_stage_byte_base + qk_swizzled_off - txl.cast(smem, "int32")]))  # .cu:1526-1528  # fmt: skip
                            packed_fp32 = txl.alloc_local((8,), "float32")  # .cu:1529
                            with txl.unroll(4) as _pair:  # .cu:1530-1539
                                txl.assign(
                                    packed_fp32[_pair * 2],
                                    txl.cuda.uint_as_float(packed[_pair + 0] << txl.uint32(16)),
                                )
                                txl.assign(
                            ...
                            packed_0 = txl.alloc_local((4,), "uint32", align=16)  # .cu:1544
                            txl.ptx.ld.shared.v4.b32(packed_0[0], packed_0[1], packed_0[2], packed_0[3], smem_raw.ptr_to([smem_kd_addr + prep_stage_byte_base + qk_swizzled_off - txl.cast(smem, "int32")]))  # .cu:1545-1547  # fmt: skip
                            packed_0_fp32 = txl.alloc_local((8,), "float32")  # .cu:1548
                            with txl.unroll(4) as _pair:  # .cu:1549-1558
                                txl.assign(
                                    packed_0_fp32[_pair * 2],
                                    txl.cuda.uint_as_float(packed_0[_pair + 0] << txl.uint32(16)),
                                )
                                txl.assign(
                                    packed_0_fp32[_pair * 2 + 1],
                                    txl.cuda.uint_as_float(
                                        packed_0[_pair + 0] & txl.uint32(0xFFFF0000)
                                    ),
                                )
                            with txl.unroll(8) as value_idx_1:  # .cu:1559-1562
                                txl.assign(k_raw_vec[value_idx_1], packed_0_fp32[value_idx_1])
                        with txl.Else():
                            with txl.If(token_valid_1 != 0), txl.Then():  # .cu:1563-1602
                                with txl.unroll(1) as _blk:  # .cu:1564-1582
                                    _vldq = txl.alloc_local((4,), "uint32", align=16)
                                    txl.ptx.ld.global_.v4.b32(
                                        _vldq[0],
                                        _vldq[1],
                                        _vldq[2],
                                    ...
                                    with txl.unroll(4) as _pair:
                                        ...
                                        txl.assign(
                                            ...
                                            (
                                                txl.cuda.uint_as_float(
                                                    _vldq[_pair] & txl.uint32(0xFFFF0000)
                                                )
                                            ),
                                        )
                                with txl.unroll(1) as _blk:  # .cu:1583-1601
                                    _vldk = txl.alloc_local((4,), "uint32", align=16)
                                    txl.ptx.ld.global_.v4.b32(
                                        _vldk[0],
                                        _vldk[1],
                                        _vldk[2],
                                    ...
                                    with txl.unroll(4) as _pair:
                                        ...
                                        txl.assign(
                                            ...
                                            (
                                                txl.cuda.uint_as_float(
                                                    _vldk[_pair] & txl.uint32(0xFFFF0000)
                                                )
                                            ),
                                        )
                    q_sum = txl.local_scalar("float32")  # .cu:1603
                    k_sum = txl.local_scalar("float32")  # .cu:1604
                    txl.assign(q_sum, txl.float32(0.0))
                    txl.assign(k_sum, txl.float32(0.0))
                    with txl.serial(0, 8) as elem_in_segment:  # .cu:1605-1612
                        txl.ptx.fma.rn.f32(
                    ...
                    for _off in (8, 4, 2, 1):
                        for _acc in (q_sum, k_sum):
                            txl.assign(_acc, _acc + _shfl_bfly_f32(_acc, txl.int32(_off)))
                    q_inv = txl.local_scalar("float32")  # .cu:1629-1630
                    txl.ptx.rsqrt.approx.ftz.f32(q_inv, q_sum + txl.float32(1e-06))
                    k_inv = txl.local_scalar("float32")  # .cu:1631-1632
                    txl.ptx.rsqrt.approx.ftz.f32(k_inv, k_sum + txl.float32(1e-06))
                    with txl.unroll(4) as _ls:  # .cu:1633-1636
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(q_raw_vec[_ls * 2], q_raw_vec[_ls * 2 + 1]),
                            txl.cuda.make_float2(q_inv, q_inv),
                        )
                        txl.assign(q_raw_vec[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.assign(q_raw_vec[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    with txl.unroll(4) as _ls:  # .cu:1637-1640
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(k_raw_vec[_ls * 2], k_raw_vec[_ls * 2 + 1]),
                            txl.cuda.make_float2(k_inv, k_inv),
                        )
                        txl.assign(k_raw_vec[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.assign(k_raw_vec[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    qd_vec = txl.alloc_local((8,), "float32")  # .cu:1641
                    kd_vec = txl.alloc_local((8,), "float32")  # .cu:1642
                    ki_vec = txl.alloc_local((8,), "float32")  # .cu:1643
                    prefix_vec = txl.alloc_local((8,), "float32")
                    with txl.unroll(2) as prefix_vec_idx:
                        txl.ptx.ld.shared.v4.f32(
                    ...
                    with txl.serial(0, 8) as elem_in_segment_1:  # .cu:1644-1653
                        ...
                        txl.assign(qd_vec[elem_in_segment_1], decay)  # .cu:1650
                        txl.assign(kd_vec[elem_in_segment_1], decay)  # .cu:1651
                        txl.assign(
                            ki_vec[elem_in_segment_1],
                            txl.cuda.fdividef(k_raw_vec[elem_in_segment_1], decay),
                        )  # .cu:1652 (-use_fast_math: / -> div.approx.f32)
                    with txl.unroll(4) as _ls:  # .cu:1654-1656
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(qd_vec[_ls * 2], qd_vec[_ls * 2 + 1]),
                            txl.cuda.make_float2(q_raw_vec[_ls * 2], q_raw_vec[_ls * 2 + 1]),
                        )
                        txl.assign(qd_vec[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.assign(qd_vec[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    with txl.unroll(4) as _ls:  # .cu:1657-1660
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(qd_vec[_ls * 2], qd_vec[_ls * 2 + 1]),
                            txl.cuda.make_float2(txl.float32(scale), txl.float32(scale)),
                        )
                        txl.assign(qd_vec[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.assign(qd_vec[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    with txl.unroll(4) as _ls:  # .cu:1661-1663
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(kd_vec[_ls * 2], kd_vec[_ls * 2 + 1]),
                            txl.cuda.make_float2(k_raw_vec[_ls * 2], k_raw_vec[_ls * 2 + 1]),
                        )
                        txl.assign(kd_vec[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.assign(kd_vec[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    packed_1 = txl.alloc_local((4,), "uint32", align=4)  # .cu:1664
                    with txl.unroll(4) as _lp:  # .cu:1665-1669
                        txl.assign(
                            packed_1[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    with txl.unroll(4) as word:  # .cu:1670-1673
                        txl.ptx.st.shared.b32(smem_raw.ptr_to([smem_qd_addr + prep_stage_byte_base + qk_swizzled_off + word * 4 - txl.cast(smem, "int32")]), packed_1[word])  # fmt: skip
                    packed_0_1 = txl.alloc_local((4,), "uint32", align=4)  # .cu:1674
                    with txl.unroll(4) as _lp:  # .cu:1675-1679
                        txl.assign(
                            packed_0_1[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    with txl.unroll(4) as word_1:  # .cu:1680-1683
                        txl.ptx.st.shared.b32(smem_raw.ptr_to([smem_kd_addr + prep_stage_byte_base + qk_swizzled_off + word_1 * 4 - txl.cast(smem, "int32")]), packed_0_1[word_1])  # fmt: skip
                    packed_1_1 = txl.alloc_local((4,), "uint32", align=4)  # .cu:1684
                    with txl.unroll(4) as _lp:  # .cu:1685-1689
                        txl.assign(
                            packed_1_1[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    with txl.unroll(4) as word_2:  # .cu:1690-1693
                        txl.ptx.st.shared.b32(smem_raw.ptr_to([smem_ki_addr + prep_stage_byte_base + qk_swizzled_off + word_2 * 4 - txl.cast(smem, "int32")]), packed_1_1[word_2])  # fmt: skip
                _prep_bar_sync()  # .cu:1695-1707
                pair_row_base: txl.int32 = prep_local_warp // 2 * 16  # .cu:1708
                pair_col_base: txl.int32 = prep_local_warp % 2 * 16  # .cu:1709
                a_frag = txl.alloc_local((4,), "uint32", align=4)  # .cu:1710
                b_frag = txl.alloc_local((4,), "uint32", align=4)  # .cu:1711
                acc = txl.alloc_local((8,), "float32", align=4)  # .cu:1712
                with txl.If(pair_row_base >= pair_col_base):
                    with txl.Then():  # .cu:1713-1990
                        ...
                        def _chain8(a_base):
                            ...
                            for _s in range(8):
                                txl.ptx.ldmatrix.sync.aligned.m8n8.x4.shared.b16(
                                    ...
                                    a_frag[2],
                                    a_frag[3],
                                    smem_raw.ptr_to([(a_base + prep_stage_byte_base + (_a_cursors[_s]) * 16) - txl.cast(smem, "int32")]),
                                )  # fmt: skip
                                txl.ptx.ldmatrix.sync.aligned.m8n8.x4.shared.b16(
                                    b_frag[0],
                                    b_frag[1],
                                    b_frag[2],
                                    b_frag[3],
                                    smem_raw.ptr_to([(smem_ki_addr + prep_stage_byte_base + (_b_cursors[_s]) * 16) - txl.cast(smem, "int32")]),
                                )  # fmt: skip
                                if _s == 0:
                        ...
                        row0: txl.int32 = pair_row_base + lane // 4  # .cu:1826
                        row1: txl.int32 = row0 + 8  # .cu:1827
                        col0: txl.int32 = pair_col_base + lane % 4 * 2  # .cu:1828
                        beta0 = txl.local_scalar("float32")  # .cu:1829
                        txl.ptx.ld.shared.f32(beta0, smem_prep_beta_all.ptr_to([stage_f32 + row0]))
                        beta1 = txl.local_scalar("float32")  # .cu:1830
                        txl.ptx.ld.shared.f32(beta1, smem_prep_beta_all.ptr_to([stage_f32 + row1]))
                        seed = txl.alloc_local((8,), "float32")  # .cu:1831
                        with txl.unroll(8) as _zi:  # .cu:1832-1839
                            txl.ptx.mov.b32(seed[_zi], txl.float32(0.0))
                        ...
                        for _i, (_row, _dc, _beta) in enumerate(
                            ...
                            with txl.If(_row > (col0 if _dc == 0 else col0 + _dc)), txl.Then():
                                txl.assign(seed[_i], acc[_i] * _beta)
                        seed_packed = txl.alloc_local((4,), "uint32", align=4)  # .cu:1864
                        with txl.unroll(4) as _lp:  # .cu:1865-1869
                            txl.ptx.mov.b32(
                                seed_packed[_lp],
                                txl.cuda.float22bfloat162_rn(
                        ...
                        seed_lane_row: txl.int32 = lane % 16  # .cu:1870
                        seed_lane_col: txl.int32 = lane // 16 * 8  # .cu:1871
                        byte_off: txl.int32 = (pair_row_base + seed_lane_row) * 128 + (
                            pair_col_base + seed_lane_col
                        ) * 2  # .cu:1872
                        swizzled_off: txl.int32 = byte_off ^ ((byte_off >> 7 & 7) << 4)  # .cu:1873
                        seed_addr: txl.int32 = (
                        ...
                        txl.ptx.stmatrix.sync.aligned.m8n8.x4.shared.b16(
                            smem_raw.ptr_to([(seed_addr) - txl.cast(smem, "int32")]),
                            seed_packed[0],
                            seed_packed[1],
                            seed_packed[2],
                            seed_packed[3],
                    ...
                    with txl.Else():  # .cu:1991-2000
                        with txl.unroll(8) as _zi:
                            txl.ptx.mov.b32(acc[_zi], txl.float32(0.0))
                row0_1: txl.int32 = pair_row_base + lane // 4  # .cu:2001
                row1_1: txl.int32 = row0_1 + 8  # .cu:2002
                col0_1: txl.int32 = pair_col_base + lane % 4 * 2  # .cu:2003
                mqk = txl.alloc_local((8,), "float32")  # .cu:2004
                with txl.unroll(8) as _zi:  # .cu:2005-2012
                    txl.ptx.mov.b32(mqk[_zi], txl.float32(0.0))
                ...
                for _i, (_row, _dc) in enumerate(
                    ...
                    with txl.If(_row >= (col0_1 if _dc == 0 else col0_1 + _dc)), txl.Then():
                        txl.assign(mqk[_i], acc[_i])
                mqk_packed = txl.alloc_local((4,), "uint32", align=4)  # .cu:2037
                with txl.unroll(4) as _lp:  # .cu:2038-2042
                    txl.ptx.mov.b32(
                        mqk_packed[_lp],
                ...
                with txl.unroll(2) as publish_pair:  # .cu:2043-2051
                    ...
                    txl.ptx.stmatrix.sync.aligned.m8n8.x2.trans.shared.b16(
                        smem_raw.ptr_to([(_pub_addr) - txl.cast(smem, "int32")]),
                        mqk_packed[publish_pair * 2],
                        mqk_packed[publish_pair * 2 + 1],
                    )
                _prep_bar_sync()  # .cu:2052-2064
                with txl.If(prep_tid < 128), txl.Then():  # .cu:2065-2069
                    total_log2_1 = txl.local_scalar("float32")  # .cu:2066
                    txl.ptx.ld.shared.f32(
                        total_log2_1, smem_gt_prefix_all.ptr_to([stage_f32 + prep_tid])
                    )
                    _pk = txl.local_scalar("float32")
                    txl.ptx.ex2.approx.ftz.f32(_pk, total_log2_1)
                    txl.ptx.st.shared.f32(
                ...
                def _restore_block(n_passes, row_of):
                    ...
                    restore_scale = txl.local_scalar("float32")
                    txl.ptx.ld.shared.f32(
                        restore_scale, smem_restore_factor_all.ptr_to([stage_f32 + 128])
                    )
                    restore_factor = txl.alloc_local((8,), "float32")
                    restore_segment: txl.int32 = lane & 15
                    with txl.unroll(2) as restore_vec:
                        txl.ptx.ld.shared.v4.f32(
                    ...
                    with txl.serial(
                        ...
                        restore_qd_values = txl.alloc_local((8,), "float32")
                        restore_kd_values = txl.alloc_local((8,), "float32")
                        restore_ki_values = txl.alloc_local((8,), "float32")
                        for _dst, _src_addr in (
                            (restore_qd_values, smem_qd_addr),
                            (restore_kd_values, smem_kd_addr),
                            (restore_ki_values, smem_ki_addr),
                        ):
                            _packed = txl.alloc_local((4,), "uint32", align=16)
                            txl.ptx.ld.shared.v4.b32(_packed[0], _packed[1], _packed[2], _packed[3], smem_raw.ptr_to([_src_addr + prep_stage_byte_base + restore_swizzled_off - txl.cast(smem, "int32")]))  # fmt: skip
                            _packed_fp32 = txl.alloc_local((8,), "float32")
                            with txl.unroll(4) as _pair:
                                txl.ptx.mov.b32(
                                    _packed_fp32[_pair * 2],
                                    txl.cuda.uint_as_float(_packed[_pair + 0] << txl.uint32(16)),
                                )
                                txl.ptx.mov.b32(
                        ...
                        restore_kr_values = txl.alloc_local((8,), "float32")
                        with txl.unroll(8) as restore_elem:
                            txl.ptx.mov.b32(
                        ...
                        for _vals in (restore_qd_values, restore_kd_values):
                            with txl.unroll(4) as _ls:
                                _pk = txl.local_scalar("uint64")
                                txl.ptx.mul.rn.ftz.f32x2(
                                    _pk,
                                    txl.cuda.make_float2(_vals[_ls * 2], _vals[_ls * 2 + 1]),
                                    txl.cuda.make_float2(restore_scale, restore_scale),
                                )
                                txl.ptx.mov.b32(_vals[_ls * 2], txl.cuda.float2_x(_pk))
                                txl.ptx.mov.b32(_vals[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                        for _vals, _dst_addr in (
                            (restore_qd_values, smem_qd_addr),
                            (restore_kd_values, smem_kd_addr),
                            (restore_kr_values, smem_kr_trans_addr),
                        ):
                            _packed_out = txl.alloc_local((4,), "uint32", align=4)
                            with txl.unroll(4) as _lp:
                                txl.ptx.mov.b32(
                                    _packed_out[_lp],
                                    txl.cuda.float22bfloat162_rn(
                            ...
                            with txl.unroll(4) as _word:
                                txl.ptx.st.shared.b32(smem_raw.ptr_to([_dst_addr + prep_stage_byte_base + restore_swizzled_off + _word * 4 - txl.cast(smem, "int32")]), _packed_out[_word])  # fmt: skip

                with txl.If(prep_local_warp >= 2), txl.Then():  # .cu:2070-2187
                    _restore_block(
                        6, lambda _pass: 8 + (prep_local_warp - 2) * 12 + _pass * 2 + (lane >> 4)
                    )  # .cu:2082
                with txl.If(prep_local_warp == 0), txl.Then():  # .cu:2188-2250
                    inverse_row: txl.int32 = lane  # .cu:2189
                    diag_block: txl.int32 = inverse_row // 8  # .cu:2190
                    lane_in_diag: txl.int32 = lane & 7  # .cu:2191
                    ...
                    packed_fp32_2 = txl.alloc_local((8,), "float32")  # .cu:2199
                    with txl.unroll(4) as _pair:  # .cu:2200-2209
                        txl.ptx.mov.b32(
                            packed_fp32_2[_pair * 2],
                            txl.cuda.uint_as_float(packed_5[_pair + 0] << txl.uint32(16)),
                        )
                        txl.ptx.mov.b32(
                            packed_fp32_2[_pair * 2 + 1],
                            txl.cuda.uint_as_float(packed_5[_pair + 0] & txl.uint32(0xFFFF0000)),
                        )
                    with txl.unroll(8) as value_idx_5:  # .cu:2210-2213
                        txl.ptx.mov.b32(inv_row[value_idx_5], packed_fp32_2[value_idx_5])
                    with txl.unroll(8) as diag_elem:  # .cu:2214-2219
                        with txl.If(lane_in_diag == diag_elem), txl.Then():
                            txl.assign(inv_row[diag_elem], txl.float32(1.0))
                    diag_group_base: txl.int32 = lane - lane_in_diag  # .cu:2220
                    ...
                    for _p in range(7):
                        row_scale: txl.f32 = -inv_row[_p]  # .cu:2223
                        for _j in range(_p):
                            pivot_lane: txl.int32 = diag_group_base + _p  # .cu:2226
                            pivot = txl.local_scalar(
                                "float32", init=_shfl_idx_f32(inv_row[_j], pivot_lane)
                            )  # .cu:2227-2228
                            with txl.If(lane_in_diag > _p), txl.Then():  # .cu:2229-2232
                                txl.ptx.fma.rn.f32(
                                    inv_row[_j], row_scale, pivot, inv_row[_j]
                                )  # .cu:2230-2231
                        with txl.If(lane_in_diag > _p), txl.Then():  # .cu:2234-2236
                            txl.ptx.mov.b32(inv_row[_p], row_scale)
                    packed_0_3 = txl.alloc_local((4,), "uint32", align=4)  # .cu:2238
                    with txl.unroll(4) as _lp:  # .cu:2239-2243
                        txl.ptx.mov.b32(
                            packed_0_3[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    byte_off_1_1: txl.int32 = inverse_row * 128 + diag_block * 8 * 2  # .cu:2244
                    swizzled_off_2: txl.int32 = byte_off_1_1 ^ (
                        (byte_off_1_1 >> 7 & 7) << 4
                    )  # .cu:2245
                    with txl.unroll(4) as word_6:  # .cu:2246-2249
                        txl.ptx.st.shared.b32(smem_raw.ptr_to([smem_inv_work_addr + prep_stage_byte_base + swizzled_off_2 + word_6 * 4 - txl.cast(smem, "int32")]), packed_0_3[word_6])  # fmt: skip
                with txl.If(prep_local_warp < 2), txl.Then():  # .cu:2251-2256
                    with txl.If(txl.cuda.elect_sync()), txl.Then():
                        _mbarrier_arrive(prep_diag_ready, prep_stage)
                    _mbarrier_wait(prep_diag_ready, prep_stage, prep_phase.phase)
                with txl.If(prep_local_warp < 2), txl.Then():  # .cu:2257-2322
                    lane_row: txl.int32 = lane & 7  # .cu:2258
                    ...
                    d_frag = txl.alloc_local((2,), "uint32", align=4)  # .cu:2268
                    c_frag = txl.alloc_local((1,), "uint32", align=4)  # .cu:2269
                    dc_acc = txl.alloc_local((4,), "float32", align=4)  # .cu:2270
                    dc_bf16 = txl.alloc_local((2,), "uint32", align=4)  # .cu:2271
                    inv_a_frag = txl.alloc_local((1,), "uint32", align=4)  # .cu:2272
                    o_acc = txl.alloc_local((4,), "float32", align=4)  # .cu:2273
                    o_bf16 = txl.alloc_local((2,), "uint32", align=4)  # .cu:2274
                    # .cu:2275-2278 ldmatrix.x1 d_frag[0]
                    txl.ptx.ldmatrix.sync.aligned.m8n8.x1.shared.b16(
                        d_frag[0], smem_raw.ptr_to([(matrix_addr) - txl.cast(smem, "int32")])
                    )
                    # .cu:2279-2282 ldmatrix.x1 d_frag[1] (same address)
                    txl.ptx.ldmatrix.sync.aligned.m8n8.x1.shared.b16(
                        d_frag[1], smem_raw.ptr_to([(matrix_addr) - txl.cast(smem, "int32")])
                    )
                    # .cu:2283-2286 ldmatrix.x1.trans c_frag
                    txl.ptx.xor.b32(matrix_addr, matrix_addr, txl.int32(16))
                    txl.ptx.ldmatrix.sync.aligned.m8n8.x1.trans.shared.b16(
                        c_frag[0], smem_raw.ptr_to([(matrix_addr) - txl.cast(smem, "int32")])
                    )
                    _mma_m16n8k8_bf16_zero(dc_acc, d_frag, c_frag)  # .cu:2287-2289
                    with txl.unroll(2) as _ls:  # .cu:2290-2293
                        _pk = txl.local_scalar("uint64")
                        txl.ptx.mul.rn.ftz.f32x2(
                            _pk,
                            txl.cuda.make_float2(dc_acc[_ls * 2], dc_acc[_ls * 2 + 1]),
                            txl.cuda.make_float2(txl.float32(-1.0), txl.float32(-1.0)),
                        )
                        txl.ptx.mov.b32(dc_acc[_ls * 2], txl.cuda.float2_x(_pk))
                        txl.ptx.mov.b32(dc_acc[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                    with txl.unroll(2) as _lp:  # .cu:2294-2298
                        txl.ptx.mov.b32(
                            dc_bf16[_lp],
                            txl.cuda.float22bfloat162_rn(
                    ...
                    txl.ptx.ldmatrix.sync.aligned.m8n8.x1.trans.shared.b16(
                        inv_a_frag[0], smem_raw.ptr_to([(matrix_addr) - txl.cast(smem, "int32")])
                    )
                    ...
                    txl.ptx.stmatrix.sync.aligned.m8n8.x1.shared.b16(
                        smem_raw.ptr_to([(matrix_addr) - txl.cast(smem, "int32")]), o_bf16[0]
                    )
                    with txl.If(txl.cuda.elect_sync()), txl.Then():  # .cu:2318-2320
                        _mbarrier_arrive(prep_inv16_ready, prep_stage)
                    _mbarrier_wait(prep_inv16_ready, prep_stage, prep_phase.phase)  # .cu:2321
                with txl.If(prep_local_warp == 0):
                    with txl.Then():  # .cu:2323-2404
                        lane_row_1: txl.int32 = lane % 16  # .cu:2324
                        lane_col: txl.int32 = lane // 16 * 8  # .cu:2325
                        byte_off_3: txl.int32 = (16 + lane_row_1) * 128 + (
                            16 + lane_col
                        ) * 2  # .cu:2326
                        swizzled_off_4: txl.int32 = byte_off_3 ^ (
                            (byte_off_3 >> 7 & 7) << 4
                        )  # .cu:2327
                        d_addr_1: txl.int32 = (
                            smem_inv_work_addr + prep_stage_byte_base + swizzled_off_4
                        )  # .cu:2328
                        byte_off_0_1: txl.int32 = (16 + lane_row_1) * 128 + lane_col * 2  # .cu:2329
                        swizzled_off_1_2: txl.int32 = byte_off_0_1 ^ (
                            (byte_off_0_1 >> 7 & 7) << 4
                        )  # .cu:2330
                        c_addr_1: txl.int32 = (
                            smem_inv_work_addr + prep_stage_byte_base + swizzled_off_1_2
                        )  # .cu:2331
                        byte_off_2_2: txl.int32 = lane_row_1 * 128 + lane_col * 2  # .cu:2332
                        swizzled_off_3_2: txl.int32 = byte_off_2_2 ^ (
                            (byte_off_2_2 >> 7 & 7) << 4
                        )  # .cu:2333
                        a_addr_1: txl.int32 = (
                            smem_inv_work_addr + prep_stage_byte_base + swizzled_off_3_2
                        )  # .cu:2334
                        d32_frag = txl.alloc_local((4,), "uint32", align=4)  # .cu:2335
                        c32_frag = txl.alloc_local((4,), "uint32", align=4)  # .cu:2336
                        dc32_acc = txl.alloc_local((8,), "float32", align=4)  # .cu:2337
                        dc32_bf16 = txl.alloc_local((4,), "uint32", align=4)  # .cu:2338
                        a32_frag = txl.alloc_local((4,), "uint32", align=4)  # .cu:2339
                        o32_acc = txl.alloc_local((8,), "float32", align=4)  # .cu:2340
                        o32_bf16 = txl.alloc_local((4,), "uint32", align=4)  # .cu:2341
                        zero32_bf16 = txl.alloc_local((4,), "uint32", align=4)  # .cu:2342
                        ...
                        txl.ptx.ldmatrix.sync.aligned.m8n8.x4.shared.b16(
                            ...
                            d32_frag[2],
                            d32_frag[3],
                            smem_raw.ptr_to([(d_addr_1) - txl.cast(smem, "int32")]),
                        )
                        _dpb: txl.int32 = (
                            (16 + lane_col) // 16 * 1024
                            + (16 + lane_row_1) * 32
                        ...
                        txl.ptx.ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16(
                            ...
                            c32_frag[2],
                            c32_frag[3],
                            smem_raw.ptr_to([(c_addr_1) - txl.cast(smem, "int32")]),
                        )
                        ...
                        with txl.unroll(4) as _ls:  # .cu:2362-2365
                            _pk = txl.local_scalar("uint64")
                            txl.ptx.mul.rn.ftz.f32x2(
                                _pk,
                                txl.cuda.make_float2(dc32_acc[_ls * 2], dc32_acc[_ls * 2 + 1]),
                                txl.cuda.make_float2(txl.float32(-1.0), txl.float32(-1.0)),
                            )
                            txl.ptx.mov.b32(dc32_acc[_ls * 2], txl.cuda.float2_x(_pk))
                            txl.ptx.mov.b32(dc32_acc[_ls * 2 + 1], txl.cuda.float2_y(_pk))
                        with txl.unroll(4) as _lp:  # .cu:2366-2370
                            txl.ptx.mov.b32(
                                dc32_bf16[_lp],
                                txl.cuda.float22bfloat162_rn(
                        ...
                        txl.ptx.stmatrix.sync.aligned.m8n8.x4.trans.shared.b16(
                            smem_raw.ptr_to([(a_publish_addr) - txl.cast(smem, "int32")]),
                            a32_frag[0],
                            a32_frag[1],
                            a32_frag[2],
                            a32_frag[3],
                        ...
                        with txl.unroll(4) as _lp:  # .cu:2386-2390
                            txl.ptx.mov.b32(
                                o32_bf16[_lp],
                                txl.cuda.float22bfloat162_rn(
                        ...
                        o_publish_addr: txl.int32 = (
                            smem_inv_addr + prep_stage_byte_base + (_opb ^ ((_opb >> 7 & 1) << 4))
                        )  # .cu:2391
                        # .cu:2392-2395 stmatrix.x4 o32 publish
                        txl.ptx.stmatrix.sync.aligned.m8n8.x4.shared.b16(
                            smem_raw.ptr_to([(o_publish_addr) - txl.cast(smem, "int32")]),
                            o32_bf16[0],
                            o32_bf16[1],
                            o32_bf16[2],
                            o32_bf16[3],
                        )
                        with txl.unroll(4) as zero_word:  # .cu:2396-2399
                            txl.ptx.mov.b32(zero32_bf16[zero_word], txl.uint32(0))
                        _zpb: txl.int32 = (
                            (16 + lane_col) // 16 * 1024
                            + lane_row_1 * 32
                            + (16 + lane_col) % 16 * 2
                        )
                        zero_publish_addr: txl.int32 = (
                            smem_inv_addr + prep_stage_byte_base + (_zpb ^ ((_zpb >> 7 & 1) << 4))
                        )  # .cu:2400
                        # .cu:2401-2404 stmatrix.x4 zero publish
                        txl.ptx.stmatrix.sync.aligned.m8n8.x4.shared.b16(
                            smem_raw.ptr_to([(zero_publish_addr) - txl.cast(smem, "int32")]),
                            zero32_bf16[0],
                            zero32_bf16[1],
                            zero32_bf16[2],
                            zero32_bf16[3],
        ...
        with txl.If((warp >= 8) & (warp <= 11)), txl.Then():
            producer_regs.emit()
        with compute:
            compute_state = txl.PipelineState(5, phase=0)
            compute_role(compute_state)
        with epilogue:
            epilogue_state = txl.PipelineState(5, phase=0)
            output_state = txl.PipelineState(1, phase=0)
            epilogue_role(epilogue_state, output_state)
        with mma:
            mma_state = txl.PipelineState(5, phase=0)
            out_state = txl.PipelineState(1, phase=0)
            mma_role(mma_state, out_state)
        with load:
            load_state = txl.PipelineState(5, phase=0)
            load_role(load_state)
        with prep_role_owner:
            prep_phase = txl.PipelineState(1, phase=0)
