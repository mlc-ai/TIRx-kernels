...
def load_kernel_configs(kernel: str, config_dir: Path = CONFIG_DIR) -> list[dict]:
    ...
    """
    path = config_dir / f"{kernel}.yaml"
    if not path.exists():
        raise FileNotFoundError(f"no config file for kernel {kernel!r}: {path}")
...
def load_config_dir(config_dir: Path = CONFIG_DIR) -> list[dict]:
    ...
    regression gate covers is a per-line flag rather than a separate file.
    """
