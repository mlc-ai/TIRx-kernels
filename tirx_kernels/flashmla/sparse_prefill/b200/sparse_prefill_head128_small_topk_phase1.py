...
def make_kernel(
    ...
    def host_prelude(params):
        ...
        kv_tma = encode(
            txl.handle_add_byte_offset(kv.data, 0),
            2,
            D_QK,
            s_kv,
        ...
        out_tma = encode(txl.handle_add_byte_offset(out.data, 0), 4, *out_shape)
        out_tma_1 = encode(txl.handle_add_byte_offset(out.data, 0), 4, *out_shape)
        q_tma = encode(
    ...
    def sparse_flashmla_prefill_head128_small_topk_phase1_kernel(
        ...
        attn_sink: txl.gptr[txl.f32, (B_H,)],
        ...
        *,
        ...
        smem = txl.smem_pool()
        pool = smem.pool
        q_smem = smem.alloc((64, 512), "bfloat16", swizzle=txl.SW128B).buf
        k_smem = smem.alloc((256, 256), "bfloat16", swizzle=txl.SW128B).buf
        ...
        k_ready = txl.TMABar(pool, 4)
        k_empty = txl.TCGen05Bar(pool, 4)
        p_empty = txl.MBarrier(pool, 1)
        umma_ready = txl.MBarrier(pool, 1)
        softmax_ready = txl.MBarrier(pool, 1)
        so_full = txl.MBarrier(pool, 1)
        ...
        tq_consumed = txl.MBarrier(pool, 1)
        ...
        class CLCJobScheduler:
            ...
            def issue_cancel(self):
                with txl.If(cta_idx == 0), txl.Then():
                    txl.cuda.mbarrier_wait(
                        txl.address_of(clc_empty.buf[0]), txl.bitwise_xor(self.epoch.phase, 1)
                    )
                    txl.ptx[
                        "clusterlaunchcontrol.try_cancel.async.shared::cta.mbarrier::complete_tx::bytes.multicast::cluster::all.b128"
                    ](
                        txl.cuda.cvta_generic_to_shared(txl.address_of(clc_response[0])),
                        txl.cuda.cvta_generic_to_shared(txl.address_of(clc_response_ready.buf[0])),
                    )
            ...
            def advance(self):
                txl.cuda.mbarrier_wait(txl.address_of(clc_response_ready.buf[0]), self.epoch.phase)
                next_job = txl.local_scalar("uint32")
                txl.query_cancel_first_ctaid_x(next_job, txl.address_of(clc_response[0]))
                remote_empty = txl.local_scalar("uint64")
                txl.ptx["mapa.shared::cluster.u64"](
                ...
                with txl.If(next_job == txl.uint32(4294967295)):
                    ...
                    with txl.Else():
                        txl.assign(self.block_idx, txl.Cast("int32", next_job))
                self.epoch.advance()
                            ...
                            with txl.If(warp_idx == 3), txl.Then():
                                with txl.If(txl.cuda.elect_sync()), txl.Then():
                                    with txl.unroll(4) as init_stage:
                                        txl.ptx["mbarrier.init.shared.b64"](
                                            txl.cuda.cvta_generic_to_shared(
                                                txl.address_of(k_ready.buf[init_stage])
                                            ),
                                            txl.uint32(1),
                                        )
                                        txl.ptx["mbarrier.init.shared.b64"](
                                            txl.cuda.cvta_generic_to_shared(
                                                txl.address_of(k_empty.buf[init_stage])
                                            ),
                                            txl.uint32(1),
                                        )
                                    with txl.unroll(4) as init_stage:
        ...
        def store_output(
            ...
            tensor_map,
            s_q_idx,
            signal_q_consumed: txl.constexpr,
            launch_dependents: txl.constexpr,
        ):
            txl.cuda.mbarrier_wait(txl.address_of(li_full.buf[0]), output_epoch)
            output_scale = txl.local_scalar("float32")
            txl.ptx.ld.shared.f32(output_scale, rowwise_li_buf.ptr_to([idx_in_warpgroup % 64]))
            ...
            output_storage = txl.alloc_local((64,))
            bf16_storage = txl.alloc_local((32,), "uint32")
            q_smem_win = q_smem.view(64, 2, 256).view(2, 64, 256).view(128, 256)
            with txl.unroll(4) as epi_k:
                local_storage = output_storage
                txl.ptx["tcgen05.ld.sync.aligned.32x32b.x64.b32"](
                    *[local_storage[i] for i in range(64)],
                    txl.cuda.get_tmem_addr(txl.uint32(0), 0, epi_k * 64),
                )
                txl.ptx["tcgen05.wait::ld.sync.aligned"]()
                with txl.If(epi_k == 0), txl.Then():
                    txl.cuda.mbarrier_wait(txl.address_of(q_consumed.buf[0]), q_epoch)
                    txl.ptx["fence.proxy.async.shared::cta"]()
                    if signal_q_consumed:
                        with txl.If(txl.cuda.elect_sync()), txl.Then():
                            tq_consumed_remote = txl.local_scalar("uint32")
                            txl.ptx["mapa.shared::cluster.u32"](
                                tq_consumed_remote,
                                txl.cuda.cvta_generic_to_shared(txl.address_of(tq_consumed.buf[0])),
                                txl.uint32(0),
                            )
                            txl.ptx["mbarrier.arrive.shared::cluster.b64"](tq_consumed_remote)
                with txl.If(epi_k == 3), txl.Then():
                    output_empty = txl.local_scalar("uint32")
                    txl.ptx["mapa.shared::cluster.u32"](
                        output_empty,
                        txl.cuda.cvta_generic_to_shared(txl.address_of(t_out_empty.buf[0])),
                        txl.uint32(0),
                    )
                    txl.ptx["mbarrier.arrive.shared::cluster.b64"](output_empty)
                ...
                for f in range(8):
                    ...
                    s_ptr = txl.ptr_byte_offset(
                        txl.address_of(q_smem_win[0, 0]), s_off * BF16_BYTES, "bfloat16"
                    )
                    r_w: txl.int32 = f * 4
            ...
            with txl.If(warp_idx == 0), txl.Then():
                with txl.If(txl.cuda.elect_sync()), txl.Then():
                    txl.ptx["cp.async.bulk.tensor.4d.global.shared::cta.tile.bulk_group"](
                        txl.reinterpret(txl.handle().ty, txl.address_of(tensor_map)),
                        0,
                        cta_idx * 64,
                        0,
                        s_q_idx,
                        txl.cuda.cvta_generic_to_shared(q_smem.ptr_to([0, 0])),
                    )
        ...
        def q_load_output():
            ...
            with txl.While(jobs.valid != 0):
                ...
                with txl.If(warp_idx == 0), txl.Then():
                    with txl.If(txl.cuda.elect_sync()), txl.Then():
                        txl.ptx["cp.async.bulk.wait_group"](0)
                        buffer_17 = txl.local_scalar("uint64")
                        txl.ptx["mapa.u64"](buffer_17, txl.address_of(q_full.buf[0]), txl.uint32(0))
                        txl.ptx[
                        ...
                        with txl.If(cta_idx == 0), txl.Then():
                            ...
                            with txl.If(last_valid != 0), txl.Then():
                                txl.cuda.mbarrier_wait(
                                    txl.address_of(tq_consumed.buf[0]), previous_epoch
                                )
                            txl.ptx["mbarrier.arrive.expect_tx.shared.b64"](
                                txl.cuda.cvta_generic_to_shared(txl.address_of(q_full.buf[0])),
                                txl.uint32(131072),
                            )
                            txl.cuda.mbarrier_wait(txl.address_of(q_full.buf[0]), jobs.epoch.phase)
                ...
                with txl.If(last_valid != 0):
                    ...
                    with txl.Else():
                        ...
                        with txl.If(txl.cuda.elect_sync()), txl.Then():
                            buffer_13 = txl.local_scalar("uint32")
                            txl.ptx["mapa.shared::cluster.u32"](
                                buffer_13,
                                txl.cuda.cvta_generic_to_shared(txl.address_of(tq_consumed.buf[0])),
                                txl.uint32(0),
                            )
                            txl.ptx["mbarrier.arrive.shared::cluster.b64"](buffer_13)
        ...
        def kv_gather():
            ...
            with txl.If(txl.cuda.elect_sync()), txl.Then():
                jobs = CLCJobScheduler()
                k_pipe = txl.PipelineState(4, phase=0)
                with txl.While(jobs.valid != 0):
                    wg1_s_q_idx = jobs.block_idx // 2
                    wg1_topk_len = txl.local_scalar("int32", init=topk)
                    with txl.If(have_topk_length), txl.Then():
                        txl.ptx.ld.global_.s32(wg1_topk_len, topk_length.ptr_to([wg1_s_q_idx]))
                    wg1_num_k_blocks = txl.max((wg1_topk_len + 64 - 1) // 64, 1)
                        ...
                        with txl.unroll(4) as row_group:
                            with txl.unroll(4) as col_atom:
                                ...
                                txl.ptx[
                                    ...
                                    txl.cuda.cvta_generic_to_shared(
                                        txl.reinterpret(txl.handle().ty, buffer_16)
                                    ),
                                    txl.uint64(1508705875169116160),
                                )
                        k_pipe.advance()
                    jobs.advance()
            txl.cuda.iket.range_end(kv_gather_token[0])

        def producer_role(role: txl.constexpr):
            with txl.If(role == 0):
                with txl.Then():
                    ...
                    with txl.If(txl.cuda.elect_sync()), txl.Then():
                        ...
                        with txl.While(jobs.valid != 0):
                            ...
                            umma_topk_len = txl.local_scalar("int32", init=topk)
                            with txl.If(have_topk_length), txl.Then():
                            ...
                            with txl.serial(umma_num_k_blocks + 1, unroll=False) as k:
                                with txl.If(k < umma_num_k_blocks), txl.Then():
                                    txl.cuda.mbarrier_wait(
                                        txl.address_of(p_empty.buf[0]),
                                        txl.bitwise_xor(txl.bitwise_and(k_pipe.stage, 1), 1),
                                    )
                                    txl.ptx["mbarrier.arrive.expect_tx.shared.b64"](
                                        txl.cuda.cvta_generic_to_shared(
                                    ...
                                    txl.cuda.mbarrier_wait(
                                        txl.address_of(k_ready.buf[k_pipe.stage]), k_pipe.phase
                                    )
                                    txl.ptx["tcgen05.fence::after_thread_sync"]()
                                    qk_accumulate = txl.local_scalar("uint32", init=txl.uint32(0))
                                    descB_local = txl.local_scalar("uint64")
                                    txl.cuda.tcgen05.encode_matrix_descriptor(
                                        txl.address_of(descB_local),
                                        txl.address_of(k_smem_gemm[0, 0, 0]),
                                        512,
                                        64,
                                        3,
                                    )
                                    with txl.unroll(1) as mi:
                                        with txl.unroll(1) as ni:
                                            with txl.unroll(16) as ki:
                                                ...
                                                txl.ptx["tcgen05.mma.cta_group::2.kind::f16"](
                                                    txl.Cast("uint32", ni * 64 + 384),
                                                    txl.Cast("uint32", ki * 8 + 256),
                                                    descB_off,
                                                    txl.uint32(136316048),
                                                    txl.uint32(0),
                                                    txl.uint32(0),
                                                    txl.uint32(0),
                                                    txl.uint32(0),
                                    ...
                                    txl.ptx[
                                        ...
                                        txl.cuda.cvta_generic_to_shared(
                                            txl.address_of(umma_ready.buf[0])
                                        ),
                                        txl.Cast("uint16", 3),
                                    )
                                    with txl.If(k == umma_num_k_blocks - 1), txl.Then():
                                        txl.ptx[
                                            "tcgen05.commit.cta_group::2.mbarrier::arrive::one.shared::cluster.b64"
                                        ](
                                            txl.cuda.cvta_generic_to_shared(
                                                txl.address_of(tq_ready.buf[0])
                                            )
                                        )
                                with txl.If(k > 0), txl.Then():
                                    prev_k = k - 1
                                    prev_buf = (k_pipe.stage + 3) % 4
                                    prev_s_o_phase = txl.bitwise_and(prev_buf, 1)
                                    txl.cuda.mbarrier_wait(
                                        txl.address_of(so_full.buf[0]),
                                        txl.bitwise_xor(prev_s_o_phase, 0),
                                    )
                                    with txl.If(prev_k == 0), txl.Then():
                                        txl.cuda.mbarrier_wait(
                                        ...
                                        with txl.unroll(1) as mi:
                                            with txl.unroll(1) as ni:
                                                with txl.unroll(4) as ki:
                                                    ...
                                                    txl.ptx["tcgen05.mma.cta_group::2.kind::f16"](
                                                        txl.Cast("uint32", n_acc),
                                                        descA_off,
                                                        descB_off,
                                                        txl.uint32(138478736),
                                                        txl.uint32(0),
                                                        txl.uint32(0),
                                                        txl.uint32(0),
                                                        txl.uint32(0),
                                    ...
                                    txl.ptx[
                                        ...
                                        txl.cuda.cvta_generic_to_shared(
                                            txl.address_of(softmax_ready.buf[0])
                                        ),
                                        txl.Cast("uint16", 3),
                                    )
                                    txl.ptx[
                                        ...
                                        txl.cuda.cvta_generic_to_shared(
                                            txl.address_of(k_empty.buf[prev_buf])
                                        ),
                                        txl.Cast("uint16", 3),
                                    )
                                with txl.If(k != umma_num_k_blocks), txl.Then():
                ...
                with txl.Else():
                    with txl.If(role == 1):
                        with txl.Then():
                            ...
                            with txl.If(lane_idx < 8), txl.Then():
                                lane_indices = txl.alloc_local((8,), "int32")
                                jobs = CLCJobScheduler()
                                index_pipe = txl.PipelineState(4, phase=0)
                                with txl.While(jobs.valid != 0):
                                    valid_s_q_idx = jobs.block_idx // 2
                                    valid_topk_len = txl.local_scalar("int32", init=topk)
                                    with txl.If(have_topk_length), txl.Then():
                                        txl.ptx.ld.global_.s32(
                                            valid_topk_len, topk_length.ptr_to([valid_s_q_idx])
                                        )
                                    valid_num_k_blocks = txl.max((valid_topk_len + 64 - 1) // 64, 1)
                                    ...
                                    with txl.serial(valid_num_k_blocks, unroll=False) as k:
                                        ...
                                        mask = txl.Cast("int8", terms[0])
                                        txl.cuda.mbarrier_wait(
                                            txl.address_of(valid_empty.buf[index_pipe.stage]),
                                            txl.bitwise_xor(index_pipe.phase, 1),
                                        )
                                        txl.ptx.st.shared.b8(
                                            is_k_valid.ptr_to([index_pipe.stage, lane_idx]),
                                            txl.reinterpret("uint8", mask),
                                        )
                                        txl.ptx["mbarrier.arrive.shared.b64"](
                                            txl.cuda.cvta_generic_to_shared(
                                                txl.address_of(valid_full.buf[index_pipe.stage])
                                            ),
                                            txl.uint32(1),
                                        )
                                        index_pipe.advance()
        ...
        def softmax():
            ...
            with txl.While(jobs.valid != 0):
                ...
                wg3_topk_len = txl.local_scalar("int32", init=topk)
                with txl.If(have_topk_length), txl.Then():
                ...
                scale_pair = txl.local_scalar(
                    ...
                    init=txl.cuda.make_float2(
                        txl.float32(sm_scale_div_log2), txl.float32(sm_scale_div_log2)
                    ),
                )
                with txl.serial(wg3_num_k_blocks, unroll=False) as k:
                    txl.cuda.mbarrier_wait(
                        txl.address_of(valid_full.buf[index_pipe.stage]), index_pipe.phase
                    )
                    p = txl.alloc_local((32,), "uint32")
                    p_peer = txl.alloc_local((32,), "uint32")
                    txl.cuda.mbarrier_wait(
                        txl.address_of(umma_ready.buf[0]), txl.bitwise_and(index_pipe.stage, 1)
                    )
                    txl.ptx["tcgen05.fence::after_thread_sync"]()
                    with txl.If(local_warp_idx < 2):
                        with txl.Then():
                            local_storage = p
                            txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                                *[local_storage[i] for i in range(32)], txl.uint32(384)
                            )
                            local_storage_1 = p_peer
                            txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                                *[local_storage_1[i] for i in range(32)],
                                txl.cuda.get_tmem_addr(txl.uint32(384), 0, 32),
                            )
                        with txl.Else():
                    ...
                    txl.ptx["tcgen05.wait::ld.sync.aligned"]()
                    txl.ptx["tcgen05.fence::before_thread_sync"]()
                    buffer_17 = txl.local_scalar("uint32")
                    txl.ptx["mapa.shared::cluster.u32"](
                        buffer_17,
                        txl.cuda.cvta_generic_to_shared(txl.address_of(p_empty.buf[0])),
                        txl.uint32(0),
                    )
                    ...
                    with txl.unroll(32) as p_i:
                        ...
                        txl.ptx.mov.b32(
                            p[p_i],
                            txl.if_then_else(invalid_p_predicate, txl.uint32(4286578688), p[p_i]),
                        )
                    ...
                    with txl.unroll(8) as exchange_i:
                        ...
                        txl.ptx["st.shared.v4.u32"](
                            txl.cuda.cvta_generic_to_shared(
                                txl.address_of(
                                    p_exchange[txl.bitwise_xor(local_warp_idx, 2), exchange_offset]
                                )
                            ),
                    ...
                    txl.ptx["bar.sync"](
                        txl.Cast("uint32", 2 + txl.bitwise_and(local_warp_idx, 1)), txl.uint32(64)
                    )
                    with txl.unroll(8) as exchange_i:
                        exchange_offset: txl.int32 = exchange_i * 32 * 4 + lane_idx * 4
                        ...
                        txl.ptx["ld.shared.v4.u32"](
                            ...
                            p_exchange_tmp[3],
                            txl.cuda.cvta_generic_to_shared(
                                txl.address_of(p_exchange[local_warp_idx, exchange_offset])
                            ),
                        )
                    ...
                    txl.assign(cur_pi_max, cur_pi_max * txl.float32(sm_scale_div_log2))
                    ...
                    txl.ptx.vote_sync.any.pred(
                        should_scale_o, cur_pi_max - mi > txl.float32(6.0), txl.uint32(4294967295)
                    )
                    ...
                    txl.cuda.mbarrier_wait(
                        txl.address_of(softmax_ready.buf[0]),
                        txl.bitwise_xor(txl.bitwise_and(index_pipe.stage, 1), 1),
                    )
                    txl.ptx["fence.proxy.async.shared::cta"]()
                    ...
                    with txl.If(txl.bitwise_and(k > 0, should_scale_o != txl.uint32(0))), txl.Then():
                        ...
                        with txl.unroll(8) as chunk_idx:
                            txl.ptx["tcgen05.ld.sync.aligned.32x32b.x32.b32"](
                                *[o_rescale[i] for i in range(32)],
                                txl.cuda.get_tmem_addr(txl.uint32(0), 0, chunk_idx * 32),
                            )
                            txl.ptx["tcgen05.wait::ld.sync.aligned"]()
                            for f in range(16):
                        ...
                        txl.ptx["tcgen05.fence::before_thread_sync"]()
                    txl.ptx["fence.proxy.async.shared::cta"]()
                    buffer_20 = txl.local_scalar("uint32")
                    txl.ptx["mapa.shared::cluster.u32"](
                        buffer_20,
                        txl.cuda.cvta_generic_to_shared(txl.address_of(so_full.buf[0])),
                        txl.uint32(0),
                    )
                    txl.ptx["mbarrier.arrive.shared::cluster.b64"](buffer_20)
                    txl.ptx["mbarrier.arrive.shared.b64"](
                        txl.cuda.cvta_generic_to_shared(
                            txl.address_of(valid_empty.buf[index_pipe.stage])
                        ),
                    ...
                    index_pipe.advance()
                with txl.If(real_mi == txl.float32("-inf")), txl.Then():
                    txl.assign(li, txl.float32(0.0))
                    txl.assign(mi, txl.float32("-inf"))
                txl.cuda.mbarrier_wait(
                    txl.address_of(li_empty.buf[0]), txl.bitwise_xor(jobs.epoch.phase, 1)
                )
                txl.ptx.st.shared.f32(
                    rowwise_li_buf.ptr_to([txl.bitwise_xor(idx_in_warpgroup, 64)]), li
                )
                txl.ptx["bar.sync"](txl.uint32(1), txl.uint32(128))
                peer_li = txl.local_scalar("float32")
                txl.ptx.ld.shared.f32(peer_li, rowwise_li_buf.ptr_to([idx_in_warpgroup]))
                txl.assign(li, li + peer_li)
                with txl.If(idx_in_warpgroup < 64), txl.Then():
                    head_idx = cta_idx * 64 + idx_in_warpgroup
                    attn_sink_value = txl.local_scalar("float32", init=txl.float32(-float("inf")))
                    with txl.If(have_attn_sink), txl.Then():
                        txl.ptx.ld.global_.f32(attn_sink_value, attn_sink.ptr_to([head_idx]))
                    attn_sink_log2 = attn_sink_value * txl.float32(1.4426950408889634)
                    ...
                    txl.ptx.st.global_.f32(
                        ...
                        real_mi * txl.float32(0.69314718055994529),
                    )
                    txl.ptx.st.global_.f32(lse.ptr_to([wg3_s_q_idx, head_idx]), cur_lse)
                jobs.advance()
