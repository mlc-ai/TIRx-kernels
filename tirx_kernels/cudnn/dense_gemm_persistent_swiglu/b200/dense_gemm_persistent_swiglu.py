    ...
    def kernel(a, b, ab12, c, alpha, *, host):
        ...
        with epilogue_role:
            ...
            if acc_dtype == "float16":
                x_packed_acc = txl.alloc_local((16,), "uint32")
                gate_packed_acc = txl.alloc_local((16,), "uint32")
                rounded_denominators = txl.alloc_local((32,), "uint16")
            ...
            def load_accumulator_pair(subtile):
                ...
                if acc_dtype == "float32":
                    ...
                    if c_major == "m":
                        txl.ptx["tcgen05.ld.sync.aligned.16x128b.x4.pack::16b.b32"](
                        ...
                        txl.ptx["tcgen05.ld.sync.aligned.32x32b.x16.pack::16b.b32"](
                    ...
                    for index in range(16):
                        txl.idioms.cast_f16x2_to_f32x2(x_values, index, x_packed_acc[index])
                        txl.idioms.cast_f16x2_to_f32x2(gate_values, index, gate_packed_acc[index])

            def apply_swiglu():
                for index in range(32):
                    txl.assign(x_values[index], x_values[index] * alpha)
                ...
                if acc_dtype == "float16":
                    for index in range(epi_n):
                        txl.ptx.cvt.rn.f16.f32(rounded_denominators[index], gate_reciprocals[index])
                    for index in range(epi_n):
                        txl.ptx.cvt.f32.f16(gate_reciprocals[index], rounded_denominators[index])
                for index in range(epi_n):
            ...
            def pack_outputs():
                if ab12_dtype == "float32":
                    for index in range(32):
                        txl.assign(ab12_x_words[index], txl.reinterpret("uint32", x_values[index]))
                        txl.assign(
                            ab12_gate_words[index], txl.reinterpret("uint32", gate_values[index])
                        )
                else:
            ...
            with txl.While(work < cluster_work):
                ...
                with txl.While(subtile < epilogue_subtiles):
                    ...
                    ab12_stage0 = txl.bitwise_and(previous_subtiles + subtile, txl.int32(3))
                    ab12_stage1 = txl.bitwise_and(previous_subtiles + subtile + 1, txl.int32(3))
                    c_stage = txl.bitwise_and(previous_subtiles + subtile // 2, txl.int32(1))
