...
def _make_kernel(
    ...
    def kernel(a, b, sfa, sfb, c, ab12, amax, sfc, norm_const, alpha, *, host):
        ...
        with epilogue_role:
            ...
            def apply_swiglu():
                if vector_f32:
                    for index in range(0, epi_n, 2):
                        ...
                        txl.assign(
                            gate_reciprocals[index], txl.float32(1.0) + gate_reciprocals[index]
                        )
                        txl.assign(
                            gate_reciprocals[index + 1],
            ...
            def generate_sfc_for_pair(pair):
                ...
                reciprocal_limit = txl.float32(
                    1.0 / (448.0 if c_dtype == "float8_e4m3fn" else 128.0)
                )
                if vector_f32:
                ...
                for scale_pair in range(scale_count // 2):
                    if sf_dtype == "float8_e8m0fnu":
                        ...
                        txl.ptx.cvt.f32.bf16(
                            ...
                            txl.cast(
                                txl.shift_right(decoded_sf_pairs[scale_pair], txl.uint32(16)),
                                "uint16",
                            ),
                        )
                    else:
    ...
    kernel.__annotations__ = {
        ...
        "sfc": txl.gptr[
            txl.u8, (L * _ceil_div(M, 128) * _ceil_div(N // 2, 4 * sf_vec_size) * 512,)
        ],
