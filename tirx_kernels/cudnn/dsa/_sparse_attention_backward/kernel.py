...
_ELEM = {"bfloat16": txl.bf16, "float16": txl.f16}
...
def _log2_soft(x):
    ...
    """
    one = txl.local_scalar(txl.f32, init=txl.float32(1.0))
    num = txl.local_scalar(txl.f32)
    den = txl.local_scalar(txl.f32)
    txl.ptx["sub.f32"](num, x, one)
    txl.ptx["add.f32"](den, x, one)
    r = txl.local_scalar(txl.f32)
    txl.ptx["div.rn.f32"](r, num, den)
    r2 = txl.local_scalar(txl.f32)
    txl.ptx["mul.f32"](r2, r, r)
    acc = txl.local_scalar(txl.f32, init=txl.float32(_LOG2_SERIES[-1]))
    for c in reversed(_LOG2_SERIES[:-1]):
        cv = txl.local_scalar(txl.f32, init=txl.float32(c))
        txl.ptx["fma.rn.f32"](acc, acc, r2, cv)
    out = txl.local_scalar(txl.f32)
    txl.ptx["mul.f32"](out, acc, r)
    return out
...
def _gemm(d, a, b, idesc, accumulate, n_iter, unroll, pred=None):
    ...
    a_desc, a_it, a_u = a
    b_desc, b_it, b_u = b
    flag = txl.local_scalar("uint32")
    txl.assign(flag, txl.cast(accumulate, "uint32"))
    ...
    a_cur = txl.local_scalar("uint64")
    b_cur = txl.local_scalar("uint64")
    txl.assign(a_cur, a_desc)
    txl.assign(b_cur, b_desc)
    it = txl.local_scalar(txl.i32, init=txl.int32(0))
    with txl.While(it < txl.int32(n_iter)):
        ...
        txl.assign(a_cur, _desc_add16(a_cur, a_it))
        txl.assign(b_cur, _desc_add16(b_cur, b_it))
        txl.assign(it, it + txl.int32(1))
...
def _bwd_load(
    ...
    for desc in (desc_q, desc_do, desc_dq):
        txl.ptx.prefetch.tensormap(txl.address_of(desc))
    ...
    qdo_leader = txl.local_scalar("uint32", init=txl.cuda.elect_sync())
    with txl.If(qdo_leader != txl.uint32(0)), txl.Then():
...
def _gather_or_zero(smem, addr, kv, row, col, src_row, head_dim, live):
    ...
    if live is None:
        ...
        return
    # `txl.If(cond)` opens the frame; Then/Else nest inside it. The
    # `with txl.If(c), txl.Then():` shorthand is if-without-else only.
    with txl.If(live):
        with txl.Then():
            txl.ptx["cp.async.cg.shared.global"](
                smem.ptr_to([addr]), kv.ptr_to([src_row * head_dim + col]), 16, 16
        ...
        with txl.Else():
            txl.ptx["st.shared.v4.b32"](
                smem.ptr_to([addr]), txl.uint32(0), txl.uint32(0), txl.uint32(0), txl.uint32(0)
            )
...
def _bwd_load_kv(
    ...
    groups = head_dim // 64
    full_tiles = (topk % txl.int32(block)) == txl.int32(0)  # :1344

    lane0_rd = txl.local_scalar("uint32")
    txl.assign(lane0_rd, txl.cast(lane == txl.int32(0), "uint32"))

    def read_indices(tile_index):
        """This warp's 16 top-k indices, read by lane 0 and broadcast."""
        r = txl.alloc_local([16], "int32")
        for i in range(rows_per_warp):
            row = i * 4 + lwarp
            gidx = tile_index * txl.int32(block) + txl.int32(row)
            v = txl.local_scalar(txl.i32, init=txl.int32(-1))
            ...
            if max_topk % block:
                issue = (lane == 0) & (gidx < txl.int32(max_topk))
            else:
                issue = lane0_rd
            txl.ptx["ld.global.b32"](v, topk_idxs.ptr_to([token * max_topk + gidx]), pred=issue)
            bc = txl.local_scalar(txl.i32)
            txl.ptx.shfl_sync.idx.b32(bc, v, txl.uint32(0), txl.uint32(31), txl.uint32(0xFFFFFFFF))
            txl.assign(r[i], bc)
        return r
    ...
    def gather_rows(tile_index, r, is_first):
        ...
        for i in range(rows_per_warp):
            ...
            if has_topk_length:
                ...
                live = (gidx < topk) & (src_row >= txl.int32(0))
            ...
            for j in range(2):
                g = lane // txl.int32(8) + txl.int32(j * 4)
                slot = (lane % txl.int32(8)) * txl.int32(8)
                col = g * txl.int32(64) + slot
                _gather_or_zero(
    ...
    def publish():
        ...
        txl.ptx["cp.async.commit_group"]()
        # Waits for ALL groups: the gather is not double-buffered.
        txl.ptx["cp.async.wait_group"](txl.int32(0))
        ...
        txl.ptx.bar.sync(txl.uint32(BAR_LOAD_KV_SYNC[0]), txl.uint32(BAR_LOAD_KV_SYNC[1]))
        txl.ptx["mbarrier.arrive.shared.b64"](p_k.full.buf.ptr_to([0]), txl.uint32(1))

    st = txl.PipelineState(1, phase=0)
    tile_index = txl.local_scalar(txl.i32, init=tile_count - txl.int32(1))
    ...
    with txl.While(tile_index >= txl.int32(0)):
...
def make_bwd_kernel(*, head_dim, num_head, dtype, max_topk, has_topk_length):
    ...
    def bwd(
        ...
        desc_dq: txl.TensorMap,
        kv: txl.gptr[elem],
        dq: txl.gptr[elem],
        ws_dkv: txl.gptr[txl.f32],
        topk_idxs: txl.gptr[txl.i32],
        ...
        if has_topk_length:
            topk = txl.local_scalar(txl.i32)
            txl.ptx["ld.global.b32"](topk, topk_length.ptr_to([token]))
        else:
            txl.keep_alive(topk_length.data)
            topk = txl.local_scalar(txl.i32, init=txl.int32(max_topk))
        ...
        with txl.If(topk <= txl.int32(0)), txl.Then():
            i = txl.local_scalar(txl.i32, init=tid)
            with txl.While(i < txl.int32(head_dim * block)):
                head_offset = i // txl.int32(head_dim)
                dim_idx = i % txl.int32(head_dim)
                head_idx = head_block * block + head_offset
                with txl.If(head_idx < txl.int32(num_head)), txl.Then():
                    zero = txl.local_scalar("uint16", init=txl.uint16(0))
                    txl.ptx["st.global.b16"](
                        dq.ptr_to([token * (num_head * head_dim) + head_idx * head_dim + dim_idx]),
                        zero,
                    )
                txl.assign(i, i + txl.int32(BWD_THREADS))
        ...
        with txl.If(topk > txl.int32(0)), txl.Then():
            txl.keep_alive(ws_dkv.data)
            tile_count = (topk + txl.int32(block - 1)) // txl.int32(block)
            ...
            p_qdo = txl.Pipeline(pool, 1, full="tma", empty="tcgen05", init_full=1, init_empty=1)
            p_k = txl.Pipeline(pool, 1, full="mbar", empty="tcgen05", init_full=128, init_empty=1)
            p_lse = txl.Pipeline(pool, 1, full="mbar", empty="mbar", init_full=32, init_empty=128)
            p_sum = txl.Pipeline(pool, 1, full="mbar", empty="mbar", init_full=32, init_empty=128)
            p_s = txl.Pipeline(pool, 1, full="tcgen05", empty="mbar", init_full=1, init_empty=128)
            p_dp = txl.Pipeline(pool, 1, full="tcgen05", empty="mbar", init_full=1, init_empty=128)
            p_dq = txl.Pipeline(pool, 1, full="tcgen05", empty="mbar", init_full=1, init_empty=128)
            p_p = txl.Pipeline(pool, 1, full="mbar", empty="tcgen05", init_full=128, init_empty=1)
            p_ds = txl.Pipeline(pool, 1, full="mbar", empty="tcgen05", init_full=128, init_empty=1)
            p_dkv = txl.Pipeline(pool, 2, full="tcgen05", empty="mbar", init_full=1, init_empty=256)

            tmem_hold = pool.alloc((1,), txl.i32, align=4)
            txl.ptx["fence.mbarrier_init.release.cluster"]()
            txl.ptx.bar.sync(txl.uint32(0), txl.uint32(BWD_THREADS))
            ...
            with r_mma:
                ...
                txl.ptx.bar.sync(txl.uint32(BAR_TMEM_ALLOC[0]), txl.uint32(BAR_TMEM_ALLOC[1]))
                tcol = txl.local_scalar("uint32")
                txl.ptx.ld.shared.b32(tcol, tmem_hold.ptr_to([0]))
                t_s = txl.cuda.get_tmem_addr(tcol, txl.uint32(0), txl.uint32(TMEM_S_OFFSET))
                # dP shares S's columns at a different lane range (:2675-2677).
                t_dp = txl.cuda.get_tmem_addr(tcol, txl.uint32(16), txl.uint32(TMEM_DP_OFFSET))

                t_dkv = [
                    txl.cuda.get_tmem_addr(tcol, txl.uint32(0), txl.uint32(off))
                    for off in (TMEM_DKV0_OFFSET, TMEM_DKV1_OFFSET)
                ]
                t_dkv4 = txl.cuda.get_tmem_addr(tcol, txl.uint32(0), txl.uint32(TMEM_DKV4_OFFSET))
                t_dkv23 = [
                    txl.cuda.get_tmem_addr(tcol, txl.uint32(0), txl.uint32(off))
                ...
                t_dq4 = txl.cuda.get_tmem_addr(tcol, txl.uint32(0), txl.uint32(TMEM_DQ4_OFFSET))
                ...
                d_q_k = _desc_at(D_WIDE, smem_base + txl.uint32(OFF_Q))
                d_k_k = _desc_at(D_WIDE, smem_base + txl.uint32(OFF_K))
                d_do_k = _desc_at(D_WIDE, smem_base + txl.uint32(OFF_DO))
                d_p_k = _desc_at(D_NARROW_K, smem_base + txl.uint32(OFF_P))
                d_ds_k = _desc_at(D_NARROW_K, smem_base + txl.uint32(OFF_DS))
                d_ds_mn = _desc_at(D_WIDE, smem_base + txl.uint32(OFF_DS))
                ...
                d_do_mn = [
                    _desc_at(D_WIDE, smem_base + txl.uint32(OFF_DO + b * 16384)) for b in range(4)
                ]
                d_q_mn = [
                    _desc_at(D_WIDE, smem_base + txl.uint32(OFF_Q + b * 16384)) for b in range(5)
                ]
                d_k_mn = [
                    _desc_at(D_WIDE, smem_base + txl.uint32(OFF_K + b * 16384)) for b in range(5)
                ]
                ...
                lane0 = txl.local_scalar("uint32", init=txl.cuda.elect_sync())

                st_k = txl.PipelineState(1, phase=0)
                st_s = txl.PipelineState(1, phase=0)
                st_dp = txl.PipelineState(1, phase=0)
                st_p = txl.PipelineState(1, phase=0)
                st_ds_m = txl.PipelineState(1, phase=0)
                st_dkv = txl.PipelineState(2, phase=0)
                # `accumulate` for dQ: 0 on the first processed tile, 1 after.
                acc_dq = txl.local_scalar(txl.i32, init=0)
                ...
                idx = txl.local_scalar(txl.i32, init=tile_count - txl.int32(1))
                with txl.While(idx >= txl.int32(0)):
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_s.full.buf.ptr_to([0]), pred=lane0)
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_dp.full.buf.ptr_to([0]), pred=lane0)
                    ...
                    with txl.If(idx != tile_count - txl.int32(1)), txl.Then():
                        ...
                        txl.ptx.bar.sync(txl.uint32(war_gen_a[0]), txl.uint32(war_gen_a[1]))
                    for j in range(2):
                        _gemm(
                            t_dkv[j],
                            (d_do_mn[j], *(256, 128)),
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_dkv.full.buf.ptr_to([st_dkv.stage]), pred=lane0)
                    st_dkv.advance()
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_k.empty.buf.ptr_to([0]), pred=lane0)

                    # --- dKV generation B: dKV2, dKV3 (:1689-1746) ----------
                    # Unconditional, unlike generation A's WAR barrier.
                    txl.ptx.bar.sync(txl.uint32(war_gen_b[0]), txl.uint32(war_gen_b[1]))
                    p_dkv.empty.wait(st_dkv.stage, st_dkv.phase ^ 1)
                    for j in range(2):
                        _gemm(
                            t_dkv23[j],
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_p.empty.buf.ptr_to([0]), pred=lane0)
                    for j in range(2):
                        _gemm(
                            t_dkv23[j],
                            (d_q_mn[2 + j], *(256, 128)),
                    ...
                    txl.ptx[TCGEN05_COMMIT](p_dkv.full.buf.ptr_to([st_dkv.stage]), pred=lane0)
                    txl.ptx[TCGEN05_COMMIT](p_ds.empty.buf.ptr_to([0]), pred=lane0)
                    st_dkv.advance()

                    txl.assign(acc_dq, txl.int32(1))
                    st_k.advance()
                    st_s.advance()
                    st_dp.advance()
                    st_p.advance()
                    st_ds_m.advance()
                    txl.assign(idx, idx - txl.int32(1))
                ...
                txl.ptx.bar.sync(txl.uint32(war_gen_a[0]), txl.uint32(war_gen_a[1]))
                txl.ptx[TCGEN05_COMMIT](p_dq.full.buf.ptr_to([0]), pred=lane0)
                txl.ptx[TCGEN05_COMMIT](p_qdo.empty.buf.ptr_to([0]), pred=lane0)

            with r_compute:
                rwarp = txl.warp_id_in_role()
                rtid = txl.tid_in_role()
                with txl.If(rwarp == txl.int32(0)), txl.Then():
                    txl.ptx["tcgen05.alloc.cta_group::1.sync.aligned.shared::cta.b32"](
                        txl.cuda.cvta_generic_to_shared(tmem_hold.ptr_to([0])),
                        txl.uint32(TMEM_ALLOC_COLUMNS),
                    )
                txl.ptx.bar.sync(txl.uint32(BAR_TMEM_ALLOC[0]), txl.uint32(BAR_TMEM_ALLOC[1]))
                tcol = txl.local_scalar("uint32")
                txl.ptx.ld.shared.b32(tcol, tmem_hold.ptr_to([0]))
                ...
                gsub = rtid // txl.int32(4)
                base_row = (gsub // txl.int32(8)) * txl.int32(16) + gsub % txl.int32(8)

                p_lse.full.wait(0, 0)
                p_sum.full.wait(0, 0)
                lse_lo = txl.local_scalar(txl.f32)
                lse_hi = txl.local_scalar(txl.f32)
                txl.ptx["ld.shared.b32"](lse_lo, smem.ptr_to([OFF_LSE + base_row * txl.int32(4)]))
                txl.ptx["ld.shared.b32"](
                    lse_hi, smem.ptr_to([OFF_LSE + (base_row + txl.int32(8)) * txl.int32(4)])
                )
                sum_lo = txl.local_scalar(txl.f32)
                sum_hi = txl.local_scalar(txl.f32)
                txl.ptx["ld.shared.b32"](sum_lo, smem.ptr_to([OFF_SUM + base_row * txl.int32(4)]))
                txl.ptx["ld.shared.b32"](
                    sum_hi, smem.ptr_to([OFF_SUM + (base_row + txl.int32(8)) * txl.int32(4)])
                )

                scale_log2e = txl.local_scalar(txl.f32)
                txl.ptx["mul.f32"](scale_log2e, scale, txl.float32(1.4426950408889634))

                st_s = txl.PipelineState(1, phase=0)
                st_dp = txl.PipelineState(1, phase=0)
                st_p = txl.PipelineState(1, phase=0)
                st_ds = txl.PipelineState(1, phase=0)
                idx = txl.local_scalar(txl.i32, init=tile_count - txl.int32(1))
                with txl.While(idx >= txl.int32(0)):
                    p_s.full.wait(0, st_s.phase)
                    p_p.empty.wait(0, st_p.phase ^ 1)
                    rs = txl.alloc_local([32], "float32")
                    ...
                    txl.ptx.mov.b64(scale2, scale_log2e, scale_log2e)
                    lse2_lo = txl.local_scalar("uint64")
                    lse2_hi = txl.local_scalar("uint64")
                    txl.ptx.mov.b64(lse2_lo, lse_lo, lse_lo)
                    txl.ptx.mov.b64(lse2_hi, lse_hi, lse_hi)
                    for i in range(0, 32, 2):
                        packed = txl.local_scalar("uint64")
                        txl.ptx.mov.b64(packed, rs[i], rs[i + 1])
                        addend = lse2_lo if (i % 4) < 2 else lse2_hi
                        txl.ptx["fma.rn.f32x2"](packed, packed, scale2, addend)
                        txl.ptx.mov.b64(rs[i], rs[i + 1], packed)
                        txl.ptx["ex2.approx.ftz.f32"](rs[i], rs[i])
                        txl.ptx["ex2.approx.ftz.f32"](rs[i + 1], rs[i + 1])
                        ...
                        txl.ptx["stmatrix.sync.aligned.m8n8.x4.trans.shared.b16"](
                            smem.ptr_to(
                                [
                                    _tile_byte(
                                        OFF_P,
                                        txl.int32(16 * b) + rtid % txl.int32(16),
                                        (rtid // txl.int32(32)) * txl.int32(16)
                                        + (rtid % txl.int32(32)) // txl.int32(16) * txl.int32(8),
                                    )
                                ]
                            ),
                    ...
                    rdp = txl.alloc_local([32], "float32")
                    txl.ptx["tcgen05.ld.sync.aligned.16x256b.x8.b32"](
                    ...
                    # rs still holds P.
                    sum2_lo = txl.local_scalar("uint64")
                    sum2_hi = txl.local_scalar("uint64")
                    txl.ptx.mov.b64(sum2_lo, sum_lo, sum_lo)
                    txl.ptx.mov.b64(sum2_hi, sum_hi, sum_hi)
                    scl2 = txl.local_scalar("uint64")
                    txl.ptx.mov.b64(scl2, scale, scale)
                    for i in range(0, 32, 2):
                        packed = txl.local_scalar("uint64")
                        txl.ptx.mov.b64(packed, rdp[i], rdp[i + 1])
                        addend = sum2_lo if (i % 4) < 2 else sum2_hi
                        txl.ptx["add.rn.f32x2"](packed, packed, addend)
                        pv = txl.local_scalar("uint64")
                        txl.ptx.mov.b64(pv, rs[i], rs[i + 1])
                        txl.ptx["mul.rn.f32x2"](packed, packed, pv)
                        # The `* softmax_scale` is folded into the quantize
                        # upstream (:1938); it is one packed multiply either way.
                        txl.ptx["mul.f32x2"](packed, packed, scl2)
                        txl.ptx.mov.b64(rdp[i], rdp[i + 1], packed)

                    # dS follows P through the identical transposing store; it
                    # is the dKV MMA's A operand.
                    rq = txl.alloc_local([16], "uint32")
                    for i in range(16):
                        txl.ptx[_CVT_PACK[dtype]](rq[i], rdp[2 * i + 1], rdp[2 * i])
                    txl.ptx["tcgen05.wait::ld.sync.aligned"]()
                ...
                for i in range(4):
                    ...
                    # heads per issue.
                    rdq = txl.alloc_local([64], "float32")
                    for e in range(8):
                        a = txl.cuda.get_tmem_addr(
                            t_dq_e[i][0], txl.int32(0), txl.int32(t_dq_e[i][1] + 8 * e)
                        )
                    ...
                    for j in range(64):
                        ...
                        txl.ptx[_CVT_NARROW[dtype]](qv, rdq[j])
                        txl.ptx["st.shared.b16"](
                            smem.ptr_to([_tile_byte(OFF_DQ, txl.int32(j), rtid)]), qv
                        )
                    ...
                    txl.ptx["fence.proxy.async.shared::cta"]()
                    txl.ptx.bar.sync(txl.uint32(BAR_COMPUTE_SYNC[0]), txl.uint32(BAR_COMPUTE_SYNC[1]))
                    with txl.If(txl.And(rwarp == txl.int32(0), lane == 0)), txl.Then():
                        for h in range(2):
                            txl.ptx[TMA_S2G](
                                txl.address_of(desc_dq),
                                txl.int32(i * 128 + h * 64),
                                head_block * txl.int32(block),
                                token,
                ...
                if tail:
                    ...
                    for e in range(4):
                        a4 = txl.cuda.get_tmem_addr(
                            tcol, txl.int32(0), txl.int32(TMEM_DQ4_OFFSET + 16 * e)
                        )
                    ...
                    for j in range(32):
                        qv = txl.local_scalar("uint16")
                        txl.ptx[_CVT_NARROW[dtype]](qv, rq4[j])
                        ...
                        txl.ptx["st.shared.b16"](
                            smem.ptr_to(
                                [
                                    _tile_byte(
                                        OFF_DQ4,
                                        head_b + txl.int32((j % 2) + (j // 4) * 8),
                                        dim_b + txl.int32(((j // 2) % 2) * 8),
                                    )
                                ]
                            ),
                            qv,
            ...
            with r_reduce:
                txl.ptx.bar.sync(txl.uint32(BAR_TMEM_ALLOC[0]), txl.uint32(BAR_TMEM_ALLOC[1]))
                rtcol = txl.local_scalar("uint32")
                txl.ptx.ld.shared.b32(rtcol, tmem_hold.ptr_to([0]))
                rtid = txl.tid_in_role()
                wg = rtid // txl.int32(128)  # two warpgroups split the 64 KV rows
                wtid = rtid % txl.int32(128)
                ...
                def t2r(col):
                    ...
                    for e in range(2):
                        a = txl.cuda.get_tmem_addr(
                            rtcol, txl.int32(16 * e), txl.int32(col) + wg * txl.int32(32)
                        )
                ...
                col_group = (wtid // txl.int32(4)) * txl.int32(4)
                row_base = wg * txl.int32(32) + (wtid % txl.int32(4)) * txl.int32(2)

                full_tiles = (topk % txl.int32(block)) == txl.int32(0)  # :2194
                        ...
                        with txl.If(full_tiles):
                            with txl.Then():
                                txl.ptx["ld.global.b32"](
                                    r[i], topk_idxs.ptr_to([token * txl.int32(max_topk) + g])
                                )
                            with txl.Else():
                                with txl.If(g < topk), txl.Then():
                                    txl.ptx["ld.global.b32"](
                                        r[i], topk_idxs.ptr_to([token * txl.int32(max_topk) + g])
                                    )
                    return r
                ...
                sink4 = txl.alloc_local([4], "float32")
                ...
                def reduce4(regs, r, sub_tile):
                    ...
                    for i in range(8):
                        b = i * 2 - i % 2
                        if True:
                            txl.ptx["atom.global.add.v4.f32"](
                                sink4[0],
                                sink4[1],
                                sink4[2],
                                sink4[3],
                ...
                st_r = txl.PipelineState(2, phase=0)
                ridx = txl.local_scalar(txl.i32, init=tile_count - txl.int32(1))
                with txl.While(ridx >= txl.int32(0)):
                    r = rows_for(ridx)
                    ...
                    if tail:
                        ...
                        txl.ptx["tcgen05.wait::ld.sync.aligned"]()
                        txl.ptx.bar.sync(
                            txl.uint32(BAR_T2R_DKV4_DONE[0]), txl.uint32(BAR_T2R_DKV4_DONE[1])
                        )
                        # The tail is 64 dims wide, so a 2-wide atomic.
                        for i in range(8):
                            b = i * 2 - i % 2
                            if True:
                                txl.ptx["atom.global.add.v2.f32"](
                                    sink4[0],
                                    sink4[1],
                                    ws_dkv.ptr_to(
                                        [
...
def _butterfly_sum_f32(value, lane_xors):
    ...
    membermask = txl.local_scalar(txl.u32, init=txl.tvm_warp_activemask())
    for lane_xor in lane_xors:
        peer = _shfl_bfly_f32(value, lane_xor, membermask)
        total = txl.local_scalar(txl.f32)
...
def make_sum_odo_kernel(*, head_dim, num_head, dtype, max_topk):
    ...
    def sum_odo(
        ...
        log2_e = txl.local_scalar(txl.f32)
        txl.ptx.neg.f32(log2_e, lse_scale)  # lse_scale is -log2(e) (:489)

        for arm in range(q_arms):
            idx_q_t = tidy + arm * SUM_ODO_THREADS_Q
            idx_q = idx_q_t + block_q * q_block
            with txl.If((idx_q_t < txl.int32(block_q)) & (idx_q < seqlen_q)), txl.Then():
                acc = txl.local_scalar(txl.f32, init=txl.float32(0.0))
                ...
                for i in range(d_steps):
                    ...
                    ow = txl.alloc_local([2], "uint32")
                    dw = txl.alloc_local([2], "uint32")
                    txl.ptx["ld.global.v2.b32"](ow[0], ow[1], out.ptr_to([base_o + d]))
                    txl.ptx["ld.global.v2.b32"](dw[0], dw[1], dout.ptr_to([base_o + d]))
                    narrow = []
                    for half in range(2):
                        prod = txl.local_scalar(txl.u32)
                        txl.ptx[mul_half](prod, ow[half], dw[half])
                    ...
                    frag = txl.local_scalar(txl.f32)
                    txl.ptx[cvt_wide](frag, narrow[0])
                    for nv in narrow[1:]:
                        # `add.rn.f32.<narrow>` takes the narrow value first.
                        txl.ptx[add_wide](frag, nv, frag)
                    txl.ptx["add.f32"](acc, acc, frag)

                total = _butterfly_sum_f32(acc, (1, 2, 4))

                with txl.If(tidx == txl.int32(0)), txl.Then():
                    lse_v = txl.local_scalar(txl.f32)
                    sink_v = txl.local_scalar(txl.f32)
                    txl.ptx["ld.global.b32"](lse_v, lse.ptr_to([idx_q * num_head + head]))
                    txl.ptx["ld.global.b32"](sink_v, attn_sink.ptr_to([head]))

                    lse_log2 = txl.local_scalar(txl.f32)
                    sink_log2 = txl.local_scalar(txl.f32)
                    txl.ptx["mul.f32"](lse_log2, lse_v, log2_e)
                    txl.ptx["mul.f32"](sink_log2, sink_v, log2_e)
                    m = txl.local_scalar(txl.f32)
                    txl.ptx["max.f32"](m, lse_log2, sink_log2)
                    # exp2 of each shifted term, summed, then log2: the
                    # numerically safe fold of the sink into the denominator.
                    a = txl.local_scalar(txl.f32)
                    b = txl.local_scalar(txl.f32)
                    txl.ptx["sub.f32"](a, lse_log2, m)
                    txl.ptx["sub.f32"](b, sink_log2, m)
                    ea = txl.local_scalar(txl.f32)
                    eb = txl.local_scalar(txl.f32)
                    txl.ptx["ex2.approx.f32"](ea, a)
                    txl.ptx["ex2.approx.f32"](eb, b)
                    ssum = txl.local_scalar(txl.f32)
                    txl.ptx["add.f32"](ssum, ea, eb)
                    lg = _log2_soft(ssum)
                    with_sink = txl.local_scalar(txl.f32)
                    txl.ptx["add.f32"](with_sink, m, lg)
                    scaled = txl.local_scalar(txl.f32)
                    txl.ptx["neg.f32"](scaled, with_sink)
                    ...
                    txl.ptx["st.global.b32"](ws.ptr_to([slot]), scaled_odo)
                    txl.ptx["st.global.b32"](ws.ptr_to([plane + slot]), out_lse)
...
def make_convert_kernel(*, head_dim, dtype, max_topk):
    ...
    def convert(ws_dkv: txl.gptr[txl.f32], dkv: txl.gptr[elem], seqlen_kv: txl.i32):
        ...
        with txl.If(seq_id < seqlen_kv), txl.Then():
            row = seq_id * head_dim
            for i in range(head_dim_main // 64):
                for j in range(2):
                    src = tidx + j * 32 + i * 64
                    v = txl.local_scalar(txl.f32)
                    txl.ptx["ld.global.b32"](v, ws_dkv.ptr_to([row + src]))
                    ...
                    nv = txl.local_scalar("uint16")
                    txl.ptx[_CVT_NARROW[dtype]](nv, v)
                    txl.ptx["st.global.b16"](dkv.ptr_to([row + dim]), nv)
            if has_tail:
                for j in range(2):
...
def make_sum_dsink_kernel(*, num_head):
    ...
    def sum_dsink(
        ...
        log2_e = txl.float32(1.4426950408889634)
        sink_v = txl.local_scalar(txl.f32)
        txl.ptx["ld.global.b32"](sink_v, attn_sink.ptr_to([head]))
        sink_log2 = txl.local_scalar(txl.f32)
        txl.ptx["mul.f32"](sink_log2, sink_v, log2_e)
        ...
        q_end = txl.local_scalar(txl.i32, init=(q_block + 1) * DSINK_BLOCK_Q)
        with txl.If(q_end > seqlen_q), txl.Then():
            txl.assign(q_end, seqlen_q)

        acc = txl.local_scalar(txl.f32, init=txl.float32(0.0))
        q_idx = txl.local_scalar(txl.i32, init=q_block * DSINK_BLOCK_Q + tid)
        with txl.While(q_idx < q_end):
            slot = q_idx * num_head + head
            sl = txl.local_scalar(txl.f32)
            txl.ptx["ld.global.b32"](sl, ws.ptr_to([plane + slot]))
            arg = txl.local_scalar(txl.f32)
            txl.ptx["add.f32"](arg, sink_log2, sl)
            p_sink = txl.local_scalar(txl.f32)
            txl.ptx["ex2.approx.f32"](p_sink, arg)
            odo = txl.local_scalar(txl.f32)
            txl.ptx["ld.global.b32"](odo, ws.ptr_to([slot]))
            txl.ptx["fma.rn.f32"](acc, p_sink, odo, acc)
            txl.assign(q_idx, q_idx + DSINK_THREADS)

        total = _butterfly_sum_f32(acc, (1, 2, 4, 8, 16))

        with txl.If(tid == 0), txl.Then():
            prev = txl.local_scalar(txl.f32)
            txl.ptx.atom.global_.add.f32(prev, d_sink.ptr_to([head]), total)
