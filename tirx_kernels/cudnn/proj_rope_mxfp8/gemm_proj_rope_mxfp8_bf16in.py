...
def _make_kernel(tokens, k_dim, num_heads, w_out_in):
    ...
    def kernel(x, w, cos, sin, out_fp8_row, out_scales_row, out_fp8_col, out_scales_col, *, host):
                            ...
                            with K.If(_elected()):
                                with K.Then():
                                    K.ptx[_TMA_G2S_2D](
                                        ...
                                        + copy * 8192,
                                        K.address_of(b_map),
                                        K.cast(head * _HEAD_DIM + copy * 64, "int32"),
                                        K.int32(k_tile * 64),
                                        ab_pipe.full.ptr_to([tma_state.stage]),
                                        K.uint16(1),
                                        K.uint64(0),
        ...
        with epilogue_role:
            ...
            drain = K.alloc_local((32,), "float32")
            staged_words = K.alloc_local((16,), "uint32")
            ...
            with K.While(work < total_work):
                ...
                with K.If(fc == 2):
                    with K.Then():
                        ...
                        for row in range(_BLOCK):
                            ...
                            K.ptx.ld.global_.v2.b16(cos0_b16, cos1_b16, cos.ptr_to([trig_offset]))
                            K.ptx.ld.global_.v2.b16(sin0_b16, sin1_b16, sin.ptr_to([trig_offset]))
                            ...
                            K.assign(values0[row], v0)
                            K.assign(values1[row], v1)
                ...
                for row in range(_BLOCK):
                    ...
                    row_scale = K.cast(
                        K.bitwise_and(K.cast(row_scale_pair, "uint32"), K.uint32(0xFF)), "uint8"
                    )
    ...
    kernel.__annotations__ = {
        "x": K.gptr[K.bf16, (tokens * k_dim,)],
        "w": K.gptr[K.bf16, (num_heads * _HEAD_DIM * k_dim,)],
