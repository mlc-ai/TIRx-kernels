...
def _load_state_row(pointer, index, count, state_dtype):
    ...
    """
    values = txl.alloc_local((count,), "float32")
    if state_dtype == "float32":
        ...
        if run_order:
            ...
            with txl.If(thread == 0), txl.Then():
                for cell in range(_SCHED_ALL_CELLS):
                    txl.ptx.st.global_.s32(scheduler.ptr_to([cell]), txl.int32(0))
                    ...
                    with txl.serial(n_local, unroll=False) as local_idx:
                        ...
                        for sub in range(2):
                            txl.ptx["tcgen05.st.sync.aligned.16x128b.x8.b32"](
                                txl.cast(
                                    tmem_col + _TM_Y + txl.shift_left(row_id + sub * 16, txl.int32(16)),
                                    "uint32",
                                ),
        ...
        with epilogue:
            ...
            with txl.While(tile < total_tiles):
                ...
                if checkpoints:
                    ...
                    ckpt_chunks = txl.local_scalar("int32", init=checkpoint_every_n // _BT)
                    ...
                    # store.
                    ckpt_coord = txl.local_scalar(
                        "int32", init=(wstart + ckpt_chunks - 1) // ckpt_chunks
                    )
                    ckpt_mod = txl.local_scalar("int32", init=cstart % ckpt_chunks)
                    with txl.If(_elected()), txl.Then():
                    ...
                    with txl.If(n_local > 0), txl.Then():
                        with txl.serial(n_local, unroll=False) as local_idx:
                            ...
                            with (
                                txl.If(txl.And(txl.And(chunk >= wstart, chunk < wend), ckpt_mod == 0)),
                                txl.Then(),
                            ):
