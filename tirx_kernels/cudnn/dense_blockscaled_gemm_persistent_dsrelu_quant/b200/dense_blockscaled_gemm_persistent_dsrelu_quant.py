...
def _issue_ptx_if(predicate, mnemonic, *args):
    with txl.If(predicate):
        with txl.Then():
            txl.ptx[mnemonic](*args)
    ...
    if bits == 16:
        if dtype == "bfloat16":
            for word in range(16):
                ...
                txl.ptx.shl.b32(low_bits, words[word], txl.uint32(16))
                txl.ptx.and_.b32(high_bits, words[word], txl.uint32(0xFFFF0000))
                txl.assign(values[2 * word], txl.reinterpret("float32", low_bits))
                txl.assign(values[2 * word + 1], txl.reinterpret("float32", high_bits))
            return
        for word in range(16):
            low = txl.local_scalar("uint16")
            high = txl.local_scalar("uint16")
            txl.ptx.mov.b32(low, high, words[word])
            txl.ptx.cvt.f32.f16(values[2 * word], low)
            txl.ptx.cvt.f32.f16(values[2 * word + 1], high)
    else:
...
def _make_kernel(
    ...
    def kernel(a, b, sfa, sfb, c, d, prob, dprob, amax, alpha, *, host):
        ...
        txl.ptx.bar.sync(txl.uint32(0), txl.uint32(224))
        ...
        with c_role:
            c_state = txl.PipelineState(c_stages, phase=1)
            work = txl.local_scalar("int32", init=cluster_work_id)
            with txl.While(work < cluster_work):
                tile_m_idx, tile_n_idx, batch_idx = scheduler_coords(work)
                ...
                with txl.While(subtile < epilogue_subtiles):
                    ...
                    with txl.If(_elected()):
                        with txl.Then():
                            txl.ptx.mbarrier.arrive.expect_tx.shared.b64(
                                c_pipe.full.ptr_to([c_state.stage]), txl.uint32(c_stage_bytes)
                            )
                            txl.ptx[
                                "cp.async.bulk.tensor.3d.shared::cta.global.tile"
                                ".mbarrier::complete_tx::bytes.L2::cache_hint"
        ...
        with epilogue_role:
            ...
            c_state = txl.PipelineState(c_stages, phase=0)
            work = txl.local_scalar("int32", init=cluster_work_id)
            executed_tiles = txl.local_scalar("int32", init=0)

            values = txl.alloc_local((32,), "float32")
            c_values = txl.alloc_local((32,), "float32")
            d_values = txl.alloc_local((32,), "float32")
            dprob_terms = txl.alloc_local((32,), "float32")
            c_words = txl.alloc_local((32,), "uint32")
            d_words = txl.alloc_local((32,), "uint32")
            d_pairs = txl.alloc_local((16,), "uint16")
            d_wide = txl.alloc_local((16,), "uint64")
            d_offsets = txl.alloc_local((8,), "int32")
            absolute_values = txl.alloc_local((32,), "float32")
            maxima = txl.alloc_local((11,), "float32")
            tile_amax = txl.local_scalar("float32")
            warp_amax = txl.local_scalar("float32")
            prob_value = txl.local_scalar("float32")
            dprob_value = txl.local_scalar("float32")
            ...
            def load_c():
                ...
                with txl.If(lane == 0):
                    with txl.Then():
                        txl.ptx.mbarrier.arrive.shared.b64(c_pipe.empty.ptr_to([c_state.stage]))
                _advance(c_state)
            ...
            def apply_dsrelu_and_dprob():
                for index in range(32):
                    txl.ptx.max.f32(values[index], values[index], txl.float32(0.0))
                for index in range(0, 32, 2):
                    c_prob_0 = txl.local_scalar("float32")
                    c_prob_1 = txl.local_scalar("float32")
                    doubled_0 = txl.local_scalar("float32")
                    doubled_1 = txl.local_scalar("float32")
                    squared_0 = txl.local_scalar("float32")
                    squared_1 = txl.local_scalar("float32")
                    multiply_pair(
                        c_prob_0,
                        c_prob_1,
                        c_values[index],
                ...
                subtile_sum = txl.local_scalar("float32", init=txl.float32(0.0))
                for index in range(31, -1, -1):
                    txl.ptx.add.f32(subtile_sum, subtile_sum, dprob_terms[index])
                txl.ptx.add.f32(dprob_value, dprob_value, subtile_sum)

            def update_amax():
                for index in range(32):
                    txl.ptx.abs.f32(absolute_values[index], d_values[index])
                ...
                txl.ptx.max.NaN.f32(
                    maxima[10], absolute_values[30], absolute_values[31], txl.float32(0.0)
                )
                ...
                txl.ptx.max.NaN.f32(maxima[3], maxima[9], maxima[10], txl.float32(0.0))
                txl.ptx.max.NaN.f32(maxima[0], maxima[0], maxima[1], maxima[2])
                subtile_amax = txl.local_scalar("float32")
                txl.ptx.max.NaN.f32(subtile_amax, maxima[0], maxima[3], txl.float32(0.0))
            ...
            with txl.While(work < cluster_work):
                ...
                txl.assign(dprob_value, txl.float32(0.0))
                txl.assign(prob_value, txl.float32(0.0))
                global_row = tile_m_idx * cta_m + warp * 32 + lane
                with txl.If(global_row < M):
                    with txl.Then():
                        prob_offset = batch_idx * (m_tiles * cta_m) + global_row
                        txl.ptx.ld.global_.b32(prob_value, prob.ptr_to([prob_offset]))

                subtile = txl.local_scalar("int32", init=0)
                with txl.While(subtile < epilogue_subtiles):
                    linear_subtile = executed_tiles * epilogue_subtiles + subtile
                    d_stage = txl.bitwise_and(linear_subtile, txl.int32(1))
                    ...
                    with txl.If((warp == 0) & (lane == 0)):
                        with txl.Then():
                            ...
                            txl.ptx.cp.async_.bulk.wait_group.read(d_stages - 1)
                    txl.ptx.bar.sync(txl.uint32(1), txl.uint32(128))
                    txl.assign(subtile, subtile + 1)
                ...
                with txl.If(global_row < M):
                    with txl.Then():
                        atomic_old = txl.local_scalar("float32")
                        txl.ptx.atom.global_.add.f32(
    ...
    kernel.__annotations__ = {
        ...
        "d": txl.gptr[txl.u8, (M * N * L * d_bits // 8,)],
        "prob": txl.gptr[txl.f32, (m_tiles * cta_m * L,)],
        "dprob": txl.gptr[txl.f32, (M * L,)],
