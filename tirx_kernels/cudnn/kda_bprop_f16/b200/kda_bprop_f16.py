...
from tirx_kernels.bench.runner import PREPARE_CUDA_ARCH_ENV, hardware_num_sms
...
def _make_main(
    *,
    ...
    def main(
        ...
        beta: txl.gptr[beta_dtype],
        cu_seqlens: txl.gptr[txl.i32],
        dgate: txl.gptr[txl.f32],
        dbeta: txl.gptr[beta_dtype],
        ...
        with super_mma:
            ...
            with txl.While(tile < total_tiles):
                ...
                with txl.serial(num_chunks, unroll=False) as reverse_index:
                    ...
                    beta_lo = txl.local_scalar("float32")
                    beta_hi = txl.local_scalar("float32")
                    ...
                    for accum_index in range(8):
                        ...
                        lower = txl.if_then_else(row_coord > col_coord, kk[accum_index], 0.0)
                        beta_value = beta_lo if accum_index % 4 < 2 else beta_hi
                        txl.assign(strict[accum_index], lower * beta_value)
                    for pair in range(4):
                        txl.assign(
                    ...
                    bsum_lo = txl.local_scalar("float32", init=txl.float32(0.0))
                    bsum_hi = txl.local_scalar("float32", init=txl.float32(0.0))
                    for accum_index in range(8):
                        row_coord = row_hi if accum_index % 4 >= 2 else row_lo
                        col_coord = (accum_index // 4) * 8 + 2 * (lane % 4)
                        if accum_index % 2:
                        ...
                        contribution = txl.if_then_else(
                            row_coord > col_coord, dm[accum_index] * kk[accum_index], 0.0
                        )
                        if accum_index % 4 < 2:
                            txl.assign(bsum_lo, bsum_lo + contribution)
                        else:
                            txl.assign(bsum_hi, bsum_hi + contribution)
                    for pair in range(4):
                        txl.assign(
                    ...
                    txl.assign(bsum_lo, bsum_lo + _shuffle_xor_f32(bsum_lo, 1))
                    txl.assign(bsum_lo, bsum_lo + _shuffle_xor_f32(bsum_lo, 2))
                    txl.assign(bsum_hi, bsum_hi + _shuffle_xor_f32(bsum_hi, 1))
                    txl.assign(bsum_hi, bsum_hi + _shuffle_xor_f32(bsum_hi, 2))
                    with txl.If(lane % 4 == 0), txl.Then():
                        txl.ptx.st.shared.f32(arena.ptr_to([_SMEM_BETA_M + row_lo * 4]), -bsum_lo)
                        txl.ptx.st.shared.f32(arena.ptr_to([_SMEM_BETA_M + row_hi * 4]), -bsum_hi)
                    txl.ptx.fence.proxy.async_.shared__cta()
        ...
        with tcgen:
            ...
            neg_beta_dy_consumer = txl.PipelineState(1, phase=0)
        ...
        with cg0:
            ...
            with txl.While(tile < total_tiles):
                ...
                with txl.serial(num_chunks, unroll=False) as reverse_index:
                    ...
                    with txl.If(warp_in_group == 0), txl.Then():
                        beta_stage = serial % 4
                        _wait_barrier(arena, 216, beta_stage, (serial // 4 + 1) & 1)
                        with txl.If(lane < 16), txl.Then():
                            beta_value = txl.local_scalar("float32", init=txl.float32(0.0))
                            with txl.If(token_base + lane < sequence_length), txl.Then():
                                beta_index = (
                                    txl.cast(bos + token_base + lane, "int64") * n_heads_out + head
                                )
                                if beta_sigmoid:
                                    beta_bits = txl.local_scalar("uint16")
                                    txl.ptx.ld.global_.b16(beta_bits, beta.ptr_to([beta_index]))
                                    txl.ptx.cvt.f32.bf16(beta_value, beta_bits)
                                    txl.assign(beta_value, _tanh_approx(beta_value * 0.5) * 0.5 + 0.5)
                                    # The source rounds sigmoid(beta) through BF16.
                                    beta_round = txl.cast(beta_value, "bfloat16")
                                    txl.assign(beta_value, txl.cast(beta_round, "float32"))
                                else:
                                    txl.ptx.ld.global_.f32(beta_value, beta.ptr_to([beta_index]))
                    ...
                    with txl.unroll(8) as pair:
                        q_lo_bits = txl.local_scalar("uint16")
                        q_hi_bits = txl.local_scalar("uint16")
                        k_lo_bits = txl.local_scalar("uint16")
                        k_hi_bits = txl.local_scalar("uint16")
                        ...
                        txl.assign(
                            q_words[pair],
                            txl.bitwise_or(
                                txl.cast(q_lo_bits, "uint32"),
                                txl.shift_left(txl.cast(q_hi_bits, "uint32"), txl.uint32(16)),
                            ),
                        )
                        txl.assign(
                            k_words[pair],
                            txl.bitwise_or(
                                txl.cast(k_lo_bits, "uint32"),
                                txl.shift_left(txl.cast(k_hi_bits, "uint32"), txl.uint32(16)),
                            ),
                        )
                    q_tmem = (
                    ...
                    with txl.unroll(2) as half:
                        ...
                        with txl.unroll(4) as pair:
                            ...
                            txl.ptx.mul.bf16x2(decay_words[pair], k_pair, exp_pair)
        ...
        with cg2:
            ...
            with txl.While(tile < total_tiles):
                ...
                with txl.serial(num_chunks, unroll=False) as reverse_index:
                    ...
                    for slot in range(4):
                        txl.assign(last_dot[slot], txl.float32(0.0))
                    ...
                    txl.ptx.tcgen05.wait__ld.sync.aligned()
                    with txl.unroll(16) as token:
                        txl.assign(dgate_values[token], -gate_decay[token] * decay[token])
                    ...
                    with txl.unroll(16) as token:
                        ...
                        txl.assign(
                            ...
                            + k_value * (txl.float32(2.0) * dgate_values[token] - dk_values[token]),
                        )
                    txl.assign(
                        dgate_values[15],
        ...
        with cg1:
            ...
            with txl.While(tile < total_tiles):
                ...
                with txl.serial(num_chunks, unroll=False) as reverse_index:
                    ...
                    _wait_barrier(arena, 184, beta_stage, (serial // 4) & 1)
                    beta_pairs = txl.alloc_local((2,), "uint32")
                    for half in range(2):
                        token0 = ((half * 4 + (lane & 3)) ^ 4) * 2
                        beta0 = txl.local_scalar("float32")
                        beta1 = txl.local_scalar("float32")
                        txl.ptx.ld.shared.f32(
                        ...
                        txl.assign(beta_pairs[half], _pack_bf16_pair(beta0, beta1))
                    ...
                    for reg in range(4):
                        ...
                        txl.ptx.mul.bf16x2(y_pack0[reg ^ 2], beta_pairs[reg // 2], raw_v0[raw_matrix])
                        txl.ptx.mul.bf16x2(y_pack1[reg ^ 2], beta_pairs[reg // 2], raw_v1[raw_matrix])
                    first_state_chunk = 0 if use_initial_state else 1
                    with txl.If(chunk >= first_state_chunk), txl.Then():
                        ...
                        for reg in range(4):
                            ...
                            txl.ptx.sub.bf16x2(difference0[reg ^ 2], raw_v0[raw_matrix], state_pair0)
                            txl.ptx.sub.bf16x2(difference1[reg ^ 2], raw_v1[raw_matrix], state_pair1)
                            txl.ptx.mul.bf16x2(
                    ...
                    txl.ptx["tcgen05.st.sync.aligned.16x128b.x2.b32"](
                        txl.cast(
                            tmem_col + _TMEM_Y_NEG_BETA_DY + txl.shift_left(row_lo, txl.int32(16)),
                            "uint32",
                        ),
                    ...
                    txl.ptx["tcgen05.st.sync.aligned.16x128b.x2.b32"](
                        txl.cast(
                            tmem_col + _TMEM_Y_NEG_BETA_DY + txl.shift_left(row_hi, txl.int32(16)),
                            "uint32",
                        ),
                    ...
                    beta_values = txl.alloc_local((4,), "float32")
                    ...
                    beta_self = txl.local_scalar("float32", init=txl.float32(0.0))
                    if beta_sigmoid:
                        with txl.If(group_thread < 16), txl.Then():
                            txl.ptx.ld.shared.f32(
                                beta_self,
                    ...
                    beta_dy0 = txl.alloc_local((8,), "float32")
                    beta_dy1 = txl.alloc_local((8,), "float32")
                    with txl.unroll(4) as pair:
                        beta_lo = txl.if_then_else(pair >= 2, beta_values[2], beta_values[0])
                        beta_hi = txl.if_then_else(pair >= 2, beta_values[3], beta_values[1])
                        product_lo, product_hi = _fmul2(
                            dy0[pair * 2], dy0[pair * 2 + 1], beta_lo, beta_hi
                        )
                        txl.assign(beta_dy0[pair * 2], product_lo)
                        txl.assign(beta_dy0[pair * 2 + 1], product_hi)
                        product_lo, product_hi = _fmul2(
                            dy1[pair * 2], dy1[pair * 2 + 1], beta_lo, beta_hi
                        )
                        txl.assign(beta_dy1[pair * 2], product_lo)
                        txl.assign(beta_dy1[pair * 2 + 1], product_hi)
                    neg0 = txl.alloc_local((4,), "uint32")
                    neg1 = txl.alloc_local((4,), "uint32")
                    dv_pack0 = txl.alloc_local((4,), "uint32")
                    dv_pack1 = txl.alloc_local((4,), "uint32")
                    for pair in range(4):
                        txl.assign(
                            neg0[pair],
                    ...
                    token_sums = txl.alloc_local((4,), "float32")
                    for token_sum in range(4):
                        txl.assign(token_sums[token_sum], txl.float32(0.0))
                    for pair in range(4):
                        diff0_lo = txl.local_scalar("float32")
                        diff0_hi = txl.local_scalar("float32")
                        diff1_lo = txl.local_scalar("float32")
                        diff1_hi = txl.local_scalar("float32")
                        ...
                        txl.assign(token_sums[sum_lo], accum_lo)
                        txl.assign(token_sums[sum_hi], accum_hi)
                    ...
                    txl.ptx.bar.sync(txl.uint32(4), txl.uint32(128))
                    with txl.If(group_thread < 16), txl.Then():
                        dbeta_value = txl.local_scalar("float32", init=txl.float32(0.0))
                        for reduction_warp in range(4):
                            partial = txl.local_scalar("float32")
                            txl.ptx.ld.shared.f32(
                                partial,
                                arena.ptr_to(
                            ...
                            txl.assign(dbeta_value, dbeta_value + partial)
                        beta_m = txl.local_scalar("float32")
                        txl.ptx.ld.shared.f32(beta_m, arena.ptr_to([_SMEM_BETA_M + group_thread * 4]))
                        txl.assign(dbeta_value, dbeta_value + beta_m)
                        if beta_sigmoid:
                            txl.assign(dbeta_value, dbeta_value * (beta_self - beta_self * beta_self))
                        token_index = chunk * 16 + group_thread
                        with txl.If(txl.And(token_index < sequence_length, chunk < wend)), txl.Then():
                            output_index = txl.cast(bos + token_index, "int64") * n_heads_out + head
                            if beta_sigmoid:
                                txl.ptx.st.global_.b16(
                                    dbeta.ptr_to([output_index]),
                                    txl.reinterpret("uint16", txl.cast(dbeta_value, "bfloat16")),
                                )
                            else:
                                txl.ptx.st.global_.f32(dbeta.ptr_to([output_index]), dbeta_value)
                    txl.ptx.bar.sync(txl.uint32(4), txl.uint32(128))
