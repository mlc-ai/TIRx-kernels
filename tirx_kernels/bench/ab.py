...
from tirx_kernels.bench.provenance import collect_repo_git
from tirx_kernels.bench.ratio_diff import build_report

_SHARED_HARNESS_PATHS = (
    Path("tirx_kernels/bench"),
    Path("tirx_kernels/__init__.py"),
    Path("tirx_kernels/basic/_shared/_runtime.py"),
)
...
def _copy_shared_harness(after_root: Path, before_root: Path) -> None:
    legacy = (before_root / "tirx_kernels/runner.py").is_file()
    for relative in _SHARED_HARNESS_PATHS:
        source = after_root / relative
        destination = before_root / relative
        if legacy and relative == Path("tirx_kernels/basic/_shared/_runtime.py"):
            destination = before_root / "tirx_kernels/basic/utils/_runtime.py"
        if source.is_dir():
...
def _extract_before_tree(repo: Path, revision: str, destination: Path) -> None:
    """Materialize the revision's kernel, substrate, and execution packages."""
    # A/B may straddle the directory migration. Older revisions keep their
    # original source tree and import paths; the worker bridges only the harness.
    present = set(_git(repo, "ls-tree", "--name-only", revision).splitlines())
    roots = [root for root in remote.TREE_ROOTS if root in present]
...
def run_ab(
    ...
    *,
    ...
    before_tree = _git(after_root, "rev-parse", f"{before_revision}^{{tree}}")
    after_tree = _git(after_root, "rev-parse", f"{after_revision}^{{tree}}")

    api = remote.load_kcoral()
    client, health = remote.connect(api, server_url)
    cuda_arch = health["target"]["arch"]
    from tirx_kernels.bench.suite import partition_workloads_by_arch, validate_workload_archs
    ...
    with tempfile.TemporaryDirectory(prefix="tirx-bench-ab-") as temporary:
        ...
        trees = {
            "after": remote.build_tree_archive(after_root),
            "before": remote.build_tree_archive(before_root),
        }
