...
Quick start:
    ...
    python tirx_kernels/bench/promote_baseline.py .bench-suite/runs/<id>.json
...
from tirx_kernels.bench.provenance import (  # re-exported for ab.py, tests, tvm
...
from tirx_kernels.bench.provenance import module_repo_root as _module_repo_root  # noqa: F401
from tirx_kernels.bench.provenance import tir_repo_root as _tir_repo_root  # noqa: F401
from tirx_kernels.bench.registry import kernel_index
from tirx_kernels.bench.remote import (
...
from tirx_kernels.bench.runner import DEFAULT_BENCH_COOLDOWN_S as DEFAULT_COOLDOWN_S
from tirx_kernels.bench.runner import DEFAULT_BENCH_ROUNDS as DEFAULT_ROUNDS

try:
    from tirx_kernels.bench.impls import our_impls
except ModuleNotFoundError:  # Support `python tirx_kernels/bench/suite.py`.
...
# Configs live beside each task's device directories: <task_set>/<task>/<kernel>.yaml.
...
# (also available on ``python -m tirx_kernels.bench.run``) and feed the ratio
...
def load_kernel_configs(kernel: str, config_dir: Path = CONFIG_DIR) -> list[dict]:
    ...
    """
    matches = sorted(config_dir.glob(f"*/*/{kernel}.yaml"))
    if not matches:
...
def load_config_dir(config_dir: Path = CONFIG_DIR) -> list[dict]:
    """The pinned sweep: every `default: true` config in task-local YAML files.
    ...
    files live directly under each ``<task_set>/<task>/`` directory.
    """
...
def write_generated_workloads(workloads: list[dict], path: Path) -> Path:
    ...
    header = (
        "# Generated by bench_suite from tirx_kernels/*/*/*.yaml\n"
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        "`default: true` config from tirx_kernels/*/*/*.yaml)",
    )
    ap.add_argument(
        "--out-dir",
        type=Path,
    ...
    if args.check_imports:
        if args.ab_before:
            ...
            sys.exit(2)
        from tirx_kernels.bench.registry import check_workload_imports
    ...
    if args.ab_before:
        ...
        if args.with_references:
            ...
            sys.exit(2)
        from tirx_kernels.bench.ab import run_ab
    ...
    tree = remote.build_tree_archive(_kernels_repo_root())
