...
Every workload becomes one ``POST /execute`` request: the local task sets, substrate, and harness
...
TREE_ARCNAME = "."
TREE_ROOTS = ("tirx_kernels",)
# Only source roots ship; repository state, build outputs and local artifacts do not.
TREE_EXCLUDES = (
    "__pycache__",
    "*.pyc",
    "*.pyo",
    ".bench-suite",
    "tirx_kernels/bench/baseline.json",
    "tirx_kernels/bench/baseline.md",
    "tirx_kernels/bench_suite/baseline.json",
...
def _suite():
    ...
    from tirx_kernels.bench import suite as run
...
def build_tree_archive(
    ...
    source_files = (path for root in TREE_ROOTS for path in (package_dir / root).rglob("*"))
    ...
    with tarfile.open(fileobj=raw, mode="w", format=tarfile.PAX_FORMAT) as archive:
        for relative, path in files:
            ...
            info = tarfile.TarInfo(str(Path(arcname) / relative))
            info.size = len(data)
            info.mtime = 0
            info.uid = info.gid = 0
...
def pipeline_metadata(
    *,
    ...
    from tirx_kernels.bench.runner import DEFAULT_BENCH_COOLDOWN_S, DEFAULT_BENCH_ROUNDS
