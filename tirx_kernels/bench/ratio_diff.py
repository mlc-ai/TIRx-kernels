...
try:
    from tirx_kernels.bench.impls import is_our_impl
...
def _load_config_dir() -> list[dict]:
    try:
        from tirx_kernels.bench.suite import load_config_dir
    except ModuleNotFoundError:  # Support direct script execution.
...
def _expected_keys(cuda_arch: str | None = None) -> tuple[set[tuple[str, str]], list[str]]:
    ...
    try:
        ...
        if cuda_arch is not None:
            try:
                from tirx_kernels.bench.suite import partition_workloads_by_arch
            except ModuleNotFoundError:  # Support direct script execution.
