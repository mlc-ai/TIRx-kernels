...
Entry points (see ``tirx_kernels/bench/remote.py`` for the client side):
...
def _install_tree(tree_root: str) -> None:
    """Import the shipped task sets, substrate and harness in this process and its children."""
    ...
    if os.path.isfile(os.path.join(tree_root, "tirx_kernels", "runner.py")):
        # Share the current harness with a pre-migration A/B revision without
        # rewriting its kernel source or changing its substrate.
        namespace = importlib.import_module("tirx_kernels")
        for leaf in ("runner", "registry", "reference_requirements"):
            module = importlib.import_module("tirx_kernels.bench." + leaf)
            sys.modules["tirx_kernels." + leaf] = module
            setattr(namespace, leaf, module)
        shared = importlib.import_module("tirx_kernels.basic.utils")
        sys.modules["tirx_kernels.basic._shared"] = shared
        sys.modules["tirx_kernels.basic._shared._runtime"] = importlib.import_module(
...
def _bind_visible_device() -> dict:
    ...
    """
    try:
        from tirx_kernels.bench.runner import bind_visible_cuda_assignment
...
def probe(tar_bytes: bytes, options_json: str) -> str:
    ...
    try:
        ...
        import tirx_kernels.bench.runner
...
def run(spec_json: str) -> str:
    ...
    try:
        ...
        from tirx_kernels.bench.runner import close_prepared_kernel_bench, run_prepared_kernel_bench
