...
# effects (tvm registrations, compile hooks) never run twice. ``tirx-lite`` is
...
AB_SHARED_MODULE_PREFIXES = ("tirx_kernels.bench", "tirx_kernels.basic._shared._runtime")
...
def ab_current_benchmark_module(module: ModuleType) -> ModuleType:
    ...
    saved = {
        ...
        if name == "tirx_kernels"
        or name.startswith("tirx_kernels.")
    }
    for name in saved:
    ...
    saved_namespace = registry.tirx_kernels
    registry.tirx_kernels = package
    try:
        # Public kernel names survive directory moves; Python module paths do not.
        public_name = getattr(module, "KERNEL_META", {}).get("name")
        record = registry.kernel_index(strict=True).get(public_name) if public_name else None
        current_module = importlib.import_module(record.module_name if record else module.__name__)
    finally:
        registry.tirx_kernels = saved_namespace
        for name in [
            n
            for n in sys.modules
            if n == "tirx_kernels"
            or n.startswith("tirx_kernels.")
        ]:
            del sys.modules[name]
...
def run_kernel_test(kernel_name: str, config: dict[str, Any], *, registry=None):
    ...
    from tirx_kernels.bench.reference_requirements import unmet_reference_requirements
