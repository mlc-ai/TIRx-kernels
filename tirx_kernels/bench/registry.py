...
import tirx_kernels
from tirx_kernels.bench.reference_requirements import (
    ReferenceRequirement,
    parse_reference_requirements,
)
from tirx_kernels.bench.reference_requirements import (
...
# Directory labels organize implementations; runtime_cuda_archs remains the
# authoritative execution allowlist (a b200 implementation may support more GPUs).
DEVICE_DIRS = ("b200", "gb300", "rubin")
...
def _kernels_root() -> Path:
    return Path(next(iter(tirx_kernels.__path__))).resolve()


def _legacy_layout() -> bool:
    """An extracted A/B before revision may predate the task directories."""
    return (_kernels_root() / "runner.py").is_file()


def discover_categories() -> list[str]:
    """Find task sets in a checkout or wheel without importing optional libraries."""
    root = _kernels_root()
    out: list[str] = []
    for child in sorted(root.iterdir()):
        if child.name.startswith(("_", ".")) or not child.is_dir():
            continue
        if _legacy_layout():
            if child.name not in {"bench", "test", "bench_suite", "tirx_lite", "experimental"}:
                if (child / "__init__.py").is_file():
                    out.append(child.name)
            continue
        if any(next(child.glob(f"*/{device}/*.py"), None) for device in DEVICE_DIRS):
            out.append(child.name)
    return out


def _iter_kernel_module_names(pkg_path: Path) -> list[str]:
    """Walk task/device files; private helper packages are never registry entries."""
    out: list[str] = []
    for path in sorted(pkg_path.rglob("*.py")):
        ...
        if _legacy_layout():
            if "utils" in relative.parts:
                continue
        elif len(relative.parts) < 3 or relative.parts[1] not in DEVICE_DIRS:
            continue
        if any(part.startswith("_") for part in relative.parts):
            continue
        out.append(".".join(relative.with_suffix("").parts))
    return out
    ...
    if args.format == "json":
        ...
        for name, mod in sorted(all_kernels.items()):
            for cfg in getattr(mod, "CONFIGS", []):
                ...
                print(
                    f"{name}|{label}|python -m tirx_kernels.bench.run --kernel {name} "
