...
`python -m tirx_kernels.bench run` for single-workload diagnostics.
...
single-architecture kernels (`sm_107a`-only Rubin ports, the `sm_100a`-only
FlexAttention backward port, `sm_103a`-only fast.cu NVFP4 GEMM, FP4 FA4
forward, MSA and FlexAttention forward ports), and three rows from the
`sm_100a`/`sm_103a` FlexAttention hd256 forward. Thus an SM107 run selects 253
rows, an SM103 run 253, and an SM100 run 247.
...
The `tirx_kernels/` package, including its task sets, `tirx_lite/`, and `bench/`,
is shipped with each request (a deterministic
...
```bash
python -m tirx_kernels.bench suite --label before-rewrite
...
python -m tirx_kernels.bench suite \
  --label after-rewrite \
  --baseline tirx_kernels/bench/baseline.json \
  --threshold 1.0
```
...
```bash
python tirx_kernels/bench/ratio_diff.py \
