...
description: Use when porting, tuning, or diagnosing TIRx CUDA kernels, especially for perf-gate failures, PTX/SASS divergence, bitwise mismatches, register spills, scoreboard stalls, address arithmetic, predication, uniformity, pipeline depth, TMA, shared-memory conflicts, exact floating-point semantics, packing, or unstable benchmark ratios. Provides symptom-indexed instruction-selection field notes and measured verification guidance.
---

# TIRx instruction-selection field notes

Use these notes for TIRx -> TVM CUDA codegen -> nvcc/ptxas -> SASS work when
matching a hand-written CUDA, CuTe-DSL, or inline-assembly reference.

Start from the observed symptom. Search the `Symptoms:` lines in this file,
read each matching entry, compare generated PTX and SASS on both sides, change
one lever, then run the affected correctness and performance matrices:

```bash
rg -n -i '^\*\*Symptoms:\*\*.*long_scoreboard' \
  .agents/skills/tirx-instruction-selection/SKILL.md
```
...
shows a reusable mechanism. Key it by an observable symptom and include the
action, boundary, verification, and essential numbers in the prose itself.

Keep `Symptoms:` as the only index. Add two to five observable snake-case terms
per entry and reuse existing terms when they fit; do not maintain a separate
table or taxonomy.

Do not add commit hashes, run IDs, file paths, line numbers, raw artifacts,
baseline-only promotions, algorithm substitutions, or unmeasured CUDA folklore.
