...
A numerical-range extension in a 12-warp recurrent backward kernel showed why
the winning level still needs timing. Levels 10/8, 5, and 2 assembled with
112, 16, and zero stack bytes, respectively. The representative ordinary
workload measured about 159, 155, and 162 us. Level 5 helped; eliminating the
last spills at level 2 made the schedule slower. All measured levels retained
the original per-output correctness checks.

A separately compiled ordinary specialization with a uniform GPU range check
kept 168 entry registers at both levels. Level 5 reduced its stack from 16 to
8 bytes and static LDL/STL counts from 26/36 to 13/35. Five alternating paired
rounds reduced complete-operation latency, including the guard and inactive
kernel launch, from 150.87 to 147.28 us; both passed the original checker. The
unmodified baseline was 136.58 us, so this compiler improvement still left a
7.83% operation-level cost. A lower spill count does not remove dispatch costs.

The ordinary specialization's lower-stack setting did not generalize across
head layouts. A wider-head workload stalled twice during repeated level5
measurements, while its original level10 reference completed. Restoring level10
for the specialization passed a five-round paired test and30 repeated timing
rounds on each of two affected layouts. The paired workload cost4.97% over
the original including the range guard and inactive launch. Both levels used
168 registers and identical matrix-issue counts; level10 used16 stack bytes
and26/36 local-load/store sites versus8 bytes and13/35 at level5. This is
an observed compiler-setting constraint, not proof of a compiler bug or a fix
for the underlying persistent protocol. Do not accept a smaller stack from
one shape without repeated execution across the dispatch domain.

A subsequent longer alternating campaign also stalled while executing the
unmodified level10 baseline on that workload. Since the two implementations
shared a CUDA context, this alone does not establish an independent baseline
defect; earlier candidate launches could have affected it. The level5 timeouts
do not establish compiler-setting causality, and successful level10 samples
do not establish a repaired dependency protocol. Treat the measured setting
as provisional until the broader repeated-launch behavior is understood;
retain and report unsuccessful requests instead of replacing them with only
successful timing samples.

A fresh process loading only the unmodified original subsequently stalled in
the same repeated workload. This confirms that the runtime issue predates the
optimization, while leaving its cause unresolved. None of these compiler levels
should be described as repairing that issue based on successful finite samples.

A further narrow-layout sweep preserved all six outputs bitwise on four shapes
and reduced complete-operation latency by 0.9–1.4% at level 5 versus 10.
It retained 168 registers while reducing the stack from 16 to 8 bytes and
static local-load/store sites from 26/36 to 13/35. The sibling layout retained
its 16-byte stack and reduced local sites from 45/46 to 33/45, yet four paired
shapes ranged from a 0.14% gain to a 1.30% regression. Fewer local instructions alone do not justify extending a setting to sibling
specializations. A subsequent original-versus-level5 campaign completed three
narrow-layout rows, then stalled in the level5 candidate's final timing round on
the fourth. This did not establish compiler causality, but the small improvement
was insufficient to accept the setting with that unresolved repeated-execution
result. The ordinary bodies retained their original level10 setting. Neither
setting is a proven repair for the independently reproduced runtime stall.
