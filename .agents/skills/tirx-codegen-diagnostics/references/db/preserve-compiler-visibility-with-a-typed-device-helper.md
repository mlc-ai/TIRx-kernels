...
A source-level CUDA helper is also not a reliable register-allocation escape.
In one extended KDA epilogue, a `__noinline__` source helper survived as 64
device calls and produced an 88-byte stack, 154/50 static local load/store
sites, 830 branch sites, and 204/204 BSSY/BSYNC sites. In the native epilogue,
forcing the corresponding helper inline removed a 32-FP32 source array but
increased stack from 56 to 96 bytes and local load/store sites from 25/46 to
44/65. The low-level call also required the exact callee name to be added to
the backend allow-list. Treat source helpers as an ABI/control-flow experiment,
not as evidence of shorter live ranges; keep a direct version as a final-SASS
negative control.
